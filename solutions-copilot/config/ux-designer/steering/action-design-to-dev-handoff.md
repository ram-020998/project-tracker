...
This action uses Atlas MCP tools to understand the app's existing data model, process patterns, and integration points, then produces a structured document that eliminates back-and-forth between design and dev.
...
Ask the user for:
1. **SAIL mockup** — The finalized interface code (pasted, file path, or Atlas interface name)
...
```
get_dependencies(app, "parent_interface_or_process")
...
```
search_objects(app, "validation", "Expression Rule")
search_objects(app, "permission", "Expression Rule")
search_objects(app, "query", "Expression Rule")
...
10. **Reference existing rules by name** — use Atlas to find real rule names
