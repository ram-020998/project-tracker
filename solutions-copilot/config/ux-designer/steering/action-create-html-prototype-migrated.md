...
## PHASE 1: Fetch, Flatten & Annotate SAIL Code (Solutions Intelligence MCP only)
...
All output files go into `{project}/solutions-intelligence-files/{task-name}/` where `{project}` is the sailwind-starter project path and `{task-name}` is a kebab-case name derived from the interface being prototyped (e.g., `evaluation-summary`, `document-review`).

### Step 1.1: Locate the interface and retrieve top-level SAIL code

Ask the user which interface/dashboard to prototype. Use Solutions Intelligence MCP tools:
- `solutions-intelligence.search_objects(app, query, "Interface")` to find by name
- `search_bundles(app, query, "action")` to find by feature
- `solutions-intelligence.get_bundle(app, bundle_id, "full")` or `solutions-intelligence.get_object_code(app, uuid)` to retrieve code

Also resolve the site context:
- Call `solutions-intelligence.get_dependencies(app, object_name)` — follow `called_by[]` up to the Site
- Extract: site name, pages/tabs, active page, how the interface is triggered

Create the output directory: `{project}/solutions-intelligence-files/{task-name}/`

### Step 1.2: Identify all sub-interfaces and sections

From the top-level SAIL code, identify every `rule!` reference. Save as `{project}/solutions-intelligence-files/{task-name}/rule-tree.md`.

### Step 1.3: Flatten and annotate EACH section individually

For EACH `rule!` reference identified in Step 1.2:

1. **Retrieve its code**: `solutions-intelligence.search_objects(app, rule_name, "Interface")` → `solutions-intelligence.get_object_code(app, uuid)`
2. **Recursively resolve** any nested `rule!` references until only basic Appian components remain
3. **Write an individual file** in `{project}/solutions-intelligence-files/{task-name}/sections/`
...
After ALL individual section files are written, create `{project}/solutions-intelligence-files/{task-name}/consolidated-spec.sail` that:
...
- `{project}/solutions-intelligence-files/{task-name}/rule-tree.md` exists
- All individual section files exist in `{project}/solutions-intelligence-files/{task-name}/sections/`
- `{project}/solutions-intelligence-files/{task-name}/consolidated-spec.sail` exists
...
**Goal**: Read the `solutions-intelligence-files/{task-name}/` folder from Phase 1, fetch design tokens and Aurora docs, and generate the HTML prototype. This phase MUST NOT call Solutions Intelligence MCP tools for SAIL code — all SAIL data comes from the solutions-intelligence-files.

### Step 2.1: Read the solutions-intelligence-files folder

Read the entire `{project}/solutions-intelligence-files/{task-name}/` folder:
...
Use the behavior descriptions from the solutions-intelligence-files to implement:
...
5. **solutions-intelligence-files folder is the handoff** — Phase 2 reads SAIL data ONLY from `solutions-intelligence-files/{task-name}/`.
