...
This action uses Aurora Design System docs and the official Appian SAIL documentation for understanding SAIL component capabilities and Solutions Intelligence MCP tools to find real-world examples of how similar patterns are implemented in existing Appian solutions.
...
For patterns classified as "workaround" or "not possible," search Solutions Intelligence for how existing apps handle similar needs:

```
solutions-intelligence.search_objects(app, "relevant_keyword", "Interface")
solutions-intelligence.get_object_code(app, "found_interface_name")
```
...
Use the following output format for the report:
    ...
    - **Existing example**: [If found in Solutions Intelligence — app name + interface name]
...
5. **Check Solutions Intelligence for real examples** — if a workaround exists in a real app, reference it
