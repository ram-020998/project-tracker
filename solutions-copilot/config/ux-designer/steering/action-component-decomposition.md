...
This action uses Atlas MCP tools to analyze how similar interfaces in the target app are structured and decomposed, then recommends a structure that follows those conventions.
...
```
get_bundle(app, bundle_id, object_type="Interface")
```

For each bundle, examine the entry-point interface and its dependencies:
```
get_object_code(app, "entry_point_interface")
get_dependencies(app, "entry_point_interface")
...
```
search_objects(app, "statusTag", "Interface")
search_objects(app, "avatar", "Interface")
```
