...
All output files go into `{project}/solutions-intelligence-files/{task-name}/` where `{task-name}` is a kebab-case name derived from the interface being prototyped (e.g., `evaluation-summary`, `document-review`).
...
From the top-level SAIL code, identify every `rule!` reference. Each `rule!` is a sub-interface or section of the page. Create a list and save it as `{project}/solutions-intelligence-files/{task-name}/rule-tree.md`:
...
**Goal**: Read AGENTS.md, read the entire `solutions-intelligence-files/{task-name}/` folder from Phase 1, and generate the React prototype. This phase MUST NOT call Solutions Intelligence MCP tools.
...
### Step 2.2: Read the solutions-intelligence-files folder
...
**Follow ALL rules from AGENTS.md.** The solutions-intelligence-files tell you WHAT to build and HOW it should behave; AGENTS.md tells you HOW to build it with Sailwind components.
...
5. **solutions-intelligence-files folder is the handoff** — Phase 2 reads from `solutions-intelligence-files/{task-name}/` and AGENTS.md only. It does NOT call Solutions Intelligence MCP.
