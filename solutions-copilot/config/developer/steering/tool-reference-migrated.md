...
Complete reference for all 20 Appian Solutions Intelligence MCP tools.
...
Key v3 changes:
- **Code separated from metadata** — use `solutions-intelligence.get_object_code` for SAIL code, `get_object_detail` for metadata
...
- To see code, use `solutions-intelligence.get_object_code` on specific objects

### `solutions-intelligence.get_dependencies`
...
Same as `solutions-intelligence.get_dependencies` but by UUID instead of name.

### `solutions-intelligence.list_orphans`
...
### `solutions-intelligence.get_object_code`
**Args**: `app_name`, `object_name`
...
### `solutions-intelligence.get_dependency_path`
...
### `solutions-intelligence.get_transitive_dependencies`
...
### `solutions-intelligence.get_hub_objects`
...
### `solutions-intelligence.list_releases`
**Args**: `app_name`
All releases with metadata and change summaries.

### `solutions-intelligence.get_changelog`
...
### `solutions-intelligence.refresh_knowledge_base`
...
### Explore an object
```
1. solutions-intelligence.search_objects(app, "vendorValidation") → find it with description
2. solutions-intelligence.get_object_code(app, "AS_GSS_BL_validateVendors") → see SAIL code
3. solutions-intelligence.get_dependencies(app, "AS_GSS_BL_validateVendors") → see calls/called_by
```

### Understand a bundle
```
1. solutions-intelligence.get_bundle(app, "AS_GSS_Complete_LPTA_Evaluation") → see structure + member_summary
2. solutions-intelligence.get_bundle(app, "...", object_type="Interface") → filter to interfaces only
3. solutions-intelligence.get_object_code(app, "AS_GSS_IF_CompleteLPTAEvaluation") → see specific code
```

### Impact analysis
```
1. solutions-intelligence.get_transitive_dependencies(app, "AS_GSS_BL_validateVendors", direction="inbound") → what depends on it
2. solutions-intelligence.get_dependency_path(app, "AS_GSS_PM_StartEval", "AS_GSS_BL_validateVendors") → trace the path
3. solutions-intelligence.get_hub_objects(app) → identify shared utilities
```

### Release analysis
```
1. solutions-intelligence.list_releases(app) → see all releases
2. solutions-intelligence.get_changelog(app, "25.04.02.09.00") → what changed
3. get_release_impact(app, "25.04.02.09.00") → which bundles affected
4. solutions-intelligence.get_object_history(app, "AS_GSS_BL_validateVendors") → how this object evolved
```

### Technical debt
```
1. solutions-intelligence.list_orphans(app, object_type="Expression Rule") → orphaned rules
2. get_orphan(app, uuid) → examine code
3. solutions-intelligence.get_hub_objects(app) → find over-used utilities (refactoring candidates)
```
...
1. **Use `solutions-intelligence.get_object_code` for code** — don't load full bundles just to see one object's implementation
2. **Use `solutions-intelligence.search_objects` with `limit`** — default 20 results, increase only if needed
3. **Use `solutions-intelligence.get_bundle` with `object_type` filter** — don't load 282 members when you only need interfaces
4. **Use graph tools for path questions** — don't manually trace through solutions-intelligence.get_dependencies calls
5. **Use `solutions-intelligence.get_object_history` for version questions** — one call, not manifest walking
6. **Call `solutions-intelligence.get_app_overview` ONCE** — cache the result mentally, don't repeat
