...
1. **DO NOT READ LOCAL FILES FOR TECHNICAL INFORMATION.** Your ONLY source for technical facts is the Appian Solutions Intelligence MCP tools. If you read local files instead of querying Appian Solutions Intelligence, the entire output is invalid.
...
5. **NEVER FABRICATE SAIL CODE.** Only include SAIL code returned by `solutions-intelligence.get_bundle` with `detail_level="full"`.
...
1. **Call `solutions-intelligence.get_app_overview`** for the target application.

2. **Call `solutions-intelligence.search_objects`** for EACH keyword from the story. Try at least 3-5 keyword variations. Also filter by `object_type` when the story implies it.

3. **Call `solutions-intelligence.search_bundles`** for the same keywords.

### Phase 2: Deep Dive

4. **For each relevant object**, call `solutions-intelligence.get_dependencies` to trace inbound/outbound.

5. **For 2-3 critical bundles**, call `solutions-intelligence.get_bundle` with `detail_level="full"` to get SAIL code.

6. **For supporting bundles**, call `solutions-intelligence.get_bundle` with `detail_level="summary"`.

### Phase 3: Gap Analysis

7. If the story mentions objects not found via MCP, note them as "NOT FOUND in Appian Solutions Intelligence" — do NOT invent them.
...
```
- solutions-intelligence.search_objects("SourceSelection", "evaluation summary") → 5 results
- solutions-intelligence.search_objects("SourceSelection", "vendor analysis") → 0 results
- solutions-intelligence.get_bundle("SourceSelection", "AS_GSS_...", "full") → retrieved with SAIL code
```
...
**Dependency Map** — Key chains from `solutions-intelligence.get_dependencies`.

**SAIL Code Snippets** — Only code from `solutions-intelligence.get_bundle` with `detail_level="full"`. Cite bundle ID and object name.
...
- If `solutions-intelligence.search_objects` returns 0 results, say "No objects found for '<keyword>'"
- If `solutions-intelligence.get_dependencies` returns an error, say "Dependencies not available for '<object>'"
