...
- User selects option 9 from Solutions Intelligence menu
...
- ⚠️ Call `solutions-intelligence.list_applications` for pipeline configuration.
...
- ❌ Deploy anything
- ❌ Access test/automation environments (Solutions Intelligence only connects to dev)
...
1. Call `solutions-intelligence.list_applications`
...
4. For each traced failure, produce:
   ...
   - Target environment from YML (for manual verification — Solutions Intelligence cannot access test environments)

**⚠️ ENVIRONMENT NOTE:** Solutions Intelligence only connects to the dev environment. The tests run against different environments (test, automation). The agent traces changes in dev (where code originates) but cannot verify behavior in the test environment. Include the `SITE_URL` in the report so the developer knows where to check manually.
...
Pipeline config from `solutions-intelligence.list_applications` per application:
