...
2. DISCOVER UTILITIES:
   a. search_objects_semantic(searchTerm="_UT_", typeIds=["Expression Rule"], appPrefix="<APP>")
   b. search_objects_semantic(searchTerm="_CO_UT_", typeIds=["Expression Rule"])
...
5. DISCOVER UTILITIES (MANDATORY — do NOT skip):
   a. search_objects_semantic(searchTerm="_UT_", typeIds=["Expression Rule"], appPrefix="<APP>")
   b. search_objects_semantic(searchTerm="_CO_UT_", typeIds=["Expression Rule"])
   c. For each discovered utility: read its signature (inputs, types) via get_appian_object
...
- If user provides an object name: use `jarvis_search_objects` to find it, then ask the user which package it belongs to (the Deployment API only exports packages, not individual objects)
...
The `analyze_appian_code` tool may report warnings that are **not actionable**. Do NOT present these to the user as "remaining issues":
...
**Rule:** After refactoring, do NOT run `analyze_appian_code` again just to report "remaining warnings." The refactoring is complete when the code changes are deployed. Only run analysis if you need to discover issues *before* refactoring.
...
**How it works:**
1. **Discovery** — The AI agent discovers available utility rules by querying the Knowledge Base (`jarvis_search_objects` with `_UT_` and `_CO_` patterns) or falling back to the live Appian API (`list_application_objects` + `get_appian_object`) if no KB exists. Both common utilities (`AS_CO_UT_*`) and application-specific utilities (`AS_<APP>_UT_*`) are searched across the full application scope.
