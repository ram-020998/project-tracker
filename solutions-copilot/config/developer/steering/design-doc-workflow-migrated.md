...
**This workflow uses the Solutions Intelligence Knowledge Base (KB) as the PRIMARY research source when available.**
**The KB replaces the Atlas power for codebase research. Atlas is no longer required.**

When the target application has a `app_name` in the `solutions-intelligence.list_applications` response:
- Track A uses KB tools (solutions-intelligence.get_app_overview, solutions-intelligence.search_objects, solutions-intelligence.get_bundle, etc.)
- Track B uses live API for recently created objects and database verification
- Step 3.5 uses solutions-intelligence.get_transitive_dependencies instead of sequential solutions-intelligence.get_dependencies calls

When the application does NOT have a `app_name`:
...
- "Design [JIRA ticket]"
- User selects option 1 from Solutions Intelligence menu
...
- Call `solutions-intelligence.list_applications` to determine appUuid for package creation
...
**Step 3 uses KB as the primary source (when app_name exists) with live API for gaps:**
...
**Track B — Solutions Intelligence MCP (live environment):**
...
**If app_name does NOT exist:** Track A is skipped entirely. Track B (live API) is the only source.
...
Step 2.5: KB Freshness Check   [ ] ❌ NOT STARTED (skip if no app_name)
Step 3: Research                       [ ] ❌ NOT STARTED ⚠️ REQUIRED: KB (if available) + Solutions Intelligence MCP
  Track A: KB Research                      [ ] ❌ (skip if no app_name)
  Track B: Solutions Intelligence MCP Research              [ ] ❌
Step 3.25: Consolidate & Reconcile     [ ] ❌ NOT STARTED ⚠️ MERGE + VALIDATE
Step 3.75: Application Info     [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.getApplication
...
```
Step 1: Get JIRA Ticket        [✅] ✅ COMPLETED - jira.get_jira_issue called
Step 2: Determine Application   [ ] ❌ NOT STARTED
...
...
Process:
  ...
  h. **DUPLICATE DETECTION (solutions-intelligence-i18n integration):**
     ...
     - **BND apps** (GSS, VM, GCW, RM, AM): Query for existing keys with matching labels:
       ...
       LIMIT 30
       ```
     - **Translation Set apps** (GSM): Use `solutions-intelligence_get_translation(app_name, query="{label}")` for each unique label
...
- ✅ Called jira.get_jira_issue with ticket key
...
⚠️ **CONDITION:** Only run if `app_name` exists (non-null, non-empty) for the matched application from `solutions-intelligence.list_applications`. If no app_name, mark as ⏭️ SKIPPED and proceed to Step 3.

```
Check the `staleCount` field from the `solutions-intelligence.list_applications` response for the matched application.
...
### Step 3: Parallel Research — KB + Solutions Intelligence MCP (MANDATORY - DO NOT SKIP)
...
- ✅ Called `solutions-intelligence.list_applications` to get application list
- ✅ Matched JIRA project/epic against config
- ✅ Determined app_name, appUuid, and app_name (if available)
...
==============================================================
⚠️ CONDITION: Only run if app_name exists in the `solutions-intelligence.list_applications` response for this application.
⚠️ If app_name does NOT exist, skip Track A entirely and rely on Track B only.
...
2. Object discovery:
   Tool: solutions-intelligence.search_objects
   Input: app_name={app_name}, query={keyword}
...
3. Feature context (for the most relevant cluster):
   Tool: solutions-intelligence.get_bundle
   Input: app_name={app_name}, clusterName={from app-tree or search results}
   Purpose: See all objects in the feature — understand how they work together

4. Existing patterns (MANDATORY — design reference):
   Tool: solutions-intelligence.search_bundles
   Input: app_name={app_name}
...
5. Data model context:
   Tool: solutions-intelligence.get_app_schema
   Input: app_name={app_name}
...
7. Source code for key objects being MODIFIED (top 2-3):
   Tool: solutions-intelligence.get_object_code
   Input: app_name={app_name}, objectName={name}
   Purpose: Read current SAIL code with resolved UUIDs before designing changes

8. Shared object awareness:
   Tool: solutions-intelligence.get_hub_objects
   Input: app_name={app_name}
...
KB limitation:
  - Snapshot — objects created after last KB generation won't appear
  - No folder UUIDs for deployment (use lcp-api.getApplication in Step 3.75)
...
TRACK B — Solutions Intelligence MCP Research (live environment, current state)
================================================================
⚠️ REQUIRED TOOLS: lcp-api.listApplications, lcp-api.searchObjects
...
3. Database verification (MANDATORY when implementation plan involves database):
   ...
   NOTE: If KB Track A provided data model via solutions-intelligence.get_app_schema, SQL verification
...
4. Database Script Detection (solutions-intelligence-smt integration — ANY database operation):

   ⚠️ **CRITICAL TRIGGER:** If the JIRA ticket mentions ANY of the following, you MUST activate the
   solutions-intelligence-smt-power and research the SMT context BEFORE writing the design doc:
   ...
   **When triggered:**
   1. **MANDATORY: Activate solutions-intelligence-smt-power FIRST** — do NOT query SMT tables directly.
...
1. If KB Track A was available:
   ...
   🆕 Live API found objects NOT in KB → VALIDATE RELEVANCE
      ...
      - MUST verify relevance before including:
        a. Read source code: call lcp-api.getInterface with the UUID
...
2. If KB Track A was NOT available (no app_name):
   Live API is the only source. Use semantic search + lcp-api.listApplications results directly.
   Read source code for top 3-5 objects being modified via lcp-api.getInterface.
...
```
⚠️ REQUIRED TOOL: lcp-api.getApplication
⚠️ THIS STEP CANNOT BE SKIPPED

Tool: lcp-api.getApplication
...
⚠️ VALIDATION: Before proceeding to Step 4, confirm you have:
  - Called lcp-api.getApplication with correct appUuid
...
```
Tool: lcp-api.createApplication
Input:
  - appUuid: From step 2
...
- ✅ Called lcp-api.createApplication
...
Input:
  ...
  - folder_id: {from `solutions-intelligence.list_applications` → matched application → designDocFolderId}
  - source_format: "html"
...
<!-- Determine i18n system from solutions-intelligence.list_applications: if translationSets[] is populated → Translation Sets format, otherwise → BND bundle format -->
<table style="width:100%; border:1px solid #999;">
...
## Configuration
- Design doc folder ID: From `solutions-intelligence.list_applications` → applications[].designDocFolderId (per application)
- User email: User's Google email (from MCP environment or ask user)
- JIRA base URL: https://appian-eng.atlassian.net/browse/
- App config: `solutions-intelligence.list_applications` API (replaces config/app_config.json)
