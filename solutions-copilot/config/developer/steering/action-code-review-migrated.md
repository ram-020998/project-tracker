...
1. **Find the objects**: `solutions-intelligence.search_objects` or `solutions-intelligence.search_bundles` to locate them.
2. **Load code**: `solutions-intelligence.get_bundle(app, bundle_id, "full")` to get SAIL code.
3. **Check enrichment**: `get_object_enrichment(app, uuid)` for complexity and depth metrics.
4. **Trace dependencies**: `solutions-intelligence.get_dependencies` to understand coupling.
