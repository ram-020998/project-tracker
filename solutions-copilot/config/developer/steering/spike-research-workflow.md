...
- User selects option 7 from JARVIS menu
...
**Skip this step if:** `searchDrive` is `false` in `globalSettings` from `get_jarvis_config`.
...
- `searchChat` is set to `true` in `globalSettings` from `get_jarvis_config`
...
**Step 3 — Fill gaps:** If semantic results are thin for a specific area, supplement with `list_application_objects` using keyword + type filter.
...
Input:
  ...
  - folder_id: {from `get_jarvis_config` → matched app → spikeDocFolderId or designDocFolderId}
  - source_format: "html"
  - user_google_email: {user's Google email}
```
...
<p class="meta">Generated by JARVIS | Application: {APP_NAME}</p>
