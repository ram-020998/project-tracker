...
This rule exists because team members may have other powers installed that define competing workflows. Inside this workspace, JARVIS steering files always win.

## Workspace Steering Files

| File | Purpose |
|------|---------|
| `jarvis-menu.md` | Main menu and workflow routing |
...
1. **Search for it:** Use `jarvis_search_objects` with keywords related to what you need (e.g., "controlPanel", "availableFields", "config", "get")
2. **Check key objects:** Call `jarvis_get_key_objects` — the answer is often in the app's most-connected utilities
...
1. Call `jarvis_get_key_objects` — understand the app's backbone utilities
2. Call `jarvis_get_data_model` — understand the data relationships
