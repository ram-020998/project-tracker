...
- User selects option 4 from Solutions Intelligence menu
...
Step 5: Preview Object              [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.createConstant
Step 6: Create Object               [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.createConstant
...
Step 2: Get Application Info        [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.getApplication
Step 3: Collect Object Parameters   [ ] ❌ NOT STARTED
Step 4: Preview Object              [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.createConstant
Step 5: Create Object               [ ] ❌ NOT STARTED ⚠️ REQUIRED TOOL: lcp-api.createConstant
...
1. Call `mcp_google_workspace_search_drive_files` with:
   ...
   - Filter results to folder: {from `solutions-intelligence.list_applications` → matched app → designDocFolderId}
...
1. Get application info to determine naming convention:
   ...
   - Call `lcp-api.getApplication` with appUuid
...
**Pre-creation database check (solutions-intelligence-smt integration — for future object types):**
...
1. Call `lcp-api.createConstant` with:
...
4. Call `lcp-api.createConstant` with same parameters
...
1. Call `solutions-intelligence.list_applications` to get list of applications
...
4. Call `lcp-api.getApplication` with appUuid
...
1. Call `lcp-api.createConstant` with collected parameters

2. Show preview and wait for confirmation (y/n)

3. If yes → Call `lcp-api.createConstant`
...
- **Design Doc Folder**: From `solutions-intelligence.list_applications` → applications[].appConfig.designDocFolderId (per application)
