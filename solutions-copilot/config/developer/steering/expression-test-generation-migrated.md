...
- If user provides an object name: use `solutions-intelligence.search_objects` to find it, then identify the package
...
1. **Run existing tests first.** Before making any decisions, evaluate every existing test case using `lcp-api.evaluateExpression` to confirm it actually passes. A test without an assertion, or with a wrong assertion, is a bad test — it provides false confidence.
...
extract_sail_from_export → for each expression rule:
  ...
  ├── Has existing tests?
  │   ├── YES → Run them with lcp-api.evaluateExpression to check correctness
...
When a rule has complex inputs (Maps, CDTs, record types, nested structures), **do not guess the field names or structure**. Instead, use the Solutions Intelligence KB and live tools to discover how the rule is actually called:

### Step 1: Find the rule in the KB

```
solutions-intelligence.get_object_detail(app_name=<app_name>, objectName="<rule_name>")
```
...
```
solutions-intelligence.get_object_code(app_name=<app_name>, objectName="<caller_name>")
```
...
1. `solutions-intelligence.get_object_detail` → finds it's called by "Create Document from Collection" process model
2. `solutions-intelligence.get_object_code("Create Document from Collection")` → shows the process has a "Download Docs From S3" node that produces `docDownloadResponse`, and a wizard interface that produces `selectedDocumentMaps`
3. `solutions-intelligence.get_object_code("AS_PD_FM_createDocFromCollectionWizard_Step3")` → reveals the exact map structure: `{fileName, fileType, appianDocId, fileSize, s3Path, isPspRecordConfig, recordTypeConfig, relatedRecord}`
...
2. Use `solutions-intelligence.get_dependencies` with `dependency_type: "DEPENDENTS"` on the record type to find rules that query it
...
2. Search for dependents: solutions-intelligence.get_transitive_dependencies(objectName="<RecordTypeName>")
   or solutions-intelligence.get_dependencies(object_uuid="<RecordTypeUUID>", dependency_type="DEPENDENTS")
...
1. Look at the record type in the KB or via `solutions-intelligence.get_dependencies` — the relationship target tells you which record type it points to
...
| Rule queries database/record types | Mark test as "requires live data"; suggest using `lcp-api.evaluateExpression` to validate |
...
**Strategy:**
1. **Find an existing query rule first.** Use `solutions-intelligence.get_transitive_dependencies` or `solutions-intelligence.get_dependencies` (DEPENDENTS) on the Record Type to find existing query expression rules (look for rules with prefixes like `QR_`, `QE_`, `get`, `load`).
...
```
lcp-api.evaluateExpression(expression="rule!RS_expressionRule_integer_easy(score: 95) = \"A\"")
→ ✅ Result:  (no error = PASS)
```

For a failing case where expected was wrong:
```
lcp-api.evaluateExpression(expression="rule!RS_expressionRule_integer_easy(score: 89) = \"A\"")
