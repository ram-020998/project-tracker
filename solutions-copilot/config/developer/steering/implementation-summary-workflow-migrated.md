...
- User selects option 8 from Solutions Intelligence menu
...
**Resolution strategy:** When the user provides an object name (Mode B), resolve the UUID AND type via `lcp-api.searchObjects` or `lcp-api.searchObjects`. The search results include the object type — map it to typeId using the table above. Do NOT guess from naming convention alone.
...
```
Tool: get_package_contents_from_url
Input: package_url
```
...
Input: searchTerm={object_name}, appPrefix={from solutions-intelligence.list_applications}
```
...
1. Use the `designDocFolderId` from the `solutions-intelligence.list_applications` response for the matched application
...
<p><strong>Generated:</strong> {date} by Solutions Intelligence</p>
...
```
Tool: fsWrite
Path: solutions-intelligence-appian/temp_impl_summary.html
Content: {HTML above}
```
...
Input:
  ...
  - file_path: "file:///Users/somasundaram.d/Soma-Repos/Appian MCP/solutions-intelligence-appian/temp_impl_summary.html"
  - source_format: "html"
  - folder_id: {designDocFolderId from solutions-intelligence.list_applications for the matched application}
```

4. Present the Google Doc link to the user
5. Clean up temp file: `deleteFile solutions-intelligence-appian/temp_impl_summary.html`
