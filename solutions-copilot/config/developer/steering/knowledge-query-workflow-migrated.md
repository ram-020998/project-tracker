...
- Says "Ask Solutions Intelligence about..."
...
- Selects option 5 from the Solutions Intelligence menu
...
| **Live** | User wants to test an expression, check a constant value, or debug something in real-time | "What is the value of cons!X?", "Evaluate this expression", "Debug this rule call" | lcp-api.evaluateExpression |
...
- Extract: `appUuid`, `appPrefix`, `app_name` from the matched application

**KB availability:** If `app_name` exists for the matched application, use KB-powered strategies. If not, use live API strategies.

**KB freshness check (MANDATORY — DO NOT SKIP):**

⚠️ **After calling `solutions-intelligence.list_applications`, BEFORE making any other tool call, check the `staleCount` field for the matched application.**

- If `app_name` is null/empty → KB is not available for this app. Skip KB strategies.
- If `staleCount` is 0 → KB is fresh. Proceed silently.
- If `staleCount` > 0 → **STOP. Your NEXT message to the user MUST be the freshness warning below. Do NOT call any KB tool (solutions-intelligence.get_app_overview, solutions-intelligence.search_objects, etc.) until the user responds.**
...
**When to use:** Domain is Codebase AND the application has a `app_name` in the `solutions-intelligence.list_applications` response.
...
1. `solutions-intelligence.get_object_detail(app_name, objectName)` — returns content + deps + metadata + impact + clusters
...
1. `solutions-intelligence.get_app_overview(app_name)` — find the relevant cluster (if not already known from context)
2. `solutions-intelligence.get_bundle(app_name, clusterName)` — get all objects in the feature with descriptions, tags, inputs
3. `solutions-intelligence.get_object_code(app_name, objectName)` — read the top 2-3 key objects (Process Models, main interfaces)
...
1. `solutions-intelligence.get_bundle(app_name, clusterName)` — get the feature cluster
2. `solutions-intelligence.get_dependencies(app_name, objectName)` — trace calls/calledBy for key objects
...
1. `solutions-intelligence.get_app_schema(app_name)` — full data model with fields, relationships, data stores
2. Optionally: `solutions-intelligence.get_dependencies(app_name, objectName)` — for specific object relationships
...
1. `solutions-intelligence.get_transitive_dependencies(app_name, objectName)` — direct callers, transitive callers, affected clusters, type breakdown
...
1. `solutions-intelligence.get_object_detail(app_name, objectNameA)` — full context for object A
2. `solutions-intelligence.get_object_detail(app_name, objectNameB)` — full context for object B
...
**For type counts:**
1. `solutions-intelligence.get_app_overview(app_name)` — objectCounts section has all type counts

**For all objects of a type:**
1. `solutions-intelligence.search_objects(app_name, objectType)` — full list with metadata

**For unused detection:**
1. `solutions-intelligence.list_orphans(app_name)` — all unreachable objects by type
...
1. `solutions-intelligence.search_objects(app_name)` — application-wide security issues
2. OR `solutions-intelligence.get_object_code(app_name, objectName)` — security config for a specific object
...
**When to use:** Domain is Codebase AND the application does NOT have a `app_name`, OR the object was not found in KB.

### Live API Search Tool Hierarchy

| Priority | Tool | Best For | Notes |
|----------|------|----------|-------|
| 1st | `lcp-api.searchObjects` | Primary discovery — natural language, ranked by relevance | Does NOT return Record Types. Use `appPrefix` for app-specific queries. |
| 2nd | `lcp-api.listApplications` | Exhaustive search, type-filtered, Record Types | Required for Record Types. Use for pagination. |
| 3rd | `lcp-api.searchObjects` | Precise prefix match when you know the naming pattern | Use when you know the exact object name prefix. |

### Strategy B1: Technical (Specific Object) — Live API

1. `lcp-api.searchObjects(objectName)` or `lcp-api.getInterface(uuid)` — find/fetch the object
2. `explain_appian_code(uuid)` — detailed logic explanation
3. Optionally: `solutions-intelligence.get_dependencies(uuid, DEPENDENTS)` — what uses it
...
1. `lcp-api.searchObjects(searchTerm, appPrefix, batchSize=15)` — ranked discovery
2. `lcp-api.listApplications(appUuid, searchTerm=keyword, objectType="Record Type")` — find data model
3. `lcp-api.getInterface(uuid)` for top 3-5 most relevant objects
4. Optionally: `solutions-intelligence.get_dependencies(uuid, DEPENDENTS)` for the primary object

**Budget:** 5-10 calls

### Strategy B3: Structural (Flow/Chain) — Live API

1. `lcp-api.searchObjects(searchTerm, appPrefix, batchSize=20)` — broad discovery
2. `lcp-api.listApplications(appUuid, searchTerm=keyword, objectType="Record Type")` — data model
3. `solutions-intelligence.get_dependencies(uuid, DEPENDENTS)` + `solutions-intelligence.get_dependencies(uuid, PRECEDENTS)` for primary objects

**Budget:** 8-15 calls

### Strategy B4: Relationship / Data Model — Live API

1. `lcp-api.listApplications(appUuid, searchTerm=keyword, objectType="Record Type")` — find Record Types
2. `lcp-api.getInterface(uuid)` for each relevant Record Type (limit 5-8)
...
2. **Level 1:** `solutions-intelligence.get_dependencies(uuid, DEPENDENTS)` — direct dependents
3. **Level 2:** For each Level 1 dependent that is shared/core: `solutions-intelligence.get_dependencies(dependent_uuid, DEPENDENTS)`
...
1. Fetch both objects: `lcp-api.getInterface(uuid)` for each
2. `solutions-intelligence.get_dependencies` for each
3. Present side-by-side comparison table
...
**For type counts:**
1. `lcp-api.listApplications(appUuid, objectType=type)` for each main type — extract `totalCount` from first page

**For unused detection:**
1. `solutions-intelligence.get_dependencies(uuid, DEPENDENTS)` — zero dependents = potentially unused

**Budget:** 14-16 calls for full inventory, 1-2 per unused check

### Strategy B8: Security — Live API

1. Find the object: `lcp-api.searchObjects(name)` or `lcp-api.listApplications`
2. `lcp-api.getInterface(uuid)` — fetch full config
...
**Prerequisites:** `searchDrive` must be `true` in the `globalSettings` from `solutions-intelligence.list_applications`. If not enabled, tell the user and suggest they enable it in the Solutions Intelligence Settings page.
...
1. `lcp-api.evaluateExpression(expression)` — evaluate directly
...
**Known limitations of lcp-api.evaluateExpression:**
...
**CRITICAL:** `lcp-api.listApplications` returns max 50 results per page.
...
- The `lcp-api.evaluateExpression` tool is powerful but runs in the live environment — use it for read-only operations (queries, constant lookups, expression testing). Do NOT use it for write operations.
