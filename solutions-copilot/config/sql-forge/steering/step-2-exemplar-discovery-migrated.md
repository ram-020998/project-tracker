...
- ⚠️ **ALL APPLICATION DATA COMES FROM ATLAS KB AND LIVE ENVIRONMENT ONLY** — Do NOT read local files, folders, or documents for application context. Only use Solutions Intelligence MCP tools and Data Generator MCP tools.
...
```
solutions-intelligence.get_record_type_map(app_name)
```
...
⚠️ **If you don't know the FK path, use `solutions-intelligence.get_schema_relationships(app_name)` to find it.**
