...
- ⚠️ **CAPTURE EVERY RESPONSE** — Record the ID returned from every create_record call.
...
2. **For each record in the file's `records` array:**
   ...
   - Call `create_record` with the record_type_uuid and fields
...
3. **Diagnose** — common causes:
   ...
   - Invalid user → not in list_users()
...
**Session ID:** {from get_session()}
**Records in session:** {count}
**Rollback available:** Yes — use `rollback_session(confirm: true)` to undo all
...
- **To rollback:** `rollback_session(confirm: true)`
```
