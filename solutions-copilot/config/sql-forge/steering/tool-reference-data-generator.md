...
- `relationshipName` must match a relationship from `get_record_properties` (e.g., "vendor", "criteria", "team")
...
```json
{
  ...
  "fields": {
    ...
    "completedBy": "<username from list_users>"
  }
}
```
...
1. Call `get_record_properties` ONCE per record type — cache the result
2. Call `list_users` ONCE — usernames don't change during a session
...
5. Check `get_session` before rollback — know what you're deleting
6. After each `create_record`, capture the returned ID immediately — you need it for child records
