...
3. ✅ **PLAN YOUR QUERIES** — You will need multiple Atlas KB queries. Plan them ALL before starting.

---

## CRITICAL RULES — MANDATORY COMPLIANCE

- ⚠️ **ALL APPLICATION DATA COMES FROM ATLAS KB ONLY** — Do NOT read local files, folders, or documents for application context. Only use Atlas MCP tools (search_bundles, get_bundle, get_object_code) and Data Generator MCP tools.
...
**Query Atlas KB for EVERY action/workflow in the lifecycle:**
...
```
get_object_code(app_name, subprocess_name)
```
...
```
get_object_code(app_name, rule_name)
```
