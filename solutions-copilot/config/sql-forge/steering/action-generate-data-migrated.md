...
3. **DO NOT call Solutions Intelligence MCP or Data Generator MCP tools during Steps 1-5.** Those are for sub-agents.
4. **YOU MAY call MCP tools in Step 6** (data-generator.create_record, data-generator.query_records, data-generator.get_session, data-generator.rollback_session for records mode; solutions-intelligence.get_app_schema, solutions-intelligence.get_field_map for SQL mode).
  ...
  stages: [
    {
      ...
      prompt_template: "You are executing Step 1 of the Solutions Intelligence SQL Forge data generation workflow.

APPLICATION: {app_name}
REQUEST: {task}
FOLDER: {folder_path}

INSTRUCTIONS: Read the steering file 'step-1-workflow-analysis' from the solutions-intelligence-sql-forge power. Follow it EXACTLY.
...
CRITICAL: Trace the ENTIRE lifecycle (Create → Start → intermediate steps → target state). Do NOT only read the final action."
    },
    {
      ...
      role: "kiro_default",
      model: "claude-opus-4-20250918",
      prompt_template: "You are executing Step 2 of the Solutions Intelligence SQL Forge data generation workflow.

APPLICATION: {app_name}
REQUEST: {task}
FOLDER: {folder_path}

INSTRUCTIONS: Read the steering file 'step-2-exemplar-discovery' from the solutions-intelligence-sql-forge power. Follow it EXACTLY.
...
CRITICAL: Query EVERY table from the analysis — not just direct relationships.",
    ...
    },
    {
      ...
      role: "kiro_default",
      model: "claude-opus-4-20250918",
      prompt_template: "You are executing Step 3 of the Solutions Intelligence SQL Forge data generation workflow.

APPLICATION: {app_name}
REQUEST: {task}
FOLDER: {folder_path}

INSTRUCTIONS: Read the steering file 'step-3-data-architecture' from the solutions-intelligence-sql-forge power. Follow it EXACTLY.
...
OUTPUT: Update {folder_path}/data-architecture.md with complete architecture. Change status from PENDING to COMPLETE.",
    ...
    },
    {
      ...
      role: "kiro_default",
      model: "claude-opus-4-20250918",
      prompt_template: "You are executing Step 4 of the Solutions Intelligence SQL Forge data generation workflow.

APPLICATION: {app_name}
REQUEST: {task}
FOLDER: {folder_path}

INSTRUCTIONS: Read the steering file 'step-4-data-payloads' from the solutions-intelligence-sql-forge power. Follow it EXACTLY.
...
OUTPUT: Write payload files to {folder_path}/payloads/ with complete payloads. Change status from PENDING to COMPLETE.",
    ...
    },
    {
      ...
      role: "kiro_default",
      model: "claude-opus-4-20250918",
      prompt_template: "You are executing Step 5 of the Solutions Intelligence SQL Forge data generation workflow.

APPLICATION: {app_name}
REQUEST: {task}
FOLDER: {folder_path}

INSTRUCTIONS: Read the steering file 'step-5-validation' from the solutions-intelligence-sql-forge power. Follow it EXACTLY.
...
- **Records mode:** Read the `step-6-execute` steering file for guidance
  ...
  - Execute `data-generator.create_record` calls, resolve FK references
  - Show progress, verify, write `execution-log.md`

- **SQL mode:** Read the `step-6-generate-sql` steering file for guidance
  - Read each payload file from `payloads/` in sequence
  - Map camelCase → UPPER_SNAKE column names via `solutions-intelligence.get_field_map`
...
4. Use `data-generator.get_session()` to see what's already been created (records mode only)
