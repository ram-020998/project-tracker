...
- ⚠️ **USE EXACT camelCase FIELD NAMES** — From `get_field_map`. Not UPPER_SNAKE. Not guessed names.
- ⚠️ **REFERENCE DATA IDs FROM STEP 3** — Use the actual IDs you queried. Not hardcoded.
- ⚠️ **USERNAMES FROM list_users()** — Never invent users.
...
3. For each field, determine the value from:
   ...
   - **Users** — pick from `list_users()` result
...
**For standalone records (separate `create_record` calls):**
...
```json
{
  ...
  "field_reasoning": {
    ...
    "contractingOfficer": "admin.user (from list_users(), first available)",
...
```
get_record_properties(record_type_uuid)
```
...
- [ ] All usernames come from list_users() result
...
| Coverage below 80% | Incomplete records = broken app state | Check with get_record_properties |
| No reasoning for field values | Can't debug when data is wrong | Document EVERY value's source |
| Inventing a username | API error or orphaned user reference | Only use list_users() values |
