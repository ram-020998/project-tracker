...
```
Data Generator MCP: data-generator.get_session()
```
...
```
Data Generator MCP: data-generator.rollback_session(confirm: true)
```
...
If the user only wants to undo specific records (not the whole session), use `data-generator.delete_record` individually:

```
Data Generator MCP: data-generator.delete_record(
...
- Session tracks ALL `data-generator.create_record` calls (not updates or deletes)
