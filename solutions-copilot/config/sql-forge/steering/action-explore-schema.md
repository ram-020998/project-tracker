...
```
Atlas MCP: list_applications()
```

### Step 2: Get schema summary
```
Atlas MCP: get_schema_summary(app_name)
```
...
| Question | Tool |
|----------|------|
| "What tables exist?" | `get_app_schema(app_name)` |
| "What business tables exist?" | `get_app_schema(app_name, classification="business")` |
| "What fields does X have?" | `get_app_schema(app_name, table_name="X")` |
| "How are tables related?" | `get_schema_relationships(app_name)` |
| "What references table X?" | `get_schema_relationships(app_name, table_name="X")` |
...
| "What order should I create records?" | `get_insertion_order(app_name)` |
...
```
Data Generator MCP: get_record_properties(record_type_uuid)
```
