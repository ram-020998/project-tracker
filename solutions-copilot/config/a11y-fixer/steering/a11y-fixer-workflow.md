...
### Step 2.1: Get Jarvis config

Call `get_jarvis_config` to get the app registry. Identify which app the interface belongs to based on the ticket's component or prefix (AS_GSS_, AS_VM_, AS_GCW_, etc.).

### Step 2.2: Search by name

Use `jarvis_search_objects` with the interface name from the ticket. If the ticket mentions a specific object name (e.g., "AS_GSS_GRD_EvaluationVendors"), search for it directly.

### Step 2.3: If not found by name, use semantic search

Use `search_objects_semantic` with descriptive terms from the ticket (e.g., "evaluation vendor grid", "select awardees form").
...
- Use `jarvis_get_object_content` to inspect the wrapper's calls and identify the child
...
[Description from Jarvis]. This matches the "[component name]" mentioned in the ticket.
...
[Description from Jarvis]. This looks like it could be the "[component name]" from the ticket, but I'm not 100% certain.
...
## Phase 3: Plan Navigation (Jarvis → Playwright)

Before opening the browser, use Jarvis to build a navigation plan. The agent needs to know exactly where to go.

### Step 3.1: Get site URLs

Call `get_jarvis_config` → find the app's `siteUrlList`. Pick the primary site URL (usually the first one, not settings/migration sites).

### Step 3.2: Get entry points

Call `jarvis_get_entry_points_for_object` with the interface name → returns which clusters/features contain it. This tells you which site page or record view leads to the interface.
...
Call `jarvis_get_object_xml` with the confirmed interface name.
...
Take the full XML string from `jarvis_get_object_xml`, make your changes directly in the string (find and replace the specific lines), and pass the entire modified XML to `deploy_modified_object` in one tool call. MCP handles 100KB+ strings. The XML is 5-20KB.
...
- `appUuid`: from `get_jarvis_config`
...
1. **Always re-fetch the XML** via `jarvis_get_object_xml` before retrying
...
Call `jarvis_get_object_xml` again. Confirm the fix parameter is present in the live XML.
...
| Interface not found in Jarvis KB | Ask user for the exact interface name |
