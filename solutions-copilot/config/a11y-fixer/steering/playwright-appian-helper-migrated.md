...
- The Solutions MCP may connect to a different environment than the one used for Playwright verification — don't assume they're the same
...
### Using Solutions KB to Find Navigation Paths
Before navigating in Playwright, use Solutions tools to understand how to reach an interface:

1. `solutions_search_objects` — find the interface by name
2. `solutions_get_entry_points_for_object` — find which features/clusters contain it
3. `solutions_get_cluster` — get the full feature tree from an entry point
4. `solutions_get_dependency_chain` — see calledBy relationships to trace the call hierarchy
