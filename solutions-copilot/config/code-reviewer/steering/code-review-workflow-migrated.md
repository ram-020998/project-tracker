...
- User selects option 2 or 3 from Solutions Intelligence menu
...
- **Application Config:** `solutions-intelligence.list_applications` API - Returns application UUIDs, naming conventions, and drive folder IDs
...
The `solutions-intelligence.get_object_history` tool requires a `typeId` integer. Use this mapping:
...
3. **Extract reference date from JIRA changelog** (for diff context in Step 4):
   ...
   - Store as `reference_date` — used in Step 4a for `solutions-intelligence.get_object_history`
...
4. **Verify baseline version using `get_version_context`** (only if `reference_date` is available):
   ...
   - Call `get_version_context` with `uuid`, `typeId`, `dateTime: reference_date`
...
1. Resolve UUID via `search_objects_by_name`
...
1. Call `get_package_contents_from_url` with the package URL
...
2. Look up appUuid from `solutions-intelligence.list_applications` response:
...
1. Call `get_application_info` with the appUuid
...
**Date:** {today}
**Reviewed By:** Solutions Intelligence
...
**Step 3d: KB Pre-Analysis (OPTIONAL — when app_name exists)**

⚠️ **CONDITION:** Only run if the matched application has a `app_name` in the `solutions-intelligence.list_applications` response. If not available, skip this sub-step.

**KB freshness check (before any KB calls):**
1. Check the `staleCount` field from the `solutions-intelligence.list_applications` response for the matched application
...
1. Feature context — understand what feature these objects belong to:
   ...
   Input: app_name={app_name}, query={primary keyword from object names}
   
   From the results, identify which cluster(s) the package objects belong to.
   Then optionally:
   Tool: solutions-intelligence_get_cluster
   Input: app_name={app_name}, clusterName={identified cluster}
...
2. Impact pre-computation — blast radius for modified objects:
   Tool: solutions-intelligence.get_transitive_dependencies
   Input: app_name={app_name}, objectName={name of key modified object}
...
Budget: 2-4 KB calls. Skip if app_name not available.
```
...
- `lcp-api.getInterface` with object UUID, name, type
- `get_version_context` with THIS object's UUID, typeId, dateTime: `reference_date` — **MANDATORY for every diffable object**. Each object has its own independent version history. DO NOT reuse a version number from another object.
  - Use the assignee logic: find the FIRST `after` version by the assignee → baseline is the version immediately before it.
  - Then call `solutions-intelligence.get_object_history` with the resolved `versionNumber` for THIS specific object.
...
From lcp-api.getInterface, extract: `name`, `description`, `parentFolder`, source code/config, `inputs[]`

**Database verification (MANDATORY when object interacts with database) — solutions-intelligence-smt integration:**
...
Call `analyze_appian_code` with object UUID and name.
...
**For Internationalization — AUTOMATED i18n AUDIT (solutions-intelligence-i18n integration):**
...
2. **For each hard-coded string found**, determine if a key already exists:
   - **BND apps** (GSS, VM, GCW, RM, AM): Query `Appian.BND_Key` table:
     ...
     LIMIT 5
     ```
   - **Translation Set apps** (GSM): Use `solutions-intelligence_get_translation(app_name, query="{text}")` 
...
1. Read the `solutions-intelligence.list_applications` response to get the drive folder:
