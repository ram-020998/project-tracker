...
1. `solutions-intelligence.search_objects(app, name)` → find the component, note its description
2. `solutions-intelligence.get_transitive_dependencies(app, name, direction="inbound")` → everything that depends on it
3. Group the results by bundle (feature) using the `bundles` field
4. `solutions-intelligence.get_hub_objects(app)` → check if this is a widely-shared component
