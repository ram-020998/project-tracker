...
1. `solutions-intelligence.list_applications()` → see all available apps
2. For each relevant app:
   - `solutions-intelligence.search_bundles(app, keyword)` → find matching features
   - `solutions-intelligence.get_app_overview(app)` → compare sizes and coverage
