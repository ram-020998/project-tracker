...
1. `solutions-intelligence.search_bundles(app, keyword)` → find the feature
2. `solutions-intelligence.get_bundle(app, bundle_id)` → get structure, flow, and member summary
...
5. If PO wants to see specific forms: `solutions-intelligence.get_bundle(app, id, object_type="Interface")`
6. If PO wants to see workflows: `solutions-intelligence.get_bundle(app, id, object_type="Process Model")`
...
- Then and only then, use `solutions-intelligence.get_object_code(app, name)` and show implementation
