...
1. `list_orphans(app)` → get the full orphan summary with `by_type` breakdown
2. `list_orphans(app, object_type="Interface")` → unused forms (user-visible debt)
3. `list_orphans(app, object_type="Expression Rule")` → unused business logic
4. `get_hub_objects(app)` → over-used components (refactoring candidates)
5. `get_app_overview(app)` → coverage stats (bundled vs orphaned)
