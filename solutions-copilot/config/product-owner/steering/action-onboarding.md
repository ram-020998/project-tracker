...
1. `list_applications()` → show available apps, let PO pick one
2. `get_app_overview(app)` → get the full map
...
4. `search_bundles(app, "")` or browse the bundle list from overview → group by business area
