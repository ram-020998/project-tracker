...
- **Testing collaboration locally (multiple instances):** a separate **`genesis-fleet`** CLI (`gsm`; repo `git@gitlab.appian-stratus.com:ramaswamy.u/genesis-fleet.git`, standalone — NOT in the genesis release chain) runs + manages multiple Genesis instances on one machine, each with its own `GENESIS_STATE_DIR`/`GENESIS_ARTIFACTS_DIR`/`--port` + collab env, sharing the same Appian Hub. `gsm add <name>` auto-provisions an instance (installs the workflows library, symlinks the host's native Dev/DevOps MCPs, seeds the dev env + `env-*` creds, enables collaboration) so you can publish from one "user" and pull into another; only the per-user Appian **identity** is set in each instance's UI. See `genesis-fleet/README.md`. **After releasing a new genesis-workflows version, refresh the fleet instances' installed libraries with `gsm provision --all --no-mcp --no-env --force`** (reinstalls the workflows library from the local checkout — defaults to `~/repo-gitlab/ramaswamy.u/genesis-workflows` — into every instance's state dir; skips the MCP symlink + env re-seed; a running `genesis serve` picks up the new library at run-start, no restart). Verify via each instance's `GET /api/catalog`.
