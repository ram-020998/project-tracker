...
- **A stale server-scoped `appian-dev/LCP_URL` secret silently overrides the env-derived dev URL → a 401 against the WRONG Appian environment (2026-09-06 live).** The Applications page 401'd on `listApplications` (`APNX-1-4187-000 "Authentication failed"`) even though the dev env + credentials were correct. Root cause: `McpRegistry` resolves `${LCP_URL}` in the order **SecretProvider → EnvironmentRegistry → os.environ**, and an **orphaned pre-ADR-048 secret `appian-dev/LCP_URL`** (a legacy environment's URL) won over the correct value the dev-tagged env auto-derives — so the (valid) credentials were sent to a *different* Appian instance. `EnvironmentRegistry.resolve_var("LCP_URL")` returned the right URL, but the merged MCP registry did not (secret precedence). **Diagnosis method that worked:** probe the endpoint directly with each auth scheme (Basic → 200, api-key → 401 ⇒ creds valid, endpoint is Basic), read the *installed* `lcp_mcp_server` (its `applications` domain uses `httpx.BasicAuth(USERNAME/PASSWORD)`; a per-request Bearer token only overrides when present), then compare `dev_environment().url` vs the merged-registry-resolved `LCP_URL` — they differed → traced to the `appian-dev/LCP_URL` secret via `SecretProvider.keys()`. **Fix:** delete the stale `appian-dev/LCP_URL` secret; `LCP_URL` then falls through to the env-derived value and follows any future Settings→Environments URL edit (no restart — resolved per call). **Lesson:** the ADR-048 migration relocates the *credentials* into the `env-<hash(label)>` scope but does NOT clean a legacy server-scoped `LCP_URL`/`APPIAN_DOMAIN` (they're public, not secrets), so an orphan can linger and, because of secret-first precedence, override the environment. A guard (drop a legacy `appian-dev/LCP_URL` in the migration, and/or make the two `ENV_MANAGED_SERVERS` **prefer the env-derived URL** over any server-scoped secret) would make this class of bug impossible — filed as a follow-up.

- **A permissive local Hub emulator hid a real contract mismatch — the first live `stage_artifact` publish 400'd (2026-09-06, genesis v0.66.2).** Publishing a completed feature stage to the live Genesis Hub failed with `PUT /records/feature_stage/<uuid> → 400: invalid kind or missing syncUuid`. Root cause: the `CollaborationService` entity binding sent **kind `feature_stage`** with a **`feature_sync_uuid`** parent field + an artifact blob keyed by the **stage's** sync_uuid, but the frozen Genesis Hub contract (36-01 §2.1) has **kind `stage_artifact`** with **`parent_sync_uuid` + `parent_kind`**, a renamed **`upstream_versions`** field (local column is `upstream_versions_json`), and an artifact blob keyed **`<parent_sync_uuid>:<stage>`** with that **`blob_key`** carried on the record. **The `LocalHubProvider` accepts ANY kind/fields**, so Phase-38's headless acceptance + all unit tests passed while the wire shape was wrong — this only surfaced on the FIRST real publish against Appian (Phase 37 live-tested KB blobs + identity/teams, not the Phase-38 records path). This is the §7 "stub hid the contract" lesson again (ACP env-list, DocumentStore.list, etc.). **Fix (v0.66.2):** remap the stage binding to `stage_artifact` (kind + `parent_kind` + `rename_out` for `upstream_versions` + `blob_key`/`content_hash` on the record; pull reverses the rename + fetches the blob by `<parent>:<stage>` key) + a guard test asserting every `_BINDINGS` kind is a real contract record kind. Verified live: `publish_feature_stage → PUT stage_artifact → created v0` against `merge-assist-dev`. **Lesson:** a local emulator of an external contract MUST validate the same way the real peer does (reject unknown kinds / enforce the field schema), or add a contract-conformance test that runs the *service's* published payloads through the frozen fixtures — otherwise a permissive double green-lights a wire mismatch that only a live call catches. epic/story/board_state/feature happened to already match; only `stage_artifact` diverged.

- **A read-only pull mirror must reconstruct local fields the frozen Hub contract doesn't carry — and the consuming UI must render a chat-less pulled artifact (2026-09-07, genesis v0.66.3–v0.66.5, collaboration puller).** After the `stage_artifact` publish fix (§7 above), the FIRST real *pull* into a second instance surfaced a cascade — again all masked by the permissive `LocalHubProvider` (which echoes the full published payload, so tests + headless acceptance passed while the real Appian Hub, which stores only its contract fields, did not): **(1)** `pull_all` **crashed** with `NOT NULL constraint failed: kb_feature_stages.title` — the Hub `stage_artifact` record has no `title`, but the local table requires it. Fix: `_upsert_local` fills every NOT-NULL-no-default local column absent from the payload on a mirror INSERT (`title` ← a nice stage label via `_STAGE_LABELS`, `*_at` ← now). **(2)** Pulled features **never populated the Workbench board** — board auto-membership requires `kb_features.stories_finalized_at IS NOT NULL`, but the Hub `feature` record has no `stories_finalized_at` (dropped). Fix: `pull_boards` **derives** it — a pulled feature that has stories WAS finalized upstream (the publisher only publishes a backlog at Finalize), so stamp it (idempotent; a no-op on the publisher). **(3)** Every stage showed **"Start the stage"** though completed — `StageBuilderPage` decided start-vs-artifact purely on `chat_session_id`, but a pulled mirror has the completed artifact with **no local chat** (chat/run stay local, ADR-063). Fix: a third branch renders a completed/in-review chat-less stage **read-only** (the artifact via `artifactUrlFor` annotate=0 + a "shared by your team" banner). **Diagnosis method:** compare, per entity, the `_map_out` payload keys against the frozen contract field set (a probe) — the genuinely-dropped, non-attribution fields were exactly `feature.stories_finalized_at` + `stage_artifact.title` (`published_by` is not a loss — the Hub stores it as `created_by`/`modified_by` and `_to_record` recovers it). **Lesson:** for a read-only mirror of a frozen external contract, (a) the write side must satisfy the local schema's NOT-NULL/derived columns from what the contract *does* carry, (b) any local-only semantic marker the contract omits (e.g. a "finalized" flag) must be **derived** from what *is* shared, (c) the consuming UI must not gate on local-only authoring handles (chat/run) when deciding whether an artifact exists, and (d) **no Hub change is needed** when the derivations are sound — do them puller-side rather than bumping a frozen contract + a cross-team Appian change. And, once more: a permissive local emulator MUST validate/normalize like the real peer (reject unknown kinds, drop non-contract fields) or the whole class stays invisible until a live call.


- **A chat-authored artifact's `html_path` is NULL until a milestone — resolve stage HTML via the sandbox-aware `_current_stage_html`, not the raw column (2026-09-09, genesis v0.68.1).** The `ux-design-analysis` run for a feature with a **completed** Spec produced a doc that said *"no spec has been authored"* and flagged every screen as a gap. Root cause: a spec authored in the bound `feature_spec` chat writes `spec.html` into the **session sandbox** (`skill-output/<sid>/spec.html`); the `kb_feature_stages.html_path` column is only populated when a **milestone** is saved, so a completed-but-unmilestoned spec has `html_path = NULL`. The UX launch (`api/features.py::_launch_analysis`) passed `spec_row["html_path"]` straight to the workflow's `spec_path` input → the `_load_spec` node saw nothing → *"no spec"*. The **Technical Design** and **Feature Breakdown** launches already avoid this — they snapshot **`_current_stage_html(spec_row)`** (which *prefers the live sandbox* over `html_path`) to a file and pass that; the UX launch had never been updated to that pattern. Fix: `_launch_analysis` snapshots `_current_stage_html(spec_row)` to `spec_snapshot.html` and passes it. **Lesson: never read a stage/spec's HTML via the raw `html_path` column for downstream consumption — always go through `_current_stage_html` (sandbox-first), because chat-authored artifacts live in the session sandbox until milestoned. When one launch path is fixed for a cross-cutting resolution rule, audit the sibling launch paths for the same rule.** (Existing runs aren't retroactively fixed — the stage must be re-run.)