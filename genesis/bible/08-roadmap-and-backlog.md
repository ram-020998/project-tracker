...
> - **Phase 36 — The Genesis Hub Appian Application** (`specs/phase-36-genesis-hub-appian-app.md` + `36-01..36-07`; **ADR-064**). **Appian-side, built by a separate Dev-MCP agent** — record types (Team/Membership/Feature/Epic/Story/BoardState/StageArtifact-metadata, UUID-keyed, versioned, attributed) + document-management record types (KbBlob + ArtifactBlob, versioned, deduped, keep-last-N) + the **Web-API surface** (records upsert/list/get + **base-version CAS→409**; blobs up/down/versions + dedup; `changes` manifest + `contract_version`; activity; teams) + a shared service account + an installable **package** + a **frozen JSON contract + fixtures** checked into genesis. **No genesis tag.** Isolated from subject apps.
