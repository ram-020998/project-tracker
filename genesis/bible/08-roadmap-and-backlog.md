...
> - **Phase 35 — Collaboration Foundations & Identity** (`specs/phase-35-collaboration-foundations.md` + `35-01..35-07`; **ADR-063**). genesis, suggest **v0.63.0**; **m0019** (`sync_uuid` + backfill on the 6 synced tables; `row_version` where missing; owner/team/provenance cols; `collab_*` local tables; `current_version` 18→19). The `genesis/collab/` `SyncProvider` seam + `CollaborationService` + a **local/mock provider** (buildable/testable **without Appian**) + `build_sync_provider()` opt-in registry; the identity/team model + `current_user()`/canonical-username unification (+ `actor` threading) + onboarding; the **Settings identity/team page** + Hub-config opt-in UI. All exercised against the mock provider.
