...
### 🟡 SPEC-DRAFTED (awaiting build sign-off) — Phase 44: Workbench, SDLC & Workspace Fixes (12-issue bundle)

> **Status:** 🟡 **DRAFTED — spec only.** genesis-only (web + backend); **one migration** (`m0022`, `current_version` 21 → 22) for the story-identifier sequence (fix 3); every other fix is code-only. **No Genesis Hub change.** Spec: `specs/phase-44-fixes-bundle.md`. **New ADR-072 (Proposed)** — global human-readable story identifier. **Analysis + spec only — no code changed. Build begins only after the user's go-ahead.**
>
