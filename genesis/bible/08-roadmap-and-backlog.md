...
### ▶ IN PROGRESS — Phase 27: UI/UX Revamp (27-01..27-11) — ALL BUILT · release v0.53.0 HELD (user verifying)
...
>
> **Sub-phase ledger.** 27-01 research/audit ✅ · 27-02 wireframes ✅ · 27-03 hi-fi mockups ✅ **APPROVED** (mockup commits on master through `16c2b8a`) · **27-04** design-system foundation (global light-first tokens, Poppins, default flip dark→light, per-theme shadows, motion utils) ✅ **BUILT** genesis `286528c` LOCAL/untagged · **27-05** app shell & navigation (differentiated Sidebar, glassy TopBar + route breadcrumbs + ⌘K CommandPalette; `shared/layout/{nav,breadcrumbs,command-palette-store,CommandPalette,TopBar}` + recomposed `AppShell`) ✅ **BUILT** genesis `cf8bfb8` LOCAL/untagged · **27-06** Applications/Features/Spec (core SDLC surface → mockups; gradient tiles, rounded-2xl, motion, glassy chrome, breadcrumb names) ✅ **BUILT** genesis `a85dd83` LOCAL/untagged · **27-07** Runs/Run-detail (runs table + glassy header + rounded graph nodes + HITL card; breadcrumb) ✅ **BUILT** genesis `1000af0` LOCAL/untagged · **27-08** Chat/Copilot (soft-gradient session list + glassy composer + rounded copilot cards) ✅ **BUILT** genesis `f541af1` LOCAL/untagged · **27-09** Documents/Memory (restyled + **tokenized the `/memory` constellation** — last hardcoded-dark surface resolved) ✅ **BUILT** genesis `9b2e733` LOCAL/untagged · **27-10** Catalog/Settings/Overview (gradient catalog cards + Overview motion) ✅ **BUILT** genesis `bf41220` LOCAL/untagged · **27-11** polish + a11y (memory-graph keyboard nav + jest-axe on every mountable page; hex gate CLEAN; dark parity verified) ✅ **WORK COMPLETE** genesis `b8defa0` LOCAL/untagged — **the v0.53.0 RELEASE IS HELD pending the user's verification of the running app** (then bump/tag/push on go-ahead). The built sub-phases each passed an independent review (SHIP); web **vitest 200**, tsc/eslint/build green; zero hardcoded brand hex in components.
>
> **Release.** 27-04 + 27-05 (+ later page phases) are UNRELEASED (LOCAL on genesis master) and ship together as **genesis v0.53.0** on the user's go-ahead (frontend-only; genesis pytest unaffected). **Resolved:** the `/memory` d3 constellation is now token-driven (27-09) — no hardcoded-dark surfaces remain.
