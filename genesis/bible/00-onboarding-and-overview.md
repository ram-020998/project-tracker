...
> §5 (ADRs), §7 (lessons), and §9 (roadmap). **Last refreshed: 2026-09-25 (✅ LATEST SHIPPED **genesis v0.75.0 — Phase 44 Workbench, SDLC & Workspace Fixes (2026-09-25; ADR-072 Accepted; CI green; genesis-only; m0022, DB→v22)** (a 12-issue bug bundle: the Workbench finalizers publish system lane moves to the Hub [fixes design→design-review not sticking + propagation]; real-time live streaming in the run node View-Details; runs server-side pagination; global GNS_ story ids; chat + catalog internal-component filters; + UI-layout fixes — `progress/phase-44-fixes-bundle.md`) — Prior: **genesis v0.74.0 — Phase 43 The Unified Story Workspace (2026-09-24; ADR-071 Accepted; CI green; genesis-only, no migration)** (ONE canonical, consolidated story view from anywhere — Overview / Design [read-only + Edit-chat in a new tab] / Implementation [master-detail doc viewer]; origin-aware breadcrumb; `progress/phase-43-unified-story-workspace.md`) — Prior: **genesis v0.73.0 + genesis-workflows v0.20.0 + genesis-core v0.9.8 — Phase 42 Code Implementation (2026-09-24; ADR-068/069/070 Accepted; CI green)** (Genesis's first write-capable stage: drag a story into the Workbench **Implementation** lane → an unattended `story-implementation` workflow reverifies the design vs the Technical Design → prepares a local rollback doc → a single agent implements the object changes in the dev env via Dev-MCP **write** tools [no deletes — deprecate-in-description] → verify → heal → updates the shared **Object Wiki** → moves the card to Code Review; ADR-068/069/070; m0021; `specs/phase-42-code-implementation.md`)** — **Phase 41: Dev MCP 26.6.90 upgrade (basic auth) + SAIL CLI install** [v0.72.2 CLI `install-native` parity; **v0.72.3** managed-CLI cards resolve via the installer (sail/gws now show **reachable** in Settings → CLI, not a PATH probe) + the fleet provisioner shares `cli-tools`; **deployed + live-verified** against merge-assist-dev — listApplications OK via basic auth on the host + the running fleet; sail/gws reachable on both fleet instances] — pin `LCP_AUTH_METHOD=basic`, drop the obsolete `LCP_API_PATH` (removed end-to-end), +5 read tools; install the bundled **SAIL CLI** in ONE action as a managed CLI wired to the dev-env basic creds + isolated `SAIL_HOME` (no consumer yet); browser/SSO auth deferred → `specs/backlog/devmcp-browser-sso-auth.md`. See `progress/phase-41-devmcp-upgrade-and-sail-cli.md`. Prior: **genesis v0.71.0** — a Workbench board→application-page link (the app name in a board header opens `/applications/:uuid` in a new tab; frontend-only). Prior: **genesis v0.70.0** — **Phase 39 Run Inspection & Node Observability** [ADR-066] + **Phase 40 Self-Healing Reliability** [ADR-067]; genesis-core v0.9.7 · genesis-workflows v0.17.0; both ADRs Accepted; see `progress/phase-39-run-inspection-observability.md` + `progress/phase-40-self-healing-reliability.md`.) — latest SHIPPED: **genesis v0.67.0 — ✅ Two-toggle collaboration enablement (ADR-065) + the collaboration puller-fidelity fixes: a Settings-level enable switch gated on Genesis Hub availability, on top of the env master; Hub URL auto-derived from the dev env (`/suite/webapi`) + a one-time Hub-app-UUID field; plus the v0.66.2–v0.66.5 fixes that make a PULLER see everything — the `stage_artifact` contract mapping, the pull mirror filling local NOT-NULL cols + deriving `stories_finalized_at` (the Hub drops both), and a read-only view of pulled stage artifacts (no Hub change needed); plus **auto-pull** (v0.67.0) so a puller reflects teammate changes automatically — app load / route change / refocus + a board-aware poll [**10s on a board**, 30s idle] that pulls when the Hub change cursor advances; reload always re-pulls. **v0.68.0**: a **backend** always-on auto-pull loop (browser-independent — fixes background-tab timer throttling) + a sticky Workbench board header; measured Hub read-back ~0.9s, no Appian lag. **v0.69.0** — Workbench board fixes: finalized stories default to **to-do** (entry lane, not 'design'); board membership is **explicit** (a shared `kb_stories.on_board` flag set on Import, m0020 — no longer auto-adding every finalized story); a drag-vs-refetch guard; + sticky/shared-scroll board. **v0.68.1**: sticky/shared-scroll Workbench board + a UX-analysis spec-load fix (snapshot the current spec HTML, not the null html_path).** Prior: **genesis v0.65.0 — ✅ Phase 38 Collaborative SDLC COMPLETE** (the local-first → shared-Hub collaboration program is DELIVERED for the human-authored SDLC: per-entity publish/pull for **features + published stage artifacts** [publish-on-complete, artifact-only — chat/run stay local; pull = a read-only mirror on the puller's disk; cross-machine parent resolution], **epics/stories** [Finalize/edit/delete; lists lossless via GH Story Item; tombstone deletes], and **boards** [lane=shared story status; auto membership; in-lane order + sidebar curation stay local], the **collaboration UX** [`<PublishToTeamButton>` publish-on-complete + Sync-now/auto-pull + advisory markers, reusing the Phase-37 components], and a one-time confirmed **adoption** bulk-publish; **opt-in / off by default** — solo installs unchanged; genesis-only, no migration; `scripts/acceptance/phase-38-collaborative-sdlc-acceptance.py`. Shared-memory sync is the one deferred future phase). Prior: **genesis v0.64.0 — ✅ Phase 37 Hub Wire-up COMPLETE** (the collaboration mechanism LIVE against the real Appian Hub for the KB + identity/teams: `AppianHubProvider` [the frozen Phase-36 contract] + `KbStore.hydrate_from_blob` pull-first KB sharing + `publish_kb`/`pull_kb_if_newer` [content-hash dedup, per-app lock, Hub-app exclusion] + live identity/teams + an onboarding-before-first-publish gate + the sync-UX foundation [`/collab/status`+`/changes`, `<HubStatus>`/`<StalenessBadge>`]; implements ADR-063/064; genesis-only, no migration). Prior: **genesis v0.63.0** — **Phase 35 Collaboration Foundations & Identity COMPLETE** (local-first → shared-Hub substrate, **opt-in/off-by-default**; m0019 `sync_uuid`/`row_version`/provenance + `collab_*` tables [`current_version`→19]; `genesis/collab/` `SyncProvider` seam + `LocalHubProvider` + `CollaborationService`; self-asserted identity + `canonical_username()` + `actor` threading; Settings→Collaboration + onboarding; ADR-063 Accepted). ✅ **Phase 36 — the Genesis Hub Appian application — COMPLETE** (ADR-064 Accepted; built + **live-validated** in the team dev env by a separate write-capable Dev-MCP agent — genesis stays read-only per ADR-036/037): the isolated `Genesis Hub` app + 11 record types + 13 relationships + record-level security + 9 constants + the base64 blob store + **11 Web APIs + 12 helper rules**, plus a **frozen JSON contract + fixtures** checked into genesis; every endpoint validated end-to-end with the service-account key (CAS→409, unicode-lossless, story explode/reassemble, /changes, activity, blob put/dedup/versions/GET). Next: 37 Hub wire-up + 38 Collaborative SDLC (see §9). Latest SHIPPED: **genesis v0.63.0 + genesis-workflows v0.16.0 + genesis-core v0.9.6 + kiro-agent-sdk v0.7.1** — **Phase 34 Design Lane Automation COMPLETE** (the first Workbench per-lane automation — dragging a ticket into **Design** opens a confirm dialog → a grounded, object-level, **code-level** `story-design-analysis` run [gather research → per-object grounded design with the actual code, process models node-by-node → deterministic assemble → grounded verify critic]; on success the card **auto-advances to Design Review** via an **audited** `LifecycleService` transition [m0013] where a bound `story_design` chat + an annotatable Lavish preview refine `design.html`; a running card is locked + shows a run link, a failed run paints it light-red; **m0018** `kb_story_stages` [per-`(story, stage)` artifact; `current_version` 17→18] + `StoryStageStore` + `StoryDesignFinalizer`; removed the superseded Phase-15 `design-doc`; ADR-062). Prior: **Phase 33 The Workbench COMPLETE** (a per-application Kanban execution board over finalized stories — curated boards as left-sidebar sub-tabs + a `+` to add applications, batched **Import** → To Do, drag across the 8 lanes [To Do → Design → Design Review → Implementation → Code Review → Verification → Deployment → Done] + intra-lane reorder; lane = `kb_stories.status`; m0017 `workbench_boards`/`kb_board_cards`; `@dnd-kit`; ADR-061; UI-only board, per-lane automation is future). Prior: **Phase 32 Finalize Stories** (a completed Feature Breakdown → first-class, editable `kb_epics`/`kb_stories`; m0016; ADR-060). Prior: **Phase 31 Feature Breakdown stage COMPLETE** — the FOURTH feature stage: once Spec + UX + Technical Design exist, Start (optional notes + up to 3 supporting docs, ADR-035) → the **`feature-breakdown-analysis`** workflow (epics = the Technical Design's functional workstreams → per-epic story/task breakdown [Appian form-vs-process-model split, entry-point splitting, Story=front-end-testable / Task=not-FE-verifiable, **Gherkin** AC, one-line TD-anchored dev notes] → **deterministic assemble** [a Lavish-safe `breakdown.html` — `<details>` cards + a CSS-only table toggle + an embedded canonical `backlog.json`] → **grounded coverage** critic → cleanup) → a **`feature_breakdown`** completion chat + a **Jira-importable CSV export** (create-new-epics via Issue ID → Parent ID); ADR-059; no migration (m0015 reused); genesis + genesis-workflows only; CI green — genesis #6735324 / workflows #6735326. Prior: run-detail workflow-graph revamp (elkjs layered LR + orthogonal routing, executed path GREEN + ×N counts, `/runs/{id}/transitions`) + run-detail perf + honest partial credit provenance + worker terminal-status fix + no-cache index.html; technical-design-analysis v0.2.0 (genesis v0.58.0 / genesis-workflows v0.14.0; CI green — genesis #6727262 / workflows #6727270). Prior: **Phase 30 Technical Design stage COMPLETE** (grounded Spec+UX→Technical Design; ADR-058 + the ADR-056 prerequisite amendment; CI green — genesis #6725001 / workflows #6725004) + clickable stage cards + openable artifacts. Prior: global ⌘K search + run-detail display fixes + single-nav feature workspace + UX **Re-upload & re-run** + StageFinalizer correctness (v0.56.0→v0.56.2) and the UX-level `ux-design-analysis` v0.1.1 (FB-4, workflows v0.12.1), all on top of Phase 29 UX Design stage COMPLETE (grounded mockup→UX Implementation Analysis, ADR-057; CI green — core #6680648 / genesis #6680663 / workflows #6680673). Prior: Phase 28 Feature Workspace framework (v0.54.0, ADR-056). Prior: genesis v0.53.0 (Phase 27) +
