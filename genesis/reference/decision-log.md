...
## ADR-056 — The Genesis Feature Workspace (parallel, plug-in stages) (Phase 28) — **Accepted** · **amended by ADR-058 (Phase 30 — stage prerequisites)**
...
## ADR-058 — The Technical Design stage: grounded, workstream-decomposed technical design (Phase 30) — **Accepted**
- **Status:** **Proposed** (2026-09-03, Phase 30; specs `specs/phase-30-technical-design-stage.md` + `phase-30-technical-design-stage/30-01..30-07`; findings `30-01-findings.md`). Refines ADR-057's stage pattern; **amends ADR-056** (introduces stage prerequisites). **Built 30-03 (workflow) / 30-04 (platform) / 30-05 (UX refinements); review-hardened 30-06 (a TD-start guard for an already-finalized stage + a finalizer stage-match guard); released 30-07 — genesis v0.57.0 + genesis-workflows v0.13.0, CI green (genesis #6725001 / workflows #6725004).** **Component reuse:** renders through the SAME generalized Phase-29 surface — `StageArtifactWorkspace`, `AnnotatablePreviewDialog`, `StageBuilderPage`, the reused `ChatThread`/annotation bridge, the m0015 `StageStore`, and a **generalized `StageFinalizer`** (a workflow→stage binding registry replacing the UX-hardcoded constants). **No migration, no genesis-core/SDK change** (a smaller phase than 29).
- **Decision:** A Feature's **Technical Design** stage consumes the finalized **Spec** + **UX Implementation Analysis** (+ an optional user **comment**) and produces a single reader-first **Technical Design** HTML artifact via a deterministic **`technical-design-analysis`** LangGraph workflow (ADR-001, run label "Technical Design Preparation"): program nodes plan/loop/persist; narrow Kiro agent nodes (reliability trio each, ADR-011) judge. Flow = `plan_sections` (decompose into **functional workstreams**) → a per-workstream **existing-state grounding** loop (`analyze_section`, MAP) → a per-workstream **design drafting** loop (`draft_section`, MAP) → `assemble` (an **agent** coherence pass, REDUCE) → a **grounded verification (critic)** node (bounded revise loop → escalation gate) → `present`. Grounded in the **live Appian environment** — structure/dependencies/blast-radius from the internal **`genesis-kb`**, actual code from the **`appian-dev` MCP** — reconciled against the Spec + UX per workstream. Output is **object-level and code-grounded** (the *inverse* of ADR-057's intent-level UX doc): each change names a real existing object OR is marked **NEW**; new tables get a data-model table; organized by **functional workstream** for readability (Objective → What exists today → What changes [DB/Records/Processes/UI/Expression-rules/Integrations] → Complex designs → Open Questions) + a global Complex Designs + Open Questions. The agent **must ground every claim and must not assume** — uncertainty becomes an Open Question. A bound **`technical_design` completion chat** (a `ChatModeProfile`, reusing the Phase-20/21 sandbox + annotatable review; the ONLY new agent prompt = `_STEERING_TD`) then walks the open questions, editing the HTML live as the user answers, until the stage is marked **completed**. **Entry** = a comment textarea (optional — empty accepted) + Start (a JSON start, NOT a file upload); **re-run** resets the stage + relaunches with a fresh comment (mirrors UX Re-upload).
- **Context:** Phase 28 (ADR-056) framed a Feature as parallel plug-in stages; Phase 29 (ADR-057) made UX Design the first live one after Spec and built the generalized reuse surface (m0015, the stage components, the StageFinalizer). Technical Design is the natural next stage and the first that **depends on** its predecessors. The real SDLC writes a Feature Technical Design AFTER the Spec + mockups exist, grounded in how the app is already built — captured in the five example docs at `~/Documents/GSS/technical-design-examples` (the format authority; `30-01-findings.md`). Research (hierarchical Planner→Manager→Worker decomposition beating monolithic 95.7% vs 80.9%; map-reduce + externally-grounded critic-refiner; context-isolated per-unit iterations) motivates the two-loop shape.
- **Alternatives:** (a) one monolithic generation of the whole TD — rejected (long single-agent runs erode after the first unit + hallucinate object-level detail); (b) decompose by **object type** not functional workstream — rejected (the user wants a reader who follows the feature area-by-area); (c) an ungrounded self-critic — rejected (the "progress mirage"); (d) reproduce the blank A–L template Q&A format — rejected (the user wants the *constructed* reader-first doc with the agent's own Open Questions); (e) a new per-stage migration — unnecessary (m0015 is already generalized); (f) a second StageFinalizer instance — rejected in favour of one finalizer with a workflow→stage binding registry.
- **Consequences:** **Amends ADR-056** — a stage MAY declare **prerequisite stages** (`StageDescriptor.requires`): Technical Design requires the Spec + UX Design **artifacts present** (status `in-review`/`completed` — full "Mark complete" NOT required), enforced BOTH frontend (a locked card + blocked workspace) AND backend (start **409** + `resolve_inputs` fail-fast). The parallel, non-gating model otherwise stands (Breakdown declares its own). **Read-only** against Appian (ADR-036/037; appian-dev read allowlist + genesis-kb; the design *proposes* object changes but never creates/edits them; the completion chat's only write authority is the fs-sandbox HTML edit — ADR-031/045). `design` becomes an `available:true` stage plugging into the ADR-056 framework via a `STAGE_DEFS` row + a `stage-registry` entry + the inner workspace — **no shell edits**. **Two-repo** release (genesis → genesis-workflows; keep the pin chain consistent — §7); `current_version` stays **15**. Two user-requested UX refinements ship with the phase (30-05): the stage **card itself is clickable** (drop the Open button, all stages, accessible overlay-link) + **artifacts are openable** (generated → read-only rendered preview; reference → the Document Library viewer). Reuses ADR-042/043, ADR-050 (the stage `LifecycleService` machine + m0013 audit), ADR-011/031/045.

## ADR-059 — The Feature Breakdown stage: grounded, workstream-decomposed, Jira-ready backlog (Phase 31) — **Accepted**
- **Status:** **Accepted** (2026-09-04, Phase 31; specs `specs/phase-31-feature-breakdown-stage.md` + `phase-31-feature-breakdown-stage/31-01..31-07`; findings `31-01-findings.md`). **Built 31-03 (workflow) / 31-04 (platform) / 31-05 (Jira CSV export); review-hardened 31-06 (a Lavish paper-bg readability fix + a deterministic Lavish-injection test); SHIPPED 31-07 — genesis v0.59.0 + genesis-workflows v0.15.0, CI green (genesis #6735324 / workflows #6735326).** Extends the ADR-056 prerequisite amendment (introduced by ADR-058) to a **three-way** prerequisite. **Component reuse:** renders through the SAME generalized Phase-29/30 surface — `StageArtifactWorkspace`, `AnnotatablePreviewDialog`, `StageBuilderPage`, the reused `ChatThread`/annotation bridge, the m0015 `StageStore`, and the **generalized `StageFinalizer`** (one more workflow→stage binding). **No migration, no genesis-core/SDK change.** Methodology authority = the `spec-to-backlog` Kiro skill (`gitlab…/allison.olson/spec-to-backlog`) — **analysis core + `Backlog_JSON` + Jira-column contract only**; format authority = the four GSS example breakdowns (`~/Documents/GSS/breakdown-examples`; `31-01-findings.md`).
- **Decision:** A Feature's **Feature Breakdown** stage consumes the finalized **Spec + UX Implementation Analysis + Technical Design** (+ optional **notes** + up to **3 uploaded supporting docs** — transcript/spike/etc.) and produces a reader-first **breakdown** HTML artifact **and a canonical `backlog.json`** via a deterministic **`feature-breakdown-analysis`** LangGraph workflow (ADR-001, run label "Feature Breakdown Preparation"): program nodes plan/loop/persist/assemble; narrow Kiro agent nodes (reliability trio each, ADR-011) judge. Flow = `plan_epics` (epics = the Technical Design's **functional workstreams**, 1:1) → a per-epic **story/task breakdown** loop (`break_epic`, MAP) → **`assemble`** (a **PROGRAM** node, REDUCE — builds `backlog.json` + renders `breakdown.html`; structured→HTML is mechanical, so no giant-emission agent turn) → a **grounded verification (critic)** node (coverage vs Spec scope + every TD "What changes" item; bounded revise → escalation gate) → `present` → `cleanup`. **Appian-native breakdown rules:** a **form** → one story (split by section only when complex); a **process model** → a **separate** story; split by **entry point**; separate happy/error; explicit edge-case/state-status/integration/config/operational stories; group notifications; flag nice-to-haves; **clarity over volume**. Each item is a **Story** (front-end testable — carries **Gherkin** Given/When/Then AC) or a **Task** (a backend/data-model/APPREF/process-only change a tester **cannot** validate from the front end — AC optional); **no** story points; the implementation note is a **one-line pointer into the Technical Design** (`devNoteRef`), not a rewrite (the TD is already object-grounded, so grounding is **light** — a read-only live spot-check for the critic only). Output HTML is **Lavish-safe** (a summary header + `<details>` story cards grouped by epic + a **CSS-only** card↔table toggle — **zero JavaScript** — with the canonical backlog JSON embedded in `<script id="genesis-backlog">` for lossless export). A bound **`feature_breakdown` completion chat** (a `ChatModeProfile`, reusing the Phase-20/21 sandbox + annotatable review; the ONLY new agent prompt = `_STEERING_FB`, which also keeps the embedded JSON in sync) refines the backlog with the user until the stage is marked **completed**. A **Jira-importable CSV export** (`GET …/stages/feature_breakdown/export.csv`) is produced deterministically from the embedded canonical JSON using the skill's column contract (`Issue ID · Issue Type · Summary · Description · Parent ID · Acceptance Criteria · Labels`), linking the hierarchy via **Issue ID → Parent ID**, **always creating new epics**, with **no custom-field columns** (built with the already-pinned `openpyxl`/`csv` — no new dependency). **Entry** = a **multipart** start (optional notes textarea + ≤3 optional file uploads, current ADR-035 allowlist) + Start; **re-run** resets + relaunches.
- **Context:** Phase 28 (ADR-056) framed a Feature as parallel plug-in stages; Phase 29 (ADR-057) made UX Design live + built the generalized reuse surface; Phase 30 (ADR-058) made Technical Design live + introduced stage prerequisites + generalized the StageFinalizer into a workflow→stage binding registry. Feature Breakdown is the **fourth and final** stage — turning the finished designs into the developer-executable backlog a team grooms + imports into Jira. Today this is a manual spreadsheet exercise (the GSS "Feature Breakdown Sheet") and is **Appian-specific** (form vs process-model split; single-owner forms; per-entry-point stories). The `spec-to-backlog` skill supplies a proven analysis methodology + JSON + Jira-column contract; the Technical Design supplies an already-grounded structural spine (its workstreams → epics; its "What changes" items → stories with TD-anchored Dev Notes). Research (the same hierarchical/map-reduce + externally-grounded critic topology as Phase 30) motivates the per-epic map + deterministic reduce.
- **Alternatives:** (a) reuse the skill's Lucid/Google-Sheets/Jira-REST renderers — rejected (external services + `gws`/Lucid; Genesis is local single-user — we produce a file, the human imports it); (b) let an agent emit the whole breakdown HTML — rejected (the Phase-30 giant-emission timeout; a backlog is structured, so `assemble` is a **program**); (c) re-ground every story against the live app — rejected (the TD is already object-grounded; a one-line TD pointer + a light critic spot-check is enough + far cheaper); (d) group epics by user-journey independent of the TD — rejected (the user chose functional workstreams for whole-pipeline coherence); (e) the skill's "* Verify…" AC — rejected in favour of **Gherkin** (the newest GSS sheet's style); (f) AI story-point estimation — rejected (noise without team velocity); (g) rich JavaScript interactivity in the artifact — rejected in favour of **CSS-only** so the Lavish annotation layer can't break; (h) a new per-stage migration — unnecessary (m0015 is generalized); (i) a push-to-Jira button — deferred (out of scope; the importable file is the boundary).
- **Consequences:** **Extends the ADR-056 prerequisite amendment** — `breakdown` declares `requires: ["spec","ux","technical_design"]`, enforced BOTH frontend (a locked card + blocked workspace via the Phase-30 `deriveAvailability`) AND backend (`_fb_prereqs_ready` → start **409** + `resolve_inputs` fail-fast). **Read-only** against Appian (ADR-036/037; genesis-kb + appian-dev read allowlists; the breakdown describes work, never creates/edits/deploys; the completion chat's only write authority is the fs-sandbox HTML edit — ADR-031/045). `breakdown` becomes an `available:true` stage plugging into the ADR-056 framework via a `STAGE_DEFS` row + a `stage-registry` entry + the reused inner workspace — **no shell edits**. **Two-repo** release (genesis → genesis-workflows; keep the pin chain consistent — §7); `current_version` stays **15**. New artifacts: `breakdown.html` + `backlog.json` (per feature stage). The Lavish-annotation compatibility of the interactive HTML is the one spike-worthy risk (31-06). Reuses ADR-042/043, ADR-050 (the stage `LifecycleService` machine + m0013 audit), ADR-011/031/045, ADR-035 (file attachments), and the Phase-19 `openpyxl` dep. **Out of scope (future):** push-to-Jira from Genesis; the skill's Lucid/Sheets renderers + REST importer; story points; docx/xlsx attachments; auto-"stale" flagging; any Appian write/deploy.


## ADR-060 — Finalized Stories: the Feature Breakdown backlog becomes first-class, editable Story records (Phase 32) — **Accepted**
- **Status:** **Accepted** (2026-09-04, Phase 32; specs `specs/phase-32-finalize-stories.md` + `phase-32-finalize-stories/32-01..32-05`). **Built 32-02 (backend m0016 + StoryStore + finalize/CRUD) / 32-03 (web: Finalize control + Stories grid + routed detail/edit); reviewed 32-04; SHIPPED 32-05 — genesis v0.60.0, CI green.** Also shipped in v0.60.0: a themed `shared/ui` Select (Radix DropdownMenu) replacing native selects in the Stories + Runs filters, and run-detail graph UX fixes (live executed path + refresh, readable ×N, standard icons, legend). Realizes the **ADR-056 reserved Stories-tab slot** + the **ADR-050/25-11** reserved `Story`/`Stage` domain ("defined now; persisted by the backlog per-story work"). **Single-repo (genesis) + one migration (m0016);** genesis-core / kiro-agent-sdk / genesis-workflows / genesis-appian-parser unchanged. **Reuse:** parses the SAME canonical backlog the Jira export uses (`_parse_embedded_backlog` over the finalized `breakdown.html`), the `row_version` CAS (m0014, ADR-050/25-08), and the `LifecycleService`/m0013 activity audit.
- **Decision:** When a Feature's **Feature Breakdown** stage is marked **`completed`**, the lead performs a **one-time, irreversible** **Finalize Stories** action (a header button near the feature name, replacing the "Back to application" link; a warning confirm dialog: "…moved into the application… one-time… cannot be reverted"). Finalize parses the canonical `backlog.json` **embedded** in the finalized `breakdown.html` and **persists every epic + story/task** into two new tables — **`kb_epics`** (`feature_id`, key, title, description, workstream, position) and **`kb_stories`** (`feature_id`, `epic_id` [parent, nullable], key, title, `story_type` Story|Task, `category` core|nice-to-have, `appian_part`, description, `acceptance_criteria` [JSON], `dev_note_ref`, `questions` [JSON], `labels` [JSON], `status` [default **`design`**, forward-compat, not surfaced], `position`, `row_version`) — inside one transaction, then stamps **`kb_features.stories_finalized_at`** (the one-time marker; m0016, `current_version` 15 → **16**). Stories become **first-class, editable records** shown in the feature's (previously reserved) **Stories tab** as a formatted grid (Title · Type · Epic · Category · Appian part), each openable at a **routed detail page** (`…/features/:id/stories/:storyId`) with an Edit mode; **story-level CRUD** — add / edit any field incl. **reassigning the parent from a dropdown of the feature's epics** / delete — all `row_version`-CAS protected (→ 409 `StaleWriteError`). **Epics are created at finalize and are otherwise fixed** (no epic CRUD this phase). **Read-only feed forward:** the persisted stories are the data every upcoming phase (per-story execution — 25-11, deploy, git) builds on.
- **Context:** Phases 29–31 made the four feature stages live; after Feature Breakdown the whole backlog exists but **only inside a file** (the embedded `backlog.json`) — reviewable + Jira-exportable, but not first-class Genesis data you can browse/edit/build on. ADR-056 reserved the Stories tab slot and ADR-050/25-11 pre-defined the `Story`/`Stage` domain and named "Breakdown→Stories" as the gate. This phase is that gate: the persistence + CRUD + UI, **without** the per-story *execution* lifecycle (the next phase — this phase only lays the forward-compatible `status='design'` seam).
- **Alternatives:** (a) keep the backlog file-only + Jira as the system of record — rejected (Genesis needs first-class stories to drive the upcoming SDLC phases); (b) re-syncable finalize (re-parse `breakdown.html` on demand) — rejected (dual sources of truth + edit conflicts; after finalize the **stories** are authoritative, per the user); (c) derive "finalized" from story-count > 0 — rejected (deleting all stories would falsely re-open finalize; an explicit `stories_finalized_at` marker is unambiguous); (d) one Jira-style `kb_issues` table with a self-referential `parent_id` — rejected in favour of explicit `kb_epics`/`kb_stories` (matches the backlog's epic→story shape + the domain's Feature→Story→Stage model; the parent dropdown = the feature's epics); (e) full epic CRUD now — deferred (the user chose story-only CRUD; epics are fixed at finalize); (f) surface the per-story `status`/execution lifecycle now — deferred to the next phase (only the seam is laid); (g) a slide-over drawer for story detail — rejected in favour of a deep-linkable **routed page**.
- **Consequences:** **One-time + irreversible** — `stories_finalized_at` gates re-finalize (409) independent of story count; no un-finalize; no `breakdown.html`→stories re-sync after finalize. `kb_epics`/`kb_stories` FK→`kb_features ON DELETE CASCADE` (untracking an app cascade-deletes its stories, ADR-042 intrinsic-to-app); `story.epic_id ON DELETE SET NULL`. Finalize is atomic (one `tx()`); a missing/garbled embedded backlog → **422** (never finalize an empty set). CRUD edits are allowed after finalize (stories are the live source of truth); only finalize is one-time. **Genesis-only + m0016** (`current_version` → 16 — every `== 15` test bumps; the `clean-install` CI job migrates a fresh DB to v16). Promotes the `Story`/`Epic` domain entities to LIVE (ADR-050). Reuses ADR-042/044/056 (the Feature Workspace + reserved slot), ADR-050/25-08 (LifecycleService + `row_version`), ADR-059 (the canonical embedded backlog + `_parse_embedded_backlog`). **Out of scope (future):** per-story **execution** (the `STORY_STAGE_TRANSITIONS` workflow/stages — 25-11+); **epic** CRUD; re-sync; bulk import; push-to-Jira; multi-user/assignment; story points. Mirror in `bible/04` on Accept.


## ADR-061 — The Genesis Workbench: a per-application Kanban execution board (Phase 33) — **Accepted**
- **Status:** **Accepted** (2026-09-04, Phase 33; specs `specs/phase-33-workbench.md` + `phase-33-workbench/33-01..33-05`). **Built 33-02 (backend m0017 + BoardStore + /api/workbench) / 33-03 (web: Workbench nav + left-sidebar board sub-tabs + dnd-kit board + import + drawer); reviewed 33-04; SHIPPED 33-05 — genesis v0.61.0, CI green.** Cards drag ACROSS lanes and reorder WITHIN a lane (dnd-kit multiple-containers) persisted via a bulk `PATCH …/lanes/{lane}` reorder endpoint (sets each card's lane=status + board_position). Realizes the **ADR-056 reserved story-execution plug-point** + the **ADR-050/25-11** `Story`/`Stage` execution seam (the `kb_stories.status` column Phase 32/ADR-060 laid, "forward-compat, not surfaced" — this phase surfaces it as lanes). **Amends ADR-049** (adds a **Workbench** item to the primary nav). **Single-repo (genesis) + one migration (m0017);** genesis-core / kiro-agent-sdk / genesis-workflows / genesis-appian-parser unchanged. **New web dep:** `@dnd-kit` (drag-and-drop; accessible; ADR-027 stack extension — approved with the user).
- **Decision:** Add a top-level **Workbench** surface. The Workbench holds a **curated set of per-application Kanban boards** — an app is explicitly **added** to the Workbench (an empty Workbench prompts "Add applications" → pick from any Genesis application → each added app gets its **own board** + a sub-tab). A board is a **Jira-style Kanban** over the application's **finalized stories** (aggregated across ALL the app's features' finalized backlogs — ADR-060), with **eight fixed vertical lanes in order: To Do → Design → Design Review → Implementation → Code Review → Verification → Deployment → Done**. A card = a story/task; a card's **lane IS its `kb_stories.status`** (single source of truth — no duplicated lane column). Stories are **not** auto-loaded: an **Import** action lists the app's finalized stories **not yet on the board** (grouped by feature/epic), the user **multi-selects in batches** and adds them (landing in **To Do**, i.e. `status='to-do'`). Cards are **dragged** between lanes (`@dnd-kit`); a drop **persists the new lane** (`status`) with `row_version` optimistic CAS. A card can be **removed from the board** (clears board membership only — the story stays in its feature's Stories tab). Clicking a card opens a **drawer** (over the board) reusing the Phase-32 story-detail content. Persistence = **m0017**: `workbench_boards` (the registry of apps added to the Workbench — one board per app) + `kb_board_cards` (which stories are on a board + per-lane `board_position`); `current_version` 16 → **17**. **This phase builds the board UI/UX + import + drag-persists-lane ONLY** — the *per-lane automation* (e.g. dragging into **Design** kicks off the design workflow, **Implementation** the build, etc.) is the subject of **future phases**; this phase lays the surface + the lane model they plug into.
- **Context:** After Phase 32 (ADR-060), a feature's stories are first-class, editable records carrying a forward-compat `status` (default `design`) that is **not surfaced**. The SDLC now needs **a place where the actual work happens** — a board where a lead pulls groomed stories into an execution pipeline and moves them through design → review → build → code-review → verification → deployment → done, with each lane eventually driving an automated Genesis workflow. Jira's board (the user's reference — `~/Documents/GSS/workbench-examples`) is the mental model: lanes with count badges, cards with a type icon + key + title + epic chip + labels, a card-detail panel, an Epic/Type/Label filter toolbar, and import/backlog controls. The board is **per application** (aggregating every feature's finalized stories) because execution is planned at the application level, not per feature. `kb_stories.status` already exists (m0016) and the domain already defines a `STORY_STAGE` machine (ADR-050) — this phase reconciles that machine to the eight user-chosen lanes (adds `to-do` + `design-review`; orders **deployment after verification**) and surfaces it.
- **Alternatives:** (a) auto-populate the board with all of an app's finalized stories — rejected (the user wants **explicit, batched import**, so a board starts empty and fills intentionally); (b) a board **per feature** — rejected (execution is planned per application; the board aggregates all features' stories); (c) store the lane in `kb_board_cards` separately from `kb_stories.status` — rejected (the user fixed **lane = status**; a second lane column would drift — the card table stores only membership + position, lane is read from status); (d) overload `kb_stories` with `on_board`/`board_position` columns instead of a `kb_board_cards` table — rejected (a membership table keeps the finalized-backlog `position` untouched and cleanly models add/remove + per-lane order; and `workbench_boards` is needed anyway for the curated add-apps registry); (e) **enforce** the ADR-050 `STORY_STAGE` transition table on every drag now — deferred (the user chose **free movement** this phase — any lane → any lane — so a drag is a **lane assignment**, not yet a gated lifecycle transition; when the per-lane automation ships, moves become real gated `LifecycleService.transition` calls with m0013 audit); (f) hand-rolled HTML5 drag-and-drop — rejected for accessibility in favour of `@dnd-kit` (keyboard sensor + ARIA); (g) a routed full-page card detail on the board (like Phase 32's) — the board uses a **drawer** (keeps board context) reusing the same detail content, while the routed `StoryDetailPage` remains for the Stories tab; (h) swimlanes (group rows by epic) — deferred (flat lanes; epic shown as a card chip); (i) render the app sub-tabs as literal top tabs — an implementation detail (a scroll/overflow tab-strip or a left list; the curated set is bounded so either works).
- **Consequences:** **New primary-nav item** (Workbench) — the one sanctioned shell edit (amends ADR-049; the ADR-056 no-shell-edit invariant is about *stages* plugging in, not about adding a new top-level surface). **m0017** adds `workbench_boards` (FK→ the applications table's uuid **ON DELETE CASCADE** — untracking an app removes its board) + `kb_board_cards` (FK→`workbench_boards(id)` + `kb_stories(id)`, both **ON DELETE CASCADE**; `UNIQUE(board_id, story_id)`); `current_version` → 17 (every `== 16` test bumps; the clean-install CI job migrates a fresh DB to v17 — the §7 lesson). **`LifecycleState` gains `TO_DO`/`DESIGN_REVIEW`** and `STORY_STAGE_TRANSITIONS` is reconciled to the 8-lane order (deployment after verification) — **forward-compat only, NOT enforced on drag this phase** (the drag writes `status` directly through the store with `row_version` CAS; the ADR-050 relaxation is deliberate + temporary + recorded here). Import sets `status='to-do'`; the finalized-backlog default (`design`, m0016) is unchanged for un-imported stories. **Genesis-only + m0017** (no core/SDK/workflows change). Reuses ADR-042/044/056 (Feature Workspace + intrinsic-to-app cascade), ADR-060 (finalized stories + `StoryStore`), ADR-050/25-08 (`row_version` CAS), ADR-027 (web stack — `@dnd-kit` added), ADR-028 (`/api` routing). **Out of scope (future phases):** the actual per-lane automation (design/impl/code-review/verification/deployment workflows triggered by a move); gated transitions + m0013 audit on moves; WIP limits; swimlanes; multi-board-per-app; card reordering persistence beyond a single position; assignees/points/sprints (single-user — ADR-026). Mirror in `bible/04` on Accept.


## ADR-062 — Workbench lane automation: the Design & Design Review lanes (Phase 34) — **Accepted**

> **Amendment (2026-09-11 — `story-design-analysis` v0.3.0, single-context design).** The design workflow is simplified to author the WHOLE design in ONE agent turn instead of a per-object loop. The original `plan_objects → design_object×N → synthesize → assemble` pipeline (+ the Phase-40 `attach_healing` patch/restart tier) broke cross-object coherence — each `design_object` ran in its OWN Kiro turn seeing only that one object, so it could not keep shared fields/naming/sequencing consistent across the ticket — and it made the output code-heavy for no benefit. New flow: `resolve_inputs → load_inputs → research` (grounded, one turn: mine Spec/UX/TD + ground the current state/code via @genesis-kb/@appian-dev → an implementation plan) `→ design` (ONE turn, full context: author the entire design body) `→ render` (deterministic wrap → one well-formed, script-free, Lavish-safe `design.html`) `→ verify` (grounded critic) `→` bounded **revise** (re-author with the critic's fixes as carry-forward guidance) `→ escalate`. Output is a LEAN implementation plan (per object: what exists / what changes / what to build, with targeted code only where it matters; process models described, not dumped node-by-node) — matching the Appian team's own `jarvis-design` skill. `attach_healing` is dropped HERE (the per-object patch tier is meaningless without granular blocks) but stays in feature-breakdown / technical-design / ux. `inputs_schema` unchanged → the genesis caller + `StoryDesignFinalizer` binding are unchanged (genesis-workflows-only release, **v0.18.0**). See `progress/phase-34-design-lane-automation.md`.
- **Status:** **Accepted** (2026-09-04, Phase 34; specs `specs/phase-34-design-lane-automation.md` + `phase-34-design-lane-automation/34-01..34-07`). **✅ SHIPPED (2026-09-05) — genesis v0.62.0 + genesis-workflows v0.16.0, CI green.** 34-02 remove design-doc → 34-03 story-design-analysis workflow → 34-04 backend (m0018 + StoryStageStore + design-start + StoryDesignFinalizer) → 34-05 web (drag confirm + card run states + Design Review workspace) → 34-06 review → 34-07 release. A post-build fix switched the board to a pointer-first collision so drops land on every lane. **Amends ADR-061** (the per-lane automation ADR-061 deferred now begins, for the first two lanes). **Multi-repo:** genesis (m0018 + backend + web) + genesis-workflows (add `story-design-analysis`; remove `design-doc`); genesis-core / kiro-agent-sdk / genesis-appian-parser unchanged. Mirror in `bible/04` on Accept.
- **Decision:** Make the Workbench **Design** and **Design Review** lanes *active*. Dragging a ticket into **Design** (from ANY lane) opens a **confirm dialog** — **Yes** moves the card to Design **and** launches (or **re-runs**, updating the same design) a grounded **`story-design-analysis`** ("Appian Ticket Design") workflow for that story; **No** just moves it to Design with no run. The workflow (modeled on `technical-design-analysis`, ADR-058) takes one story + its feature's finalized **Spec + UX Implementation Analysis + Technical Design**, **gathers** the ticket-relevant research into a file, then **grounds against the live Appian app** (structure via `@genesis-kb`, the ACTUAL code via `@appian-dev`, read-only) and writes an **object-level design HTML**: one `<section>` per object to create/modify, each tagged **NEW**/**UPDATE**, with granular change detail **and the actual code** to be written/added; **process models are NOT summarized** — each node is explained with the code for any node that needs it. Every agent node wears the ADR-011 reliability trio; a **grounded verification critic** re-checks the design (bounded → escalate); assembly is a **deterministic program** node (the Phase-30 "don't emit a huge doc in one turn" lesson). While the run is active the card is **locked** (non-draggable) and shows **"Design generation running" + a run link**; on **success** the card **auto-advances to Design Review** via an **audited `LifecycleService` transition** (m0013 — the first gated + audited Workbench move, delivering ADR-061's promise); on **failure** the card shows a **light-red background + "Workflow failed" + the run link** (draggable again — re-dragging into Design re-runs, updating the same design). A card in **Design Review** opens on click into a **full-page workspace** that reuses the Spec/TD experience (a bound **`story_design`** completion chat + an **annotatable Lavish preview** of `design.html`); the user's annotations flow to the agent, which **revises the design HTML**. Persistence for the per-story artifact = **m0018** `kb_story_stages` (+ `kb_story_stage_revisions`) — the per-`(story, stage)` analog of the m0015 per-`(feature, stage)` model; `current_version` 17 → **18**. A `StoryDesignFinalizer` (the run→story-artifact bridge, mirroring the `StageFinalizer` `_BINDINGS` registry — bound-run guard + idempotency + on-read recovery) does the done→design-review finalize. The superseded Phase-15 **`design-doc`** workflow is **removed** (from the library and the running app's local library).
- **Context:** Phase 33 (ADR-061) shipped the Workbench board but left every lane inert (a drag just persists `kb_stories.status`) and explicitly deferred the per-lane automation + gated/audited moves. The SDLC now needs the first lane to *do* something: turn a groomed ticket into a concrete, code-level Appian design a developer can build from — grounded in the feature's finished Spec/UX/Technical-Design and the live app. The Technical Design stage (ADR-058) already proved the grounded, decomposed, deterministically-assembled, critic-verified design workflow; the stage artifact model (m0015) + StageFinalizer + the annotatable stage workspace (ADR-057) already proved the artifact + completion-chat + review loop. This phase applies both **at the story grain**. The old `design-doc` workflow (Phase 15, a JIRA-ticket→Markdown design doc via dual-source Atlas/Jarvis research) is a different, superseded thing and is dropped.
- **Alternatives:** (a) store the design run/artifact on `kb_stories` columns — rejected in favour of a **per-story artifact table (m0018)** mirroring m0015, so later lanes (Implementation, Code Review, …) reuse the seam; (b) reuse the per-*feature*-stage m0015 `StageStore` for the story design — rejected (design is per story, not per feature/stage); (c) auto-start the design on drop (no dialog) — rejected (the user wants an explicit confirm, and a plain move must stay possible); (d) prompt only on To Do→Design — rejected (the user wants the prompt on entry from ANY lane, so rework/re-run is always available); (e) one big design agent turn — rejected (the Phase-30 timeout/truncation lesson → decompose by object + deterministic assemble); (f) summarize process models — rejected (the user requires per-node explanation + code); (g) derive the card's running/failed state from a live observer only — rejected (derive from the durable `runs` table so it's correct even if a `run.final` was missed — the §7 orphaned-worker lesson); (h) enforce the whole `STORY_STAGE` transition table on every drag now — deferred (manual drags stay free per the ADR-061 relaxation; only the automated design→design-review completion transition is gated + audited this phase); (i) build "leave Design Review / advance to Implementation" now — deferred to a later phase (manual drag for now). 
- **Consequences:** **m0018** adds `kb_story_stages` + `kb_story_stage_revisions` (FK→`kb_stories` ON DELETE CASCADE — a story removed / its app untracked cascades; ADR-042); `current_version` → 18 (every `== 17` test bumps; the clean-install CI job migrates a fresh DB to v18 — the §7 lesson). A new `POST /api/workbench/boards/{app}/cards/{story_id}/design/start` (move + launch/re-run; **fail-fast 409** on no dev-tagged env / app not synced / workflow not installed — the design must ground against the live app, mirroring `_launch_td`). A `StoryDesignFinalizer` observer + a `story_design` `ChatModeProfile`. The board card DTO gains `design_run_id`/`design_run_status`/`design_chat_session_id`/`design_status`. The **first audited Workbench lifecycle transition** (design→design-review via `LifecycleService`, m0013). A new web route `/workbench/:appUuid/cards/:storyId` (the Design Review workspace, reusing `StageArtifactWorkspace`/`AnnotatablePreviewDialog`/`ChatThread`). **New workflow** `story-design-analysis` (read-only `@genesis-kb`+`@appian-dev`, reliability trio, grounded critic, deterministic assemble); **`design-doc` removed** (`validate_library` count −1; uninstalled from the running app). **Read-only against Appian** — the design is proposal-only (it shows code, never writes/deploys it). Reuses ADR-058 (grounded design), ADR-057 (m0015 pattern + stage workspace + StageFinalizer), ADR-050/25-08 (LifecycleService + audit + `row_version`), ADR-011 (reliability trio), ADR-036/037 (read-only grounding), ADR-043/045 (Lavish review + the reused chat), ADR-028 (`/api`). **Out of scope (future phases):** leaving Design Review + every later lane's automation (Implementation/Code Review/Verification/Deployment); an explicit approve/complete control; enforcing the full transition table on manual drags; writing/deploying into Appian; WIP limits; multi-board-per-app; multi-user. Mirror in `bible/04` on Accept.

## ADR-063 — The Genesis Collaboration Hub: local-first with selective centralization (Phase 35–38) — **Accepted**
- **Status:** **Accepted** (2026-09-05; Phase 35 — Collaboration Foundations & Identity — SHIPPED as genesis **v0.63.0**; the substrate [`SyncProvider` seam + `CollaborationService` + `LocalHubProvider` + identity/teams + m0019] is built + gated opt-in/off-by-default. Phases 36–38 remain specs-drafted — specs `specs/phase-35-collaboration-foundations.md` [+ `35-01..35-07`], `specs/phase-36-genesis-hub-appian-app.md`, `specs/phase-37-hub-wireup.md`, `specs/phase-38-collaborative-sdlc.md`). **Amends ADR-026** (local single-user). The concrete Appian backend + contract is **ADR-064**. Mirrored in `bible/04`.
- **Decision:** Genesis remains a **local-first, single-user** application (works fully offline/solo), but gains an **opt-in** capability to **selectively centralize a defined subset of data** to a shared **Collaboration Hub** so a team (PO, UX, developers) can collaborate. **Shared** entities: the application **KB** (parse once, everyone pulls — as a **gzipped blob**, content-hash-deduped, `hydrate_from_blob` into the local SQLite read cache; the local KB stays the query/grounding substrate), **features + their published stage artifacts** (Spec/UX/Technical-Design/Breakdown — the artifact **bytes + metadata**), **stories/epics**, and **board status** (lane = a story's shared `status`). **Local-only** (never centralized): **chat, runs, the installed workflow catalog, documents, and personal memory**. **Shared memory is deferred** to a later phase. The transport is a **pluggable `SyncProvider` seam** (`genesis/collab/`, mirroring ADR-051's `AgentProvider` / ADR-052's `DocumentProvider`), so the Hub can be Appian now (ADR-064) and git/a service later. The interaction model: **publish-on-complete** (an explicit push of the *finished* artifact — the chat/run stay local) + **pull** (**auto** for read-only shared views + boards; **notify-then-apply** where a local draft exists; a manual **"Sync now"**; **pull-first** for the KB — a sync pulls the latest blob when newer, a full export+parse+publish happens only on a deliberate "refresh from Appian"). Synced entities carry a stable **`sync_uuid`** (client-generated; local int PKs collide across machines); concurrency = **optimistic `row_version` CAS → `HubConflict`** + a **soft advisory "in-progress by X"** marker (heartbeat/TTL) — **no hard lock**. Cross-stage dependency staleness is **notify-only with provenance** (record the upstream artifact versions a stage consumed; badge on republish; never auto-cascade/block). **Identity is self-asserted** at onboarding (name + Appian **username** + email — **Appian is not an identity provider** here); the **Appian username is the single canonical user id** everywhere (`published_by`/`modified_by`, `LifecycleEvent.actor`, the personal-memory `owner`). **Teams are a Genesis-owned, UUID-keyed Hub entity** (the dev environment has user accounts but no group structure). **Visibility is open** for now — an `owner_username` + `team_uuid` are **stamped** on shared entities as a forward-compat scoping seam, **not enforced**. The whole layer is **opt-in / off by default** — with no Hub configured, Genesis behaves exactly as today.
- **Context:** Genesis is deliberately local single-user (ADR-026), which is right for a solo engineer but blocks a team: three developers re-parse the *same* app; a PO's finished spec is invisible to the UX designer; a board's status is private. The team wants collaboration **without** giving up local-first (offline/solo must keep working) and **without** new infrastructure to host/secure. The work is **role-sequential with an explicit hand-off** (PO→UX→Dev), so it is **single-writer-per-artifact with publish/pull**, NOT concurrent co-editing — which means **no CRDT/OT/real-time engine is needed** (the key simplification). Genesis already has the seams: the provider-interface pattern (ADR-051/052), the DB-agnostic repositories (ADR-030), `row_version` CAS (m0014), the lifecycle `actor` seam (m0013, ADR-050), memory `scope=personal|shared` (ADR-053), and publish-on-complete patterns (the finalizers, Finalize Stories). Identity is the prerequisite (attribution is impossible without it) and is currently unwired.
- **Alternatives:** (a) **A dedicated hosted Genesis-Hub service + Postgres** — rejected: it reintroduces the hosting/auth/ops burden ADR-026/046 deliberately avoided, and would need multi-tenancy built. (b) **A git-repo Hub** (artifacts are files; GitLab is the trust boundary) — a strong low-infra alternative kept in reserve (weak for live boards, no semantic search); the `SyncProvider` seam keeps it swappable. (c) **Store the KB as normalized Appian records + a remote query MCP** — rejected: multi-hop graph traversal + the grounding hot path are expensive/fragile over a remote API and duplicate the query engine; the KB stays a **blob → local materialized read cache** (fast, offline, reuses the existing local MCP). (d) **CRDT / real-time co-editing** — rejected: the workflow is role-sequential; single-writer + CAS + a soft advisory suffices. (e) **Verified identity via Appian auth** — not possible (no whoami/IdP); self-asserted, canonical username; anti-spoofing parked. (f) **Sync everything (incl. chat/runs/memory)** — rejected: chat/runs/catalog/documents/personal-memory are private/local by design; shared memory is a hard cross-user graph merge, deferred. (g) **Hard/pessimistic locks** — rejected (fragile offline + with self-asserted identity); soft advisory + CAS instead. (h) **A separate Hub environment** — rejected: teams have one dev env; it doubles as the Hub (ADR-064).
- **Consequences:** A new **opt-in** collaboration layer (`genesis/collab/`: the `SyncProvider` Protocol + `CollaborationService` + a local/mock provider [Phase 35] + the Appian provider [Phase 37] + identity/teams). A migration (**m0019**, `current_version` 18→19) adds + backfills `sync_uuid` on the six synced tables, `row_version` where missing, and owner/team/provenance columns + local `collab_*` tables — additive + harmless for solo installs. Identity is finally threaded (`LifecycleEvent.actor` populated; `memory_owner_username` derives from the canonical username). Publish/pull is per-entity (KB blob → features/artifacts → stories → boards), reusing a shared sync-UX foundation (poll / Sync-now / notify-then-apply / offline). It is the **multi-user trigger** ADR-030 anticipated — but realized as a **pluggable publish/pull Hub**, NOT a Postgres swap of `genesis.db`. **The Hub is the first shared, networked, credential-bearing surface** — publishing is an explicit, authenticated, user-initiated act (a shared service account, ADR-064), consistent with "no silent outbound". **Amends ADR-026** (multi-user becomes possible — opt-in, selective, for the shared subset only; local-first + offline/solo operation stand). Relates to ADR-048 (per-env creds → the Hub key), ADR-047 (scheduler pulls), ADR-053 (shared memory becomes literally shared in a later phase). Delivered across Phases 35 (foundations), 36 (the Appian Hub app; ADR-064), 37 (wire-up: provider + KB + identity/teams), 38 (features/stories/boards + the collaboration UX + adoption). Mirror in `bible/04` on Accept.

## ADR-064 — The Genesis Hub Appian application + its frozen API contract (Phase 36) — **Accepted**
- **Status:** **Accepted** (2026-09-05, Phase 36 — **COMPLETE + live-validated** in the team dev env; spec `specs/phase-36-genesis-hub-appian-app.md` [+ `36-01..36-08`]; as-built `progress/phase-36-genesis-hub-appian-app.md`; contract `specs/phase-36-genesis-hub-appian-app/contract/`). The isolated `Genesis Hub` app is fully built: 11 UUID-keyed record types + 13 relationships + record-level security + groups/folders/9 constants, **11 Web APIs + 12 helper expression rules + the base64 blob store**, and a **frozen contract + fixtures** checked into genesis. **Validated end-to-end against the live env** with the service-account API key (all 7 write kinds w/ base-version CAS→409, unicode-lossless, story-item explode/reassemble, /changes, activity set/list/clear, blob put/dedup/versions/GET-lossless). The app **is** the Hub (ADR-063: the dev env, no separate Hub env); an installable package export is an optional portability step. **Realizes ADR-063.** Built by a separate write-capable Dev-MCP agent — NOT by Genesis (read-only against Appian per ADR-036/037). Mirror in `bible/04`.
- **Decision:** Realize the Collaboration Hub (ADR-063) as a **dedicated "Genesis Hub" Appian application** hosted in the team's **existing Appian dev environment** (the same environment Genesis already syncs — **no separate Hub environment**), exposing **Web APIs** that Genesis calls to publish/pull. The Hub holds **UUID-keyed record types** (`sync_uuid` + a per-record `version` + payload-carried attribution `created_by`/`modified_by` + `owner_username`/`team_uuid`): **Team, Membership, Feature, Epic, Story, BoardState, StageArtifact-metadata**; and **document-management record types** for the versioned **blobs** — **KbBlob** (one gzipped code-free KB parse-result per app) and **ArtifactBlob** (published stage-artifact HTML) — with **version history**, **content-hash dedup**, and **keep-last-N** retention. The **Web-API surface**: `PUT /records/{kind}/{sync_uuid}` (upsert with **base-version CAS → 409 `{current_version}`**, mapping to ADR-063's `HubConflict`), `GET /records/{kind}?since=` (paginated) / `GET /records/{kind}/{sync_uuid}`, `POST/GET /blobs/{kind}/{key}` (+ `/versions`; dedup 200-unchanged/201-new), `GET /changes?cursor=` (the change manifest + a `contract_version`), advisory-marker set/clear/list (TTL), and teams/memberships. A **shared service account** performs all writes (its API key rides the ADR-048 per-env credential store); **attribution is taken from the request payload**, not Appian auth. **Visibility is open** (owner/`team_uuid` tags, unenforced). The app is **isolated** from the customer applications Genesis analyzes — a distinct Appian application that Genesis never adds as a tracked subject app and never parses. A **frozen JSON contract + shared fixtures** are checked into the **genesis** repo as the authoritative interface Phase 37's `AppianHubProvider` implements + tests against (one source of truth for both sides).
- **Context:** ADR-063 needs a real shared backend. The team already has an Appian dev environment (no new infra), the team are Appian engineers who can build + maintain it, and Appian provides **document versioning** + **Smart Search** (AI semantic search across the data fabric incl. attached documents) natively + custom **Web APIs** (JSON, API-key auth). Because Genesis is **read-only against Appian** (ADR-036/037), Genesis must **not** build or write this app — a **separate agent with a write-capable Dev MCP** builds it from the Phase-36 spec, delivering an installable package. The load-bearing artifact is therefore the **frozen contract** so the Appian build and the Genesis client can proceed against one stable interface.
- **Alternatives:** (a) **Normalized KB records + a remote query MCP** — rejected (ADR-063 alt (c)): the KB is stored/served as a **blob**, queried locally. (b) **Genesis builds the Appian app** — rejected (violates the read-only-against-Appian posture; and the team already has a write-capable Dev-MCP agent for it). (c) **A generic external DB/service instead of Appian** — deferred behind the `SyncProvider` seam (git/service remain swappable). (d) **Per-user Appian credentials for authenticated attribution** — recommended but rejected for v1 (Appian isn't an IdP here; a shared service account writes, attribution is payload-carried; hardening parked). (e) **Enforced per-team record security now** — deferred (open visibility; tags stamped for later).
- **Consequences:** A new Appian application (record types + document/blob store + Web APIs + service account + an installable, versioned package + an install/upgrade runbook) built + maintained **outside** the genesis repo (Appian-side; **no genesis version tag** from Phase 36). The genesis-repo footprint is the **checked-in frozen contract + fixtures** (reused by Phase 37's provider tests) + optionally a typed client stub. The contract is **versioned** (`contract_version`) so Genesis can detect a mismatch. The Hub app must stay **isolated** (Phase 37 enforces the subject-app exclusion by `app_uuid`). Blob sizes are modest (a large app's KB ≈ 1–2 MB gzipped — well within document limits). **Synced-record-type text tiers honored** (Text 255 / Long Text 4000 / **Extra Long Text 64000**, max 3 per record type): `description`/AC fit without truncation; a story's variable-length lists (`acceptance_criteria`/`questions`/`labels`) are **normalized into a `GH Story Item` child record type** (exploded/reassembled by the Web API) — the **Genesis-side local model stays JSON, unchanged**; the sync is **lossless** (a round-trip fidelity gate). Reuses ADR-048 (the service-account credential store). Realizes ADR-063. Mirror in `bible/04` on Accept.

## ADR-065 — Two-toggle collaboration enablement (Settings switch gated on Hub availability) (post-Phase-38) — **Accepted**
- **Decision:** Collaboration is active only when BOTH gates are on: the **env master** `GENESIS_COLLAB_ENABLED` (builds the provider at boot) AND a persisted **Settings-level toggle** (`ui_enabled`, in Settings → Collaboration). The Settings toggle defaults OFF and can only be turned ON when the Genesis Hub app is reachable in the dev env (a `GET /collab/availability` guard; `PUT /collab/config` → 409 otherwise). The Hub URL is derived from the dev-tagged env (`{dev.url}/suite/webapi`); the Hub app UUID is a one-time static field. Persisted at `~/.genesis/collab.json` (no migration).
...
## ADR-066 — Run inspection & node observability: capture what the agent was given & produced, per iteration (Phase 39) — **Accepted** (SHIPPED genesis v0.70.0)
- **Decision:** Rebuild the run-detail node experience into **single-click summary (a new node `description` + basic per-execution metrics) + a large adaptive turn-explorer modal**. genesis-core **additively captures the rendered prompt (as a blackboard-file pointer), attached artifacts, a generic `_iteration` grouping label, and `attempt`** on the turn events (no schema change — event log + blackboard, honoring ADR-010/018 "no bulk inline"). genesis adds a **per-iteration fold** (`fold_iterations`) + endpoint and a **`GraphNode.description`** passthrough. The modal's left rail is an **adaptive Pass › Item › Attempt › Turn tree** (depth = reality); the right pane shows a turn's full prompt / artifacts / conversation / output / metrics.
- **Context:** Today clicking a node dumps everything yet hides what matters — the prompt sent, the artifacts provided, and per-iteration output/credits/timing are not captured (`kiro_node` never emits the prompt) and everything is folded per-node. This is the platform's core surface.
- **Alternatives:** (a) inline the prompt in `run_events` — rejected (bulk in the event log, ADR-010/018); (b) a rigid fixed Pass/Item/Attempt nesting — rejected (mostly-empty levels; the adaptive tree matches reality, the LangSmith/Braintrust convention); (c) backfill old runs — rejected (never captured; new-runs-only).
- **Consequences:** new-runs-only; a `/dev` mockup gates the build; multi-repo additive release (genesis-core → genesis → genesis-workflows for node descriptions + `_iteration` stamps); `CORE_MAJOR` unchanged; no genesis.db migration. See `specs/phase-39-run-inspection-observability.md`.

## ADR-067 — Grounded artifact verification & cost-aware self-healing: the second reliability tier (Phase 40) — **Accepted** (SHIPPED genesis-core v0.9.7 + genesis-workflows v0.17.0)
- **Decision:** Add a **second reliability tier beside the ADR-011 trio** as a first-class genesis-core construct **`attach_healing`**: `verify (grounded critic) → heal (agent judges blast radius) → {patch flagged items in place → re-verify | one guided full restart} → escalate (HITL gate)`, bounded by `META.healing {max_heal:1, max_restart:1}`. `verify` + `heal` are agent nodes that each still wear the trio. New core contracts: **`VerificationReport`** (item-keyed, sourced fixes) + **`HealDecision`** (patch|restart + carry-forward guidance). All four analysis workflows adopt it, replacing the wasteful "revise re-runs everything."
- **Context:** On a `verify` failure, feature-breakdown re-breaks ALL epics from scratch; technical-design/story-design re-run `synthesize` which cannot fix the frozen per-block content; ux re-synthesizes wholesale. Credits (ADR-032) are a first-class cost and this is the largest avoidable waste.
- **Alternatives:** (a) per-workflow bespoke fixes — rejected (this is a core concept; must be solid + uniform); (b) overload the ADR-011 trio — rejected (different scope: whole-artifact vs single-turn); (c) unbounded reflection — rejected (credit blow-up; bounded 1-heal + 1-restart → human).
- **Consequences:** complements ADR-011; genesis-core additive (`CORE_MAJOR` unchanged) + a reliability-lint extension; genesis-workflows adopts in all four workflows (structured verify + a heal agent each). Land feature-breakdown first. See `specs/phase-40-self-healing-reliability.md`.


## ADR-038/040 amendment — Phase 41 (Dev MCP 26.6.90 + SAIL CLI, genesis v0.72.1 + genesis-workflows v0.19.0)
- **Dev MCP 26.6.90 (ADR-038):** the bundle's default auth flipped to **browser**, so `appian-dev` pins a static `LCP_AUTH_METHOD=basic` (browser/SSO auth deferred → `specs/backlog/devmcp-browser-sso-auth.md`); **`LCP_API_PATH` is dropped** (the bundle defaults it) and removed end-to-end from the env registry/API/web (back-compat load kept). Allowlist +5 read tools (72 total); read-only posture unchanged (`LCP_TOOL_MODE=readonly` not implemented in 26.6.90). `truststore` auto-trusts corporate CAs.
- **SAIL CLI (ADR-040):** a second managed-native CLI, **installed from the Dev MCP bundle** in the same `appian-dev` install action (host-platform binary; macOS quarantine cleared; version from `BUILD-INFO.txt`). Fully wired to be callable (`cli-registry` `sail` + `SailClient` + `ctx.extras['sail']`; basic auth from the dev-tagged env via the ADR-048 seam; isolated `SAIL_HOME`) with **no consumer yet**. See `progress/phase-41-devmcp-upgrade-and-sail-cli.md`.


## ADR-068 — Write-capable Appian implementation lane: Genesis implements object-level code (Phase 42) — **Accepted**
- **Status:** **Accepted** — SHIPPED 2026-09-24 (Phase 42; genesis-core v0.9.8 + genesis v0.73.0 + genesis-workflows v0.20.0, CI green; the Appian Hub Object-Wiki record types built + live-validated). Specs `specs/phase-42-code-implementation.md` + `phase-42-code-implementation/42-01..42-09`. Genesis's **first environment-mutating capability.** **Amends ADR-021** (pre_mutation), **ADR-036/037** (read-only against Appian), **ADR-038** (read-only Dev-MCP allowlist), and **supersedes ADR-062's proposal-only clause** for the Implementation lane. Multi-repo: genesis + genesis-workflows (+ genesis-core only if the ADR-069 skill primitive lands in core). Mirror in `bible/04` on Accept.
- **Decision:** Make the Workbench **Implementation** lane *active* and write-capable. Dragging a story **Design Review → Implementation** (from any lane) opens a **confirm dialog** whose **Yes is the single, blanket approval** for every object change in that story's completed design; it moves the card + launches an unattended **`story-implementation` ("Ticket Implementation")** workflow. The workflow: **(1) reverify** the story's design against the feature's Technical Design (a mismatch **escalates** to a HITL gate: proceed-anyway / send-back-to-Design-Review); **(2) prepare a local, temporary rollback document** — the current code + object **version** (`listObjectVersions` + the `get*` tools) of every object the design will *update*, new objects noted "delete to roll back", **no indirect dependents**; **(3) implement** — a **single agent** (armed with the Appian skills, ADR-069, and Dev-MCP **write** tools) iterates object-by-object over the design and applies the changes to the **dev-tagged environment only** (ADR-048 creds); **(4) verify** only the objects this run touched (`validateDesignObject`/`validateExpression`/`test*`); **(5) heal** on failure (`attach_healing`, ADR-067 — write fixes in place → re-verify | one guided restart → escalate); **(6) update the Object Wiki** (ADR-070) and move the card to **Code Review** (an audited `LifecycleService` `submit-review` transition, m0013). **No object is ever deleted** — a design that removes/retires an object instead **marks the object's description "deprecated by Genesis" via update**; the write allowlist **excludes every `delete*` tool**. Write capability is enabled by a **dedicated, write-scoped managed-native MCP entry `appian-dev-write`** (all `create*`/`update*` for all object types) injected **only** by the implement/heal nodes — the shared `appian-dev` stays literally read-only for chat / KB grounding / design (`node.tools ∩ server.allowlist`, ADR-029). A **read-only review surface** (implementation report + rollback doc + basic info) is reachable from the card in both Implementation and Code Review.
- **Context:** Everything through Phase 41 reads the environment and writes documents; the read-only posture (ADR-021/036/037/038, ADR-062) was an explicit placeholder "until a stable Dev MCP makes object-level writes safe." Phase 41 adopted **Dev MCP 26.6.90**, which exposes **full object CRUD** (157 tools tagged `meta.action_type=GET|CREATE|UPDATE|DELETE`) with **no server-side readonly gate** (`LCP_TOOL_MODE=readonly` documented but unimplemented) — so writes are enabled purely by widening Genesis's allowlist. The Phase-34 Design automation already proved the drag-confirm → bound run → finalizer → audited lane transition pattern; the verify/rollback Dev-MCP tools are already allowlisted; `attach_healing` (ADR-067) already models verify→heal. This is the SDLC's centre and the natural next lane.
- **Alternatives:** (a) **A per-object `pre_mutation` gate** (strict ADR-021) — rejected by the user: the human already approved by dragging into Implementation + reviewing the design; a per-object gate would make an unattended workflow interactive. The safety net is the reverify step + the rollback doc + verify/heal + dev-env-only. (b) **Widen the shared `appian-dev` allowlist** and rely on per-node `tools=` — rejected in favour of a **dedicated `appian-dev-write` entry** (defense in depth + clean audit; the read entry stays provably read-only). (c) **Allow deletes** (the design may retire an object) — rejected: autopilot deletes are too dangerous; **deprecate-in-description** instead (the `appian` skill's own delete workflow demands human confirmation anyway). (d) **A per-object implement loop** (program-driven) — rejected: the user wants a **single agent** that iterates internally with the skill's validate-then-iterate loop (accepting that a very large ticket leans on that loop + healing rather than a program map). (e) **Write to a separate build/impl environment** — n/a: Genesis has one dev-tagged env; writes target it. (f) **Auto KB re-sync after implementing** — deferred (backlog `kb-resync-after-implementation.md`; delta sync already exists, only the auto-trigger is deferred).
- **Consequences:** genesis + genesis-workflows (+ Appian Hub for the wiki, ADR-070). New: the `appian-dev-write` registry entry; the `story-implementation` workflow (reliability trio per agent; `attach_healing`; the escalation gate; write MCP only on implement/heal; **no `delete*` anywhere**); a Workbench implementation-start endpoint + a `StoryImplementationFinalizer` (done → code-review, audited; on-read recovery; **no completion chat** — unattended); a read-only review surface; the drag-confirm. **The KB stays read-only + code-free (ADR-037)** — the environment changes; the local KB reflects them only after a (manual/scheduled) `sync-application mode=delta` "Refresh" (auto-trigger deferred). Writes are bounded to the **dev-tagged env** (ADR-048) and to **create/update** (never delete); concurrency leans on the Dev MCP's own version stale-write protection. Preserves ADR-001 (LangGraph owns the implementation control flow; the agent doesn't orchestrate), ADR-011/067 (reliability + healing), ADR-050 (`LifecycleService` + m0013 audit), ADR-061/062 (the Workbench lane-automation pattern). **This is the largest posture change in the program — recorded, deliberate, and gated behind an explicit human drag on the dev environment only.** Mirror in `bible/04` on Accept.

## ADR-069 — Workflow-node skill provisioning: inject a Kiro skill into a workflow agent session (Phase 42) — **Accepted**
- **Status:** **Accepted** — SHIPPED 2026-09-24 (Phase 42, sub-phase 42-03; genesis-core v0.9.8, CORE_MAJOR unchanged). Spec `specs/phase-42-code-implementation/42-03-workflow-node-skill-provisioning.md`. **Extends ADR-034** (skills were chat-only). genesis (± genesis-core if the primitive lands in core). Mirror in `bible/04` on Accept.
- **Decision:** Provide a shippable mechanism to **inject a Kiro skill into a workflow agent node's session**. Because `kiro_node` runs with `cwd = the run blackboard` and kiro-cli auto-discovers skills from `<cwd>/.kiro/skills/`, Genesis **provisions the named skill(s) into the run blackboard's `.kiro/skills/<id>/`** at launch (the worker copies them from the managed skills workspace / the genesis-workflows skills library before the agent turn) — so a workflow can declare "this node runs with skill X" and get it deterministically, rather than relying on the host's global `~/.kiro/skills`. For Phase 42, the **`appian`** skill (mandatory Appian-MCP domain knowledge) is provisioned for the implement/verify/heal nodes and **`appian-object-generation`** additionally for the implement node.
- **Context:** ADR-034 established Skills as filesystem-provisioned into a managed **chat** workspace (`~/.genesis/.kiro/skills/`), explicitly deferring "workflow-node skills." The Implementation workflow's implement agent needs the `appian` domain rules (naming, relationship rules, dependency order, UUID handling, deletion policy) + the `appian-object-generation` create/update-via-lcp-mcp-server skill to write correct objects. Relying on the host's global `~/.kiro/skills/appian` is not shippable (a fresh Genesis install has no guarantee it's there); the skill must be provisioned by Genesis into the run.
- **Alternatives:** (a) **Rely on the global `~/.kiro/skills`** — rejected (not shippable/deterministic; a clean install wouldn't have it). (b) **Inline the skill content into the node prompt** — rejected (bloats the prompt every turn; the skill's reference files are large and loaded on demand — the whole point of the SKILL.md mechanism). (c) **Set `KIRO_HOME` to relocate skills** — rejected (the §7 lesson: it also relocates the user's agents/sessions/auth). (d) **A wire param on `session/new`** — rejected (skills are filesystem-discovered, not an ACP wire param — the Phase-14 spike finding).
- **Consequences:** A small additive mechanism (the worker provisions declared skills into the run's `.kiro/skills/` before an agent turn; a workflow node declares its skills). If a genesis-core primitive is cleanest (`kiro_node(skills=[...])`), it is additive, `CORE_MAJOR` unchanged. Preserves ADR-001/011 (the skill is instruction context for a narrow agent node, not orchestration) and the ADR-034 safety posture. Reusable by any future write/author workflow. Mirror in `bible/04` on Accept.

## ADR-070 — The Object Wiki: a local-first, Hub-shared, append-only per-object change ledger (Phase 42) — **Accepted**
- **Status:** **Accepted** — SHIPPED 2026-09-24 (Phase 42, sub-phase 42-06; genesis v0.73.0 m0021 + the Appian Hub `GH Object Wiki Page`/`Entry` record types, live-validated). Spec `specs/phase-42-code-implementation/42-06-object-wiki.md`. Reuses **ADR-063/065** (the collaboration substrate) + **additive to ADR-064** (a new Genesis Hub record type, built by the separate write-capable Appian agent). genesis (m0021) + the Appian Hub. Mirror in `bible/04` on Accept.
- **Decision:** Genesis maintains an **Object Wiki** — a durable, per-object record of *what changed and why*, accumulated as objects are implemented across tickets. Model = **two tables** (the MediaWiki page+revision × data-dictionary business/technical split × append-only audit-ledger synthesis): **`wiki_object_pages`** (one row per Appian object, keyed `(app_uuid, object_uuid)`, with a mutable `current_summary`/`latest_entry_id`) + **`wiki_object_entries`** (**append-only**, one immutable row per change: `change_kind` ∈ {created, updated, deprecated}, a **business_description** [what changed for the business], a **technical_description** [what changed in the object — e.g. "added a rule input; new branch for X"], the originating **story** [`story_key`/`story_sync_uuid`] + an `is_bugfix`/`change_reason` flag, the **author**, the **timestamp**, and the resulting **object_version**). **No object code is ever stored.** The Implementation workflow's `author_wiki` step (a single wiki-authoring agent, ADR-068 step 6) writes one entry per object it touched. The wiki is **local-first** (the local tables are the primary write target and always succeed), **published to the Genesis Hub when available**, and **kept in sync** (teammates' entries pulled) — the same publish/pull pattern as features/stories/boards (ADR-063), so the Hub aggregates object-level change history from all users when collaboration is on, without being a hard dependency.
- **Context:** Once Genesis implements objects across many tickets, the single most valuable missing artifact is a per-object history: "what is this object, why does it exist, what changed for it and in which ticket, business- and technically." Appian itself doesn't carry that narrative (it has versions, not intent). The wiki is that narrative — a data-dictionary/changelog hybrid — and it must be **shared** (an object is touched by many users across tickets), which the collaboration Hub already provides. Research converged on the canonical shape: a page per subject + append-only revisions (MediaWiki), a technical-vs-business description split (data dictionary vs business glossary), and an append-only ledger (one row per event).
- **Alternatives:** (a) **Hub-authoritative, always-read-from-Hub** (an earlier user framing) — reconsidered and rejected in favour of the **local-first** collab pattern already used everywhere (write local, publish + pull when available; no hard Hub dependency — implementation must run whether or not collaboration is enabled). (b) **Store the object code in the wiki** — rejected (ADR-037 code-free; the code lives in Appian + the rollback doc; the wiki stores intent, not code). (c) **A single denormalized table** (page fields repeated on every entry) — rejected in favour of page + entries (clean append-only history + one mutable page pointer; collab-friendly since entries publish once and never conflict). (d) **Reuse the KB `kb_objects` rows** — rejected (the KB is code-free structure rebuilt per sync; the wiki is human/agent-authored intent that must persist across syncs and be shared — a distinct concern). (e) **Agents read the wiki as grounding/memory now** — deferred (backlog `wiki-as-agent-memory.md`; powerful, but a separate capability).
- **Consequences:** **m0021** (`current_version` 20→21) adds `wiki_object_pages` + `wiki_object_entries` (+ `sync_uuid`/`row_version`/provenance for collab). New: `genesis/kb/object_wiki.py::ObjectWikiStore`; two `collab/service.py` `_BINDINGS` entries (`wiki_page` [parent = global `app_uuid`] + `wiki_entry` [parent = `wiki_page`]) + publish/pull + autopull integration; a **new Genesis Hub record type** (`GH Object Wiki Page` + `GH Object Wiki Entry`, or a normalized pair) **built by the separate write-capable Appian agent** — additive to the frozen ADR-064 contract (round-trip fidelity gate, like the v0.69.0 `on_board` addition). Append-only entries make sync conflict-free (each entry publishes once). Preserves ADR-063 (opt-in/local-first), ADR-037 (code-free), ADR-030 (SQLite). Pairs with ADR-068 (the workflow that authors it). Mirror in `bible/04` on Accept.

## ADR-071 — The unified Story Workspace: one consistent, consolidated story view (Phase 43) — **Accepted**
- **Status:** **Accepted** — SHIPPED 2026-09-24 (genesis v0.74.0; CI green; genesis-only, web + a thin read endpoint; **no migration**). Specs `specs/phase-43-unified-story-workspace.md` + `phase-43-unified-story-workspace/43-01..43-05`. **Amends the story-view *presentation*** of ADR-060 (StoryDetailPage), ADR-061 (the board card drawer), ADR-062 (the Design-Review workspace routing), ADR-068 (the implementation review surface) — their data/artifacts are unchanged; only how a story is presented is unified. Mirror in `bible/04` on Accept.
- **Decision:** Replace the three disjoint story surfaces (the feature `StoryDetailPage`; the board `BoardCardDrawer`; the lane-routed `StoryCardPage` → `StoryDesignWorkspace` | `ImplementationReviewWorkspace`) with **one canonical Story Workspace page** at **`/applications/:appUuid/stories/:storyId`**, reached from **every** entry point (the feature Stories tab, every board lane, legacy-route redirects). The page = a persistent header + **tabs**: **Overview** (always — details/AC/questions + inline Edit/Delete), **Design** (when a design stage exists — the design doc read-only + an **"Edit / chat"** button that opens the existing design chat **in a new browser tab**, enabled only in Design/Design-Review, read-only afterward), **Implementation** (when an implementation stage exists — the implementation report + objects-modified table + rollback document, read-only). Tabs render **only** when their artifacts exist; the page **lands on the stage-relevant tab**. A **single documents-viewer chrome** gives every document tab the same chrome + a **type label** ("Design document" / "Implementation report" / "Objects modified" / "Rollback document") and dispatches to the appropriate existing renderer (themed HTML / sandboxed iframe / `MarkdownView` / table). A **consolidated, board-independent read endpoint** feeds the page (`StoryStageStore.get_for_story` directly), so a story reachable from the feature side but **not on a board** still resolves (Overview only) and documents "light up" once it has been through the board.
- **Context:** Today the screen a user gets when opening a story depends on where they clicked and the card's lane — three surfaces, each showing only a slice (core details XOR the design chat XOR the implementation docs), decided by `BoardPage.openCardView` lane branching. There is no single place to see "everything about this story", and the look-and-feel changes as the card advances. The artifacts already exist (design.html; implementation report/rollback/objects) and reusable renderers already exist (`AnnotatablePreviewDialog`, `MarkdownView`, the sandboxed report iframe, `ChatThread`); the gap is purely presentation + a board-independent read.
- **Alternatives:** (a) **Keep two routes** (feature-side + board-side) — rejected: the user wants one canonical route from anywhere. (b) **Keep the To-Do drawer for quick peek** — rejected: fully-consistent means always the same page. (c) **Inline the design chat inside the Design tab** — rejected: the user chose a new-browser-tab hand-off to the existing chat workspace (keeps the chat surface unchanged). (d) **A single scroll of all documents** — rejected in favour of tabs (avoid dumping; the user asked for meaningful separation). (e) **Board-scoped reads only** — rejected: the feature side must resolve a story with no board card, so the read is board-independent.
- **Consequences:** genesis-only. New: a consolidated read endpoint (`…/stories/{id}/workspace`) + relaxing the design-artifact/implementation-report reads to story-scoped (dropping the Phase-42 Q16 lane gate — redundant since a tab renders only when its artifact exists); a `StoryWorkspacePage` (header + tabs) + a thin `StoryDocument` chrome (the one net-new component) + `defaultStoryTab` (a pure, tested helper); the board `openCardView` collapses to a single navigate, the `BoardCardDrawer` is deleted (Remove-from-board relocates to the page header), and `StoryDetailPage`/`StoryCardPage` become redirects (the design chat workspace stays as the new-tab target). **No new write surface, no new auth, no migration.** Preserves ADR-055 (design language), ADR-043 (Lavish read view), ADR-028 (`/api` + typed client), and the underlying stage data model (ADR-060/062/068 unchanged). Mirror in `bible/04` on Accept.

## ADR-072 — Global human-readable story identifier (GNS_000001) (Phase 44) — **Accepted**

- **Status:** **Accepted** — SHIPPED 2026-09-25 (genesis v0.75.0; CI green; genesis-only; adds migration `m0022`, DB → v22). Spec: `specs/phase-44-fixes-bundle.md` (fix 3). Mirror in `bible/04` on Accept.
- **Context.** Finalized stories currently take `kb_stories.key` from the breakdown's backlog id (usually blank), so a story has no meaningful, shareable identifier. Users need a stable, human-readable id that is unique across the whole install and visible to anyone who pulls the story from the Hub.
- **Decision.** Assign each finalized (and manually-added) story a **globally unique, monotonic, human-readable** identifier **`GNS_<6-digit>`** (e.g. `GNS_000001`), allocated at write time from a durable counter table **`kb_sequences`** (m0022; `current_version` 21 → 22) inside the finalize/create transaction (serialized writes), and stored in the existing `kb_stories.key` column. The id is **assigned locally** and **rides the existing story sync payload** to the Genesis Hub — `key` is a shared, non-`_LOCAL_ONLY` column already carried by the `story` binding, and the `GH Story` record already has a key field, so there is **no Hub/contract change**. **Epics keep** their workstream-derived key (the scheme applies to stories only). **Existing** stories are **not** backfilled (they keep their current key / blank).
- **Scope.** A **single global** sequence across all stories install-wide (chosen over per-feature/per-app for guaranteed uniqueness + cross-story meaning).
- **Alternatives rejected.** Deriving from `MAX(key)` (gaps + race-prone); per-feature/per-app scoping (not globally unique); a Hub-side sequence (would require a Hub change — explicitly out).
- **Consequences.** A new `kb_sequences` counter table (the only Phase-44 schema change); allocation happens inside the existing write transaction; the id displays on the stories grid, the story detail/workspace header, and the board card. Mirror in `reference/decision-log.md` (here) + `bible/04-adrs-and-constraints.md` on Accept.
