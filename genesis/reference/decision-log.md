...
## ADR-064 — The Genesis Hub Appian application + its frozen API contract (Phase 36) — **Proposed**
- **Status:** **Proposed** (2026-09-05, Phase 36; spec `specs/phase-36-genesis-hub-appian-app.md` [+ `36-01..36-07`]). **Realizes ADR-063.** **Built by a separate write-capable Dev-MCP agent — NOT by Genesis** (which stays read-only against Appian per ADR-036/037). Mirror in `bible/04` on Accept.
