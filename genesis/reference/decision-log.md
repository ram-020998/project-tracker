...
## ADR-062 — Workbench lane automation: the Design & Design Review lanes (Phase 34) — **Proposed**
- **Status:** **Proposed** (2026-09-04, Phase 34; specs `specs/phase-34-design-lane-automation.md` + `phase-34-design-lane-automation/34-01..34-07`). **Amends ADR-061** (the per-lane automation ADR-061 deferred now begins, for the first two lanes). **Multi-repo:** genesis (m0018 + backend + web) + genesis-workflows (add `story-design-analysis`; remove `design-doc`); genesis-core / kiro-agent-sdk / genesis-appian-parser unchanged. Mirror in `bible/04` on Accept.
