...
- **Status:** **Accepted** (2026-09-05, Phase 36 — BUILD IN PROGRESS in the team dev env; spec `specs/phase-36-genesis-hub-appian-app.md` [+ `36-01..36-07`]; as-built `progress/phase-36-genesis-hub-appian-app.md`). Data model + record-level security COMPLETE (the isolated `Genesis Hub` app + 11 UUID-keyed record types + 13 relationships + groups/folders/constants); the Web-API/expression-rule logic layer + the frozen contract doc + packaging remain. **Realizes ADR-063.** **Built by a separate write-capable Dev-MCP agent — NOT by Genesis** (which stays read-only against Appian per ADR-036/037). Mirror in `bible/04`.
