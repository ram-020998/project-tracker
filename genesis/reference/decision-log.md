...
## ADR-055 — UI/UX design language + light-first theming (Phase 27) — **Proposed**
- **Status:** **Proposed** — to be decided in Phase 27 sub-phase **27-01** *with the user*; recorded here for traceability.
- **Decision (proposed):** Modernize the entire web UI to a **light-first** (light is the hard default; dark stays a first-class, persisted, toggleable theme) **modern, future-looking** design language using **MUI / Material Design 3 as the visual/UX north star**. **Evolve the existing token + Tailwind + shadcn-style primitive system to a MUI-inspired language** rather than adopting `@mui/material` as a dependency — *pending the user's call in 27-01.*
