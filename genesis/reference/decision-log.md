...
## ADR-059 — The Feature Breakdown stage: grounded, workstream-decomposed, Jira-ready backlog (Phase 31) — **Proposed**
- **Status:** **Proposed** (2026-09-04, Phase 31; specs `specs/phase-31-feature-breakdown-stage.md` + `phase-31-feature-breakdown-stage/31-01..31-07`; findings `31-01-findings.md`). Extends the ADR-056 prerequisite amendment (introduced by ADR-058) to a **three-way** prerequisite. **Component reuse:** renders through the SAME generalized Phase-29/30 surface — `StageArtifactWorkspace`, `AnnotatablePreviewDialog`, `StageBuilderPage`, the reused `ChatThread`/annotation bridge, the m0015 `StageStore`, and the **generalized `StageFinalizer`** (one more workflow→stage binding). **No migration, no genesis-core/SDK change.** Methodology authority = the `spec-to-backlog` Kiro skill (`gitlab…/allison.olson/spec-to-backlog`) — **analysis core + `Backlog_JSON` + Jira-column contract only**; format authority = the four GSS example breakdowns (`~/Documents/GSS/breakdown-examples`; `31-01-findings.md`).
