...
## ADR-068 — Write-capable Appian implementation lane: Genesis implements object-level code (Phase 42) — **Proposed**
- **Status:** **Proposed** (2026-09-23, Phase 42; specs `specs/phase-42-code-implementation.md` + `phase-42-code-implementation/42-01..42-09`). Genesis's **first environment-mutating capability.** **Amends ADR-021** (pre_mutation), **ADR-036/037** (read-only against Appian), **ADR-038** (read-only Dev-MCP allowlist), and **supersedes ADR-062's proposal-only clause** for the Implementation lane. Multi-repo: genesis + genesis-workflows (+ genesis-core only if the ADR-069 skill primitive lands in core). Mirror in `bible/04` on Accept.
...
## ADR-069 — Workflow-node skill provisioning: inject a Kiro skill into a workflow agent session (Phase 42) — **Proposed**
- **Status:** **Proposed** (2026-09-23, Phase 42, sub-phase 42-03; spec `specs/phase-42-code-implementation/42-03-workflow-node-skill-provisioning.md`). **Extends ADR-034** (skills were chat-only). genesis (± genesis-core if the primitive lands in core). Mirror in `bible/04` on Accept.
...
## ADR-070 — The Object Wiki: a local-first, Hub-shared, append-only per-object change ledger (Phase 42) — **Proposed**
- **Status:** **Proposed** (2026-09-23, Phase 42, sub-phase 42-06; spec `specs/phase-42-code-implementation/42-06-object-wiki.md`). Reuses **ADR-063/065** (the collaboration substrate) + **additive to ADR-064** (a new Genesis Hub record type, built by the separate write-capable Appian agent). genesis (m0021) + the Appian Hub. Mirror in `bible/04` on Accept.
