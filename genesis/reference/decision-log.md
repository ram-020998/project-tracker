...
## ADR-061 — The Genesis Workbench: a per-application Kanban execution board (Phase 33) — **Proposed**
- **Status:** **Proposed** (2026-09-04, Phase 33; specs `specs/phase-33-workbench.md` + `phase-33-workbench/33-01..33-05`). Realizes the **ADR-056 reserved story-execution plug-point** + the **ADR-050/25-11** `Story`/`Stage` execution seam (the `kb_stories.status` column Phase 32/ADR-060 laid, "forward-compat, not surfaced" — this phase surfaces it as lanes). **Amends ADR-049** (adds a **Workbench** item to the primary nav). **Single-repo (genesis) + one migration (m0017);** genesis-core / kiro-agent-sdk / genesis-workflows / genesis-appian-parser unchanged. **New web dep:** `@dnd-kit` (drag-and-drop; accessible; ADR-027 stack extension — approved with the user).
