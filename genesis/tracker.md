...
| 17 | `specs/phase-17-business-application-map.md` (+ `phase-17-business-application-map/17-01..17-06` + `business-model-contract.md`) | Business Application Map 📋 | Agent-synthesized **business-language** map of what an app does end-to-end — **(A)** value stream(s) + **(B)** capability constellation — generated by a new deterministic **`generate-business-map`** LangGraph workflow that reads the code-free KB (via `genesis-kb`/`KbStore`) as ground truth, with **evidence-grounding + coverage + business-language** validators (anti-hallucination) + a HITL review gate, persists a versioned **`BusinessModel v1`** (migration **m0008** `kb_business_maps`, code-free, point-in-time, stale-on-sync), and renders it in the web (React Flow **A+B**, linked focus+context). **No** objects/bundles/pages vocabulary; **not** the technical graph. **ADR-039.** **DRAFT — spec only (umbrella + contract + 17-01..17-06); awaiting approval to implement.** |
