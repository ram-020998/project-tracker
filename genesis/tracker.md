...
> **Note:** Phases 17 (Business Map), 18 (Parser accuracy), 19 (Document Library), 20 (Features & Spec authoring), and
> 21–29 (Feature Workspace/Chat parity · Distribution · Scheduled syncs · UX+env-creds · Foundation hardening · Agentic
> Memory · UI/UX Revamp · Feature Revamp · **UX Design Stage**) are tracked
...
- **2026-09-25 — Phase 44 (Workbench, SDLC & Workspace Fixes — 12-issue bundle): ✅ SHIPPED — genesis v0.75.0, CI green. PHASE 44 COMPLETE.** Twelve live-reported defects bundled into one **genesis-only** fix phase (spec `specs/phase-44-fixes-bundle.md`); **one migration** (`m0022`, `current_version` 21 → 22) for the story-identifier sequence, every other fix code-only; **no Genesis Hub change**. Fully root-caused against the current tree: **(1)** Features tab → next to Overview (`ApplicationDetail.tsx` reorder); **(2)** delete-feature UI (backend `DELETE /features/{id}` + `useDeleteFeature` already exist — add the button+confirm in `FeaturePage.tsx`); **(3)** running story ids **`GNS_000001`** — a global monotonic `kb_sequences` counter (m0022) allocated in `kb/stories.py::finalize()`+`create_story()`, stored in the already-synced `kb_stories.key` (**ADR-072 Accepted**; no Hub change; existing stories untouched) + displayed on grid/card/workspace; **(4)** publish **system** lane moves — the finalizers (`story_design_finalizer`/`story_implementation_finalizer`) transition the lane via `LifecycleService` but never call `collab.publish_board_move` (manual moves publish from the frontend) → pass `collab_service` into both + publish best-effort/opt-in-gated after each transition; **(5)** design card not advancing to Design Review — **same cause as (4)** (unpublished `design→design-review` reverted by the always-on auto-pull re-mirroring the Hub's stale status; collab-enabled only) → fixed by (4); **(6)** real-time node **View Details** — `fold_iterations` freezes a running turn's `seq_end` at the prompt seq (conversation slice hides in-progress events) + `Conversation live={false}` + poll-only → extend seq_end + tail the run SSE scoped to the open node (true streaming, user decision); **(7)** chat page shows stage chats — `list_sessions` excludes only `feature_spec` → exclude all stage modes; **(8)** runs count stuck at 100 + no pagination — `RunStore.list` hard-caps 100 → server-side pagination+filters (20/page, `{runs,total}`); **(9)** catalog shows internal MCPs (`genesis-kb`/`appian-dev`…) as unmet prereqs → filter the internal/managed set out of `required_mcp` in the **backend** `/catalog`; **(10)** remove the Documents "Linked apps" column (`DocumentTable.tsx`); **(11)** move the gws+sail connector banners into each CLI's detail (`CliTab`→`CliDetail`; both are curated CLIs); **(12)** Settings **General** first + default (`SettingsPage.tsx`). Locked with the user via a 7-question round (2026-09-25): collab-only repro for (5); `GNS_`+6-digit+global+manual-included+no-backfill for (3); server-side pagination for (8); true SSE for (6); backend filter for (9); General first+default + Business-Map default kept for (1)/(12); all 12 in one phase. Built 44-01…44-08; independently reviewed = SHIP. Released **genesis v0.75.0** (tag `v0.75.0`, master `19b733fe`), CI green; **m0022** story-sequence migration (DB → v22). Two live fixes folded in from fleet testing (the paginated-`/runs` workflow-detail crash via `catalogApi.runsFor`; `appian-dev-write` added to the catalog internal-MCP filter). Gates: genesis pytest **851** + ruff; web tsc + eslint(0) + **vitest 288** + build. Both fleet instances restarted on v0.75.0. As-built: `progress/phase-44-fixes-bundle.md`.

- **2026-09-24 — Phase 43 (The Unified Story Workspace): ✅ SHIPPED — genesis v0.74.0, CI green. PHASE 43 COMPLETE.** An **enhancement** (first of this iteration; "a few issue fixes" are a later phase). Today, opening a story gives a **different screen depending on where you click + the card's lane** — the feature `StoryDetailPage` (details, no documents), the board `BoardCardDrawer` (details, no documents), or the lane-routed `StoryCardPage` → `StoryDesignWorkspace` (design chat) | `ImplementationReviewWorkspace` (impl report/rollback/objects) — three disjoint surfaces, each a slice, decided by `BoardPage.openCardView` lane branching. **This phase unifies it:** ONE canonical **Story Workspace** page at `/applications/:appUuid/stories/:storyId`, reached from **everywhere** (feature Stories tab, every board lane, legacy-route redirects), with **tabs** — **Overview** (details/AC/questions + inline Edit/Delete, always) · **Design** (design doc read-only + an **"Edit / chat"** button that opens the existing design chat **in a new browser tab**, only in Design/Design-Review; read-only after) · **Implementation** (report + objects-modified + rollback, read-only) — tabs appear only when the artifact exists, and the page **lands on the stage-relevant tab**. A **single documents-viewer chrome** labels each doc by type (Design document / Implementation report / Objects modified / Rollback) and dispatches to the existing renderers. A **consolidated, board-independent read endpoint** (`StoryStageStore.get_for_story` directly) feeds it so a feature-side story not on a board still resolves (Overview only) and documents light up after the board. **Decisions locked with the user (2026-09-24, Q1–Q11):** single canonical route; always the same page (drawer removed); tabs + stage-relevant default; edit from here; the design chat opens in a new browser tab; design read-only after implementation; the 4-document set is sufficient; converge on one doc-viewer chrome; reachable pre-board (Overview only); **consolidated read path + UI only, no migration**; **genesis-only**. **ADR-071 (Accepted)** — amends the *presentation* of ADR-060/061/062/068 (data/artifacts unchanged). Specs: umbrella `specs/phase-43-unified-story-workspace.md` + `phase-43-unified-story-workspace/43-01..43-05`. **Released genesis v0.74.0** (tag `v0.74.0`, master `d939fa71`), CI green (genesis + frontend + clean-install). Built per-sub-phase (43-01 consolidated board-independent read + Q16 lane-gate drop → 43-02 the `StoryWorkspacePage` + one canonical route + legacy redirects + drawer removed → 43-03 the `StoryDocument` viewer chrome + Design read-only/Edit-chat-new-tab + Implementation master-detail viewer + stage-aware default tab → 43-04 legacy cleanup + a live-feedback refinement pass [origin-aware breadcrumb; Dev note moved to the main column; Edit-in-Overview; removed back/remove/delete; dropped the redundant Objects-modified doc]); each sub-phase independently reviewed = SHIP. genesis-only, **no migration** (DB stays v21). Gates: genesis pytest **844** + ruff; web tsc + eslint(0) + **vitest 286** + build. Both fleet instances restarted on v0.74.0. **ADR-071 Accepted.** As-built: `progress/phase-43-unified-story-workspace.md`.

- **2026-09-24 — Phase 42 (Code Implementation) ✅ SHIPPED — PHASE 42 COMPLETE.** Genesis's **first environment-mutating capability** released in **ADR-019 order**, CI green on all three: **genesis-core v0.9.8** (`e5c99ca` / tag `v0.9.8` — the `kiro_node(skills=)` primitive; `CORE_MAJOR` unchanged) → **genesis v0.73.0** (`b9f287a0` / tag `v0.73.0` — the whole Phase-42 backend + web + the `""`→None optional-parent pull hardening) → **genesis-workflows v0.20.0** (`00e2c2c` / tag `v0.20.0` — the `story-implementation` workflow + the `appian-dev-write` MCP). The **Appian Genesis Hub** gained `GH Object Wiki Page`/`Entry` record types (rt `29ede62a…`/`c9a367bb…`), built + **live-validated** by the separate write-capable agent (additive to the ADR-064 contract; 18/18 harness green). **ADR-068/069/070 → Accepted.** Gates at release: genesis pytest **838** + ruff; genesis-core pytest **96** + ruff; genesis-workflows `validate_library` **13** + pytest **213**; web **vitest 276** + build; fresh DB → **v21**. **First live acceptance is pending and must be on a throwaway/sandbox app** (headless-undrivable; procedure in the progress doc). See `progress/phase-42-code-implementation.md`.

- **2026-09-23 — Phase 42 (Code Implementation): 🧱 BUILD LOG (per-sub-phase; ✅ SUPERSEDED by the 2026-09-24 SHIPPED entry above) — 42-01…42-08 were built LOCALLY, each independently reviewed = SHIP.** User signed off at 42-01 and is driving the build per-sub-phase (build → gates green → local commit → independent review → docs). Done so far: **42-01** contract lock — the frozen contracts (m0021 data model, the `appian-dev-write` allowlist, the workflow I/O, finalizer/lifecycle, review-surface API) code-grounded against the real repos (transition edges, `StoryStageStore.VALID_STAGES`, the read-only `appian-dev` allowlist incl. the verify+rollback tools); ADR-068/069/070 drafted. **42-02** the **`appian-dev-write`** write-scoped managed MCP entry (genesis-workflows) — reuses the `appian-dev` binary, `mode:read-write`, allowlist = the exact **60** CREATE/UPDATE tools of Dev MCP 26.6.90 incl. `updateObjectSecurity`, **ZERO `delete*`** (the 23 object deletes + the UPDATE-tagged, delete-named `deleteProcessModelNode` excluded); +4 guard tests. **42-03** workflow-node skill provisioning (**option B**, ADR-069): genesis-core **`kiro_node(skills=[...])`** (additive, `CORE_MAJOR` unchanged) provisions declared skills into the run's `.kiro/skills/` (fail-fast, no `KIRO_HOME`, traversal-safe, idempotent) + genesis `build_context` wires the `skill_source` resolver (managed `skills_dir` → global `~/.kiro/skills`); +7 tests. **42-04** the **`story-implementation`** ("Ticket Implementation") workflow (genesis-workflows) — `resolve_inputs → reverify_design →(gap) escalate(PROCEED/SEND_BACK) → prepare_rollback → implement → verify → [attach_healing] → author_wiki → persist_wiki(off-loop) → render_report → present → cleanup`; reliability trio per agent; `attach_healing` {1,1}; the write MCP + skills injected ONLY on implement/heal; **no node has any `delete*` tool** (deprecate-in-description; validator rejects a delete action); the reverify gate is **fail-safe** (only PROCEED_ANYWAY writes); +28 tests + registry entry (13 workflows). **42-05** platform backend (genesis) — **m0021** (`current_version` 20→21) `wiki_object_pages` + append-only `wiki_object_entries` (no code stored; m0019 collab cols); `ObjectWikiStore` (upsert idempotent; append-only + **(page,story) dedupe** for re-launches; history); `StoryStageStore.VALID_STAGES += 'implementation'`; the audited **`(IMPLEMENTATION,"send-back")→DESIGN_REVIEW`** edge; `StoryImplementationFinalizer` (done → implemented: bind artifacts + `submit-review`→code-review, audited m0013 / send_back → `send-back`→design-review; bound-run + lane-guarded idempotency + on-read reconcile; **no completion chat**); `POST …/implementation/start` (fail-fast 409 matrix mirroring `start_design`) + `GET …/implementation/report` (read-only review surface) + board DTO impl-run fields; +2 test modules + bumped every `current_version==20` head assertion → 21. **42-06** Object-Wiki collab (genesis) — `wiki_page` + `wiki_entry` `_BINDINGS` (append-only entry parented on its page via cross-machine `page_sync_uuid`; the local `latest_entry_id` pointer excluded from sync — no-leak); `publish_object_wiki_page`/`publish_object_wiki` helpers; `build_context` wires `ctx.extras['object_wiki']` + a best-effort, offline-tolerant `object_wiki_publish` (activates the 42-04 `persist_wiki` node; local-first, no hard Hub dependency); the **Genesis Hub `GH Object Wiki Page`/`GH Object Wiki Entry`** contract addition (additive to the frozen v1.0.0; no new Web API) + fixtures; extended the binding-kinds-vs-contract guard. **42-07** web (genesis) — drag-into-Implementation confirm (`StartImplementationDialog`, the single blanket approval, warns it writes to the dev env) → `startImplementation`; card **running/locked/failed** states + run link; a **read-only review surface** (`ImplementationReviewWorkspace`, reachable from Implementation OR Code Review) = per-object table + the report in a **sandboxed iframe** + the rollback via MarkdownView (no chat, no Lavish); types/api/hooks/query-keys + jest-axe; `web/static` rebuilt + committed. **42-08** code review & hardening — an independent **whole-phase write-safety review** (SHIP-WITH-MUST-FIX); **MUST-FIX applied**: scoped the `appian-dev-write` allowlist to **design-object authoring (60 → 55)** by removing 5 non-authoring runtime/data/security writes (`completeTask`, `insertRecordData`/`updateRecordData`, `addGroupMembers`/`removeGroupMember`); **SHOULD-FIX applied**: report-endpoint lane gate (Q16), `persist_wiki` attribution (`author_username`/`story_sync_uuid` via `ctx.extras['actor']` + the story `sync_uuid`), stale docstring; + a new allowlist-exclusion guard; live-acceptance procedure written. **Gates green:** genesis-core pytest **96** + ruff; genesis pytest **837** + ruff (fresh DB → v21); genesis-workflows `validate_library` **13** + pytest **213**; web **vitest 276** + tsc + eslint (0 errors) + build (stale-bundle guard clean). **Local commits (UNPUSHED / UNTAGGED — no code-repo push until 42-09 on the user's go-ahead):** genesis-core `4d7b3d3` (ahead 1); genesis `1148545`+`4efbeb3`+`92ea722`+`1b6e8f0`+`4010000`+`ecb8e0c` (ahead 6); genesis-workflows `e5e1260`+`bc92a09`+`7d152fd`+`d5b4c19` (ahead 4). Progress synced to `progress/phase-42-code-implementation.md`. **Remaining:** 42-09 coordinated release (ADR-019 order genesis-core → genesis → genesis-workflows + the Appian Hub `GH Object Wiki Page`/`Entry` record types; ADR-068/069/070 → Accepted; live-acceptance on a sandbox app first) — **awaiting the user's explicit push/tag go-ahead.** **LATEST SHIPPED is unchanged: genesis v0.72.3 / genesis-workflows v0.19.0** (Phase 42 code is not released).

- **2026-09-23 — Phase 42 (Code Implementation — the Workbench Implementation lane): 🟡 SPEC-DRAFTED (awaiting build sign-off).** Genesis's **first write-capable stage** — it turns the corner from read-the-env/write-documents to **actually implementing object-level code** in the dev-tagged Appian environment (a deliberate, eyes-open contradiction of the read-only posture the earlier ADRs held **until a stable Dev MCP** — Phase 41 adopted 26.6.90 with full CRUD). Flow: dragging a story **Design Review → Implementation** (a confirm dialog = the single blanket approval) launches an unattended **`story-implementation` ("Ticket Implementation")** workflow → **reverify** the design vs the feature Technical Design (mismatch → escalate: proceed / send-back) → **prepare a local rollback doc** (current code + object version of every UPDATE'd object; NEW → "delete to roll back"; no indirect dependents) → **implement** (a SINGLE agent + the `appian`/`appian-object-generation` skills + Dev-MCP **write** tools, object-by-object; **no deletes ever** — a retire → mark the object description "deprecated by Genesis") → **verify** only the touched objects (`validate*`/`test*`) → **heal** (`attach_healing`, ADR-067) → **update the Object Wiki** (a shared, append-only per-object change ledger: business + technical descriptions) → move the card to **Code Review**. A read-only review surface (implementation report + rollback doc) is reachable from the card in Implementation + Code Review. **New ADRs (Proposed):** **ADR-068** (write-capable implementation — amends ADR-021 `pre_mutation`/036/037/038, supersedes ADR-062's proposal-only clause; dev-env only; no deletes; a dedicated `appian-dev-write` MCP entry injected only by implement/heal), **ADR-069** (workflow-node skill provisioning — extends ADR-034 beyond chat), **ADR-070** (the Object Wiki — local-first, Hub-shared, append-only; new Genesis Hub record type additive to ADR-064). Migration **m0021** (`wiki_object_pages` + `wiki_object_entries`; `current_version` 20→21). Multi-repo: genesis + genesis-workflows + the Appian Hub record type (built by the separate write-capable agent) [+ genesis-core only if the skill primitive lands in core]. Specs: umbrella `specs/phase-42-code-implementation.md` + `phase-42-code-implementation/42-01..42-09`; 3 backlog docs (`implementation-rollback-execution.md`, `wiki-as-agent-memory.md`, `kb-resync-after-implementation.md`). Decided over two Q&A rounds with the user (see the umbrella §3). **Analysis + specs only — no code changed. Build begins only after user sign-off at 42-01.**

- **2026-09-15 — Phase 41: Dev MCP 26.6.90 upgrade (basic auth) + SAIL CLI install: ✅ SHIPPED — genesis v0.72.1 + genesis-workflows v0.19.0** (`e75acaa`/`43d121a`, tags `v0.72.1`+`v0.19.0`; CI green — genesis #6813229 [genesis+frontend+clean-install], workflows #6813196). Adopt the Appian **Dev MCP 26.6.90** with **basic auth only** (browser/SSO deferred → `specs/backlog/devmcp-browser-sso-auth.md`): the `appian-dev` registry pins a static **`LCP_AUTH_METHOD=basic`** (26.6.90 flipped the default to browser) and **drops `LCP_API_PATH`** (bundle defaults it; removed end-to-end from the env registry/API/web, back-compat load kept); allowlist **+5 read tools** (getPortal/listPortals/listObjectVersions/listMyTasks/getDevMcpVersionInfo; 72 total; all 67 prior tools still exist → read-only posture intact). **SAIL CLI** installs from the SAME bundle in one action (`ConfigService.install_appian_dev_bundle` → extract host-platform binary via `cli_tools/native/sail_bundle.py` + `NativeCliInstaller` with a BUILD-INFO version + **macOS quarantine clear**; unsupported arch → MCP-only + reason); a fully-wired managed CLI (`cli-registry` `sail` + `genesis/integrations/sail/SailClient` + `ctx.extras['sail']`; dev-env basic creds via the ADR-048 seam; isolated `SAIL_HOME`) surfaced in **Settings → CLI** (`SailConnectorCard`) — **no consumer wired yet**. Amends ADR-038/040 (mirrored in decision-log). Gates: genesis pytest **803** (+13 `test_sail_cli.py`) + ruff/tsc/eslint clean + web **vitest 271** + build; workflows validate_library **12** + pytest **184**. **Verified end-to-end** against the real 26.6.90 bundle (`sail --version` → 26.6.90). **§7 lesson recurred:** v0.72.0 forgot `git add tests/` → CI red on the old LCP_API_PATH tests → fixed forward as v0.72.1 (no tag move). **DEPLOYED + live-verified:** v0.72.2 gives the `genesis mcp install-native` CLI combined-installer parity; installed the 26.6.90 bundle on the host (appian-dev `0.1.0+528e6bbb` + sail `20260903-195919`) + reinstalled the library + `gsm provision`/restart the fleet (both on v0.72.2 + new Dev MCP via symlink); `listApplications` returns real apps via basic auth on the host AND the running fleet :8760. CI green genesis #6813349 (v0.72.2). **v0.72.3:** managed-CLI cards resolve via the installer (`sail`/`gws` now show reachable in Settings → CLI, not a PATH check) + `genesis-fleet` provisioner shares `cli-tools` into instances → `sail`/`gws` reachable on both fleet instances (CI green #6813404). **No active phase.**

- **2026-09-15 — Workbench: link the board's application name to its Genesis application page: ✅ SHIPPED — genesis v0.71.0** (`c94101e`, tag `v0.71.0`; CI green #6812636 — genesis + frontend + clean-install). A small UX add: in the Workbench board header, an **external-link icon beside the application name** opens the Genesis **application detail page** (`/applications/:uuid`) in a **new tab** (`target=_blank` + `rel=noopener`). Added `ExternalLink` to the curated `shared/ui` icon re-export + a `WorkbenchPage` test asserting the href/target/rel. **Frontend-only** (no API/DB/behavioural change); genesis-core / genesis-workflows unchanged. Gates: web tsc/eslint(0)/**vitest 271** + build + stale-bundle guard; genesis pytest unaffected (web-only). Both fleet instances restarted on v0.71.0. **No active phase.**

- **2026-09-11 — `story-design-analysis` v0.3.0: single-context design (ADR-062 amendment): ✅ SHIPPED — genesis-workflows v0.18.0** (`da42b9b`, tag `v0.18.0`). User feedback (grounded in the Appian team's own `jarvis-design` skill): the Ticket Design workflow was over-engineered and the **per-object decomposition broke cross-object coherence** — each `design_object` ran in its OWN Kiro turn seeing only that one object, so it couldn't keep shared fields/naming/sequencing consistent. **Rebuilt to a single-context design:** `resolve_inputs → load_inputs → research` (grounded, ONE turn: mine Spec/UX/TD + ground current state/code via @genesis-kb/@appian-dev → an implementation plan) `→ design` (ONE turn, FULL context → the entire design body) `→ render` (deterministic wrap → one well-formed, script-free, Lavish-safe `design.html`) `→ verify` (grounded critic → VerificationReport) `→ verify_route` (ok→present | **revise** ≤2: re-author `design` with the critic's fixes as carry-forward guidance + reset `retries[design]=0` | exhausted→escalate) `→ present → cleanup`. Output is a **lean implementation plan** (per object: what exists / what changes / what to build; targeted code only where it matters; process models described, not dumped node-by-node). **Removed** the per-object loop (`plan_objects`/`design_object×N`/`synthesize`/`assemble`) + `attach_healing` **here** (no granular blocks to patch → a plain bounded verify→revise loop; `attach_healing` stays in feature-breakdown / technical-design / ux). **`inputs_schema` UNCHANGED** → the genesis caller (`_launch` design-start) + `StoryDesignFinalizer` binding (workflow_id `story-design-analysis`, artifact `design.html`) are unchanged — **genesis-workflows-only** release (genesis-core v0.9.7 / genesis v0.70.0 pins unchanged). **Gates:** `validate_library` 12; story-design pytest **21**; full workflows pytest **184**. ADR-062 amended (decision-log + bible/04). See `progress/phase-34-design-lane-automation.md`. (Note from the same session: the run the user flagged [`r-1af0c18cedda`] engaged the Phase-40 heal tier correctly — verify→heal chose *patch* — and was **cancelled** mid-heal-turn, not a crash/timeout.) **technical-design-analysis** may get the same single-context reconsideration later (deferred — it's the one workflow where multi-workstream size caused a real timeout). **No active phase.**

- **2026-09-10 — ✅ SHIPPED: Phase 39 (Run Inspection & Node Observability, ADR-066) + Phase 40 (Self-Healing Reliability, ADR-067) — genesis-core v0.9.7 · genesis v0.70.0 · genesis-workflows v0.17.0.** A coordinated multi-repo release (ADR-019 order; no DB migration). **Phase 39:** the run-detail node experience is a **single-click summary (authored node `description` + per-execution metrics) + a large adaptive turn-explorer modal** (Pass › Item › Attempt › Turn) exposing per iteration the exact prompt / artifacts / conversation / output / credits+timing — genesis-core captures the prompt as a blackboard-file pointer (`agent.prompt`, bytes never inlined) + a reserved `_iteration` channel; genesis adds `fold_iterations` + `GET /runs/{id}/nodes/{node}/iterations` + `GraphNode.description`; all 12 workflows' 175 graph nodes described. New-runs-only. **Phase 40:** a **second reliability tier** in genesis-core — **`attach_healing`** (`verify` grounded critic emits a `VerificationReport` → `heal` agent judges blast radius → **patch the flagged items in place → re-verify** | **one guided restart** with carry-forward guidance → **escalate**, bounded `META.healing {max_heal:1,max_restart:1}`; `verify`+`heal` each wear the ADR-011 trio) + `VerificationReport`/`HealDecision` contracts + a reserved `_healing` channel; **all four analysis workflows adopt it** (feature-breakdown 0.2.0 / technical-design 0.3.0 / story-design 0.2.0 / ux-design 0.2.0), replacing the wasteful full-redo-on-verify-failure (credit-safety, ADR-032). Both phases **independently reviewed (no MUST-FIX)**. **Gates:** genesis-core pytest **91** + ruff; genesis pytest **790** + ruff, web **vitest 270** + build; genesis-workflows `validate_library` clean (12) + pytest **185**. **CI green:** genesis-core #6781318 (v0.9.7), genesis-workflows #6781339 (v0.17.0), genesis #6781332 (v0.70.0). ADR-066 + ADR-067 → **Accepted**. See `progress/phase-39-run-inspection-observability.md` + `progress/phase-40-self-healing-reliability.md`. **Deployed to the running fleet:** `gsm provision --all --force` installed the **v0.17.0** workflow library into both instances (main :8760 + userb :8761) — the catalog on both serves the healing versions (feature-breakdown 0.2.0 / technical-design 0.3.0 / story-design 0.2.0 / ux-design 0.2.0), so a fresh analysis run now exercises the `verify → heal → {patch | guided restart} → escalate` tier.

- **2026-09-10 — Phase 39 (Run Inspection & Node Observability): 🟢 BUILT 39-01..39-06 (independently reviewed, all gates green) — ⏳ 39-07 release PENDING (committed LOCALLY, UNPUSHED, no tag).** The run-detail node experience is rebuilt into **single-click summary (a new authored node `description` + per-execution metrics) + a large adaptive turn-explorer modal** (Pass › Item › Attempt › Turn, depth = reality) exposing, per iteration, the exact **prompt / artifacts provided / conversation / output / credits+timing**. **genesis-core** (additive, `CORE_MAJOR` unchanged): `kiro_node` writes each turn's rendered prompt to a blackboard file `_prompts/…` and emits **`agent.prompt`** (pointer-only — bytes never inlined, ADR-010/018) + `attempt`/`iteration` on `agent.result`; a reserved **`_iteration`** `PlatformState` channel. **genesis** (no DB migration): `agent.prompt` persisted (`_CANONICAL_CUSTOM`); **`fold_iterations`** + **`GET /runs/{id}/nodes/{node}/iterations`**; `GraphNode.description` passthrough; the `Inspector` + **`NodeIterationsDialog`** (lazy prompt fetch, per-turn Conversation sliced by seq range). **genesis-workflows**: a `description` on **all 12 workflows / 175 graph nodes**; the 4 analysis workflows stamp `_iteration` in their loop drivers (+ re-stamp pass on revise — audit M1); `validate_library` gained a non-fatal description warning (passes with 0). A `/dev/run-inspection` mockup. **Independent audit** (sub-agent) fixed M1 (heal-pass labeling) + S1 (Executions→Turns metric). **Gates:** core pytest **85**; genesis pytest **790** + web vitest **270** + build; workflows `validate_library` clean + pytest **179**; tsc/eslint clean. **Fleet restarted** — both instances run the new code (new-runs-only capture). **Local commits to release at 39-07:** core `94041e2`; genesis `c7bd47f7`; workflows `ed72b16`. **→ SUPERSEDED (shipped 2026-09-10 — see the entry above):** 39-07 released as genesis v0.70.0 (+ genesis-core v0.9.7 / genesis-workflows v0.17.0) together with Phase 40; ADR-066 Accepted.

- **2026-09-10 — Two enhancement programs SPEC-DRAFTED (awaiting build sign-off): 🟡 Phase 39 (Run Inspection & Node Observability) + Phase 40 (Self-Healing Reliability).** After a grounding + design session with the user (both enhancements traced to real code), created two separate umbrella specs + sub-specs + Proposed ADRs. **Phase 39 (ADR-066):** rebuild the run-detail node experience — single-click **summary** (a new authored node `description` + basic per-execution metrics) + a large **adaptive turn-explorer modal** (Pass › Item › Attempt › Turn, depth = reality) exposing, per iteration, the exact **prompt / artifacts provided / conversation / output / credits+timing**. genesis-core additively captures the prompt (blackboard-file pointer) + a generic `_iteration` grouping label + `attempt` on the turn events (no schema change; ADR-010/018); genesis adds `fold_iterations` + endpoint + `GraphNode.description`; genesis-workflows authors node descriptions + `_iteration` stamps; a `/dev` mockup gates the build; new-runs-only. Specs: `specs/phase-39-run-inspection-observability.md` (+ `39-01..39-07`). **Phase 40 (ADR-067):** a **second reliability tier beside the ADR-011 trio** — a genesis-core **`attach_healing`** construct: `verify (grounded critic, structured item-keyed fixes) → heal (agent judges blast radius) → {patch flagged items in place → re-verify | one guided full restart} → escalate`, bounded `META.healing {max_heal:1, max_restart:1}`; `VerificationReport`/`HealDecision` contracts + carry-forward guidance. Replaces today's wasteful "revise re-runs everything" (feature-breakdown re-breaks ALL epics; TD/story-design re-run synthesize which can't fix frozen blocks) — the biggest avoidable credit waste (ADR-032). All four analysis workflows adopt it (feature-breakdown first). Specs: `specs/phase-40-self-healing-reliability.md` (+ `40-01..40-06`). Both ADRs mirrored in `reference/decision-log.md` (Proposed). **Analysis + specs only — no code changed. Build begins only after user sign-off at 39-01 / 40-01.**

- **2026-09-09 — Workbench board fixes (explicit membership + to-do default + drag guard + shared scroll): ✅ SHIPPED — genesis v0.69.0** (`f3aa30e`, tag `v0.69.0`). Live-reported bug: importing ~6 stories of a newly-finalized feature onto a board showed **all 50**, most stuck in **Design**, and moves to To Do wouldn't stick. Diagnosed to three compounding causes (see §7). **Fixes:** **(1)** finalized + manually-created stories default to status **`to-do`** (the board's entry lane), not `design`. **(2)** board membership is now **explicit** — new **m0020** `kb_stories.on_board` (a shared flag, backfilled=1 for currently-carded stories; rides the story sync payload) set by `import_stories` / cleared by remove-from-board; `reconcile_board_membership` cards **only `on_board=1`** stories (was: every finalized story of the app — which, with the v0.68.0 backend auto-pull loop, continuously re-added them); frontend import/remove publish the affected stories so membership propagates. **(3)** the 10s board `refetchInterval` no longer resyncs the lane mirror while a move is persisting (`reorder.isPending`) — moves stop flickering. Also folded in the **sticky/shared-scroll board header** (staged from the prior request). **Hub-side change (owner: user) — DONE + verified:** the `GH Story` record type + records upsert/get/list Web APIs now carry an additive **`on_board`** (0/1) field (JSON key `on_board`); round-trip verified end-to-end (main import→publish→userb pull: on_board 1 and 0 both sync — Appian returns a JSON boolean, genesis coerces bool↔int). Remediated main's existing board: flipped the stuck Document Builder cards to To Do. `current_version` 19→20. Gates: backend pytest **787** + ruff; web tsc/eslint 0 / **vitest 266**; CI green (#6766607). Both instances restarted on v0.69.0. **Deferred follow-up:** puller-side remove-propagation (reconcile also deleting a card when a teammate clears `on_board`). **No active phase.**

- **2026-09-09 — UX-analysis spec-load fix + sticky/shared-scroll Workbench board: ✅ SHIPPED — genesis v0.68.1** (`a3ba059`, tag `v0.68.1`). **Bug:** a `ux-design-analysis` run for a feature with a **completed** Spec produced a doc that said *"no spec has been authored"* and analyzed every screen as a gap (found on run `r-23639e1aed28`, feature 4). Root cause: a chat-authored spec's HTML lives in the `feature_spec` **session sandbox** and `kb_feature_stages.html_path` stays **NULL until a milestone**; `api/features.py::_launch_analysis` passed the raw `html_path` (→ null) as the workflow's `spec_path`, so `_load_spec` saw no spec. The TD + Breakdown launches already snapshot `_current_stage_html(spec_row)` (sandbox-preferred) — the UX launch was never updated. **Fix:** `_launch_analysis` snapshots the current spec HTML to `spec_snapshot.html` + passes that (mirrors TD/Breakdown). +1 regression test (`test_ux_upload_passes_sandbox_spec_even_when_html_path_null`); backend pytest **787**. Existing runs aren't retroactive — re-run the UX stage. See §7. **Also (UI):** the Workbench board is now **one shared scroll** with **sticky lane headings** — all lanes scroll together (no per-lane scrollbars), app name + filters stay fixed (`WorkbenchPage`/`BoardPage`/`BoardColumn` sticky-header layout). web **vitest 266**; tsc/eslint 0. Both fleet instances restarted on v0.68.1. **No active phase.**

- **2026-09-08 — Backend collab auto-pull loop + sticky Workbench board header: ✅ SHIPPED — genesis v0.68.0** (`b236d95`, tag `v0.68.0`). The board lag persisted after v0.67.1; **measured the real cause** (captured userb's request log): with the Hub read-your-writes at ~0.9s, the delay was **browser background-tab timer throttling** — a passive puller's frontend `setInterval` drops to **~1 poll/min** when its tab is backgrounded (confirmed: **1 request in 45s**). A frontend driver can't beat that. **Fix — move the cadence to the backend:** `CollaborationService.autopull_tick()` (change-gated via a persisted `autopull` cursor — pulls `pull_all`+`pull_boards` only when the Hub manifest advances) driven by `runtime/collab_autopull.py::CollabAutoPuller` — an **always-on asyncio loop** (10s) started in the app lifespan, gated live on `is_enabled()`, ticking **off the event loop** (`asyncio.to_thread`, §7 deadlock lesson). Now a puller stays synced **regardless of any browser tab**; `useBoard` gained a 10s local `refetchInterval` so an open board reflects the synced local data. **Verified live:** with **no browser open**, userb's local story lane flipped `to-do → code-review` ~10s after a publish from main. **UI:** the Workbench board header is now **sticky** — `WorkbenchPage` is a full-height flex column (app name fixed), `BoardPage`'s toolbar/filters stay fixed, and each lane column has a fixed heading with an internally-scrolling card body, so **app name + filters + lane headings stay put while the cards scroll**. Gates: backend pytest **786** (+2 autopull tests) + ruff; web tsc/eslint 0 / **vitest 266** / build. Both fleet instances restarted on v0.68.0. **No active phase.**

- **2026-09-08 — Collaboration auto-pull latency fix: ✅ SHIPPED — genesis v0.67.1** (`4a1e3be`, tag `v0.67.1`). A teammate's board move was taking "some time + multiple refreshes" to appear on a second instance. **Measured the cause first** (a live probe: publish a Story from main, then poll the Hub): the change is visible via the Appian Hub `/changes` manifest **and** `get_record` in **~0.9s** — **no Appian replica lag** (the earlier eventual-consistency hypothesis was wrong; read-your-writes holds here). The delay was entirely the **frontend auto-pull cadence** — a flat **30s** interval (auto-pull is a foreground driver; the backend doesn't pull on its own). **Fix** (`useAutoPull`): (1) **board-aware interval** — **10s** while a Workbench board (`/workbench/:appUuid`) is open, 30s otherwise; (2) **event-driven** — a cheap `GET /collab/changes` poll issues a real `pull` **only when the Hub change cursor advances** (no write/DB churn when nothing changed); (3) **force pull on app load / window reload** (bypasses the 8s throttle) + on every route change. Net: a teammate board move reflects within **~10s** on the board (or instantly on reload/navigation). +1 web test (pulls when the manifest advances); web **vitest 266**; tsc/eslint clean. Both fleet instances restarted on v0.67.1. **No active phase.**

- **2026-09-07 — Collaboration auto-pull: ✅ SHIPPED — genesis v0.67.0** (`abbdbdc`, tag `v0.67.0`). A puller previously only pulled via **Settings → Collaboration → "Sync now"**, so a teammate's board move / feature + story updates didn't reflect until a manual sync (diagnosed live: a card moved to Implementation on main published fine — Hub story + BoardState both `implementation` — but userb still showed the old lane until pulled). **`useAutoPull`** (mounted once in `AppShell` via `CollabAutoPull`) now pulls automatically on: **app load**, **every route change** (opening the board, features, stories, a stage), a **30s interval**, and **window refocus / tab-visible** — **throttled** (8s min + in-flight guard), gated on collaboration **enabled + Hub reachable**, and **quiet on error** (no toast per interval when the Hub blips). Safe to auto-apply: pulled entities are **read-only mirrors** on the puller (no local draft to clobber). `usePull` gained a `{quiet}` option. web **vitest 265** (+2: auto-pulls when enabled; no-op when disabled); tsc/eslint clean; `web/static` rebuilt. As-built: `progress/collab-settings-toggle-adr-065.md` §v0.67.0. **No active phase.**

- **2026-09-07 — Collaboration puller-fidelity fixes: ✅ SHIPPED — genesis v0.66.3 → v0.66.5** (`70bb128`, tag `v0.66.5`; v0.66.4 `64d26a3`, v0.66.3 `6bf728a`). Making a *second instance* (a "new user") actually **see** pulled work exposed a cascade — all masked by the permissive `LocalHubProvider` (which echoes the full payload, so tests passed while the real Appian Hub, storing only its contract fields, did not). **v0.66.3:** `pull_all` was **crashing** (`NOT NULL constraint failed: kb_feature_stages.title` — the Hub `stage_artifact` has no `title`); `_upsert_local` now fills NOT-NULL-no-default local columns absent from the payload (`title`←a nice stage label, `*_at`←now). Pulled features also **never populated the Workbench board** (the Hub `feature` record has no `stories_finalized_at`); `pull_boards` now **derives** it (a pulled feature with stories was finalized upstream). **v0.66.4:** the stage workspace showed **"Start"** on completed pulled stages — `StageBuilderPage` gated on `chat_session_id`, but a pulled mirror has the artifact with no local chat; added a **read-only** branch (`PulledStageView` — the artifact via `artifactUrlFor` annotate=0 + a "shared by your team" banner). **v0.66.5:** the derived stage `title` maps to a human label (`_STAGE_LABELS`). **Live-verified:** userb pulled 1 feature / 12 epics / **72 stories** / 4 stage artifacts + **72 board cards**, and each stage opens read-only. **No Hub change needed** — the two genuinely-dropped fields (`feature.stories_finalized_at`, `stage_artifact.title`) are correctly derived puller-side (the frozen contract stands; a probe confirmed `published_by` round-trips as `created_by`/`modified_by`). Gates: backend pytest **784** (+ regression tests: title-less pull, board-populate-after-pull, read-only pulled view) + ruff; web tsc/eslint 0/**vitest 263**/build. As-built: `progress/collab-settings-toggle-adr-065.md` §§v0.66.3–v0.66.5 + bible §7. **No active phase.**

- **2026-09-07 — `genesis-fleet` (`gsm`): a standalone multi-instance manager — NEW REPO.** A separate, stdlib-only Python CLI (`git@gitlab.appian-stratus.com:ramaswamy.u/genesis-fleet.git`, v0.3.0) to run + manage multiple local Genesis instances (sessions) from one command — built to test **collaboration** by running two "users" on one machine (publish from one, pull into the other). It has **no dependency on the genesis codebase** — it drives the `genesis` executable (`up/down/status/logs/db upgrade/install`) with per-instance `GENESIS_STATE_DIR`/`GENESIS_ARTIFACTS_DIR`/`GENESIS_COLLAB_*` + `--port`, over a registry at `~/.genesis-fleet/config.json`. Verbs: `add/start/stop/restart/status/list/open/logs/rm/exec/provision`. **Auto-provisioning** (on `add`/`start`): installs the workflows library (→ the `appian-dev`/`appian-devops`/`genesis-kb` registry entries), **shares** the host's native Dev/DevOps MCP installs via a symlink, seeds the dev environment + ADR-048 `env-*` credentials from a source install (`~/.genesis`), and flips the collaboration toggle — so a new instance is collaboration-ready out of the box (only the per-user Appian **identity** is set in its UI). Repo has its own README + 17 unit tests; not part of the genesis release chain. See `genesis-fleet/README.md`.

- **2026-09-06 — Two-toggle collaboration enablement (ADR-065): ✅ SHIPPED — genesis v0.66.0 → v0.66.2** (`ac986fc`, tag `v0.66.2`; v0.66.1 `d304288` #6745373/#6745374; v0.66.0 `399fd17` #6745367/#6745366). A Settings-level in-app enable toggle **on top of** the env master (`GENESIS_COLLAB_ENABLED`), so collaboration is active only when **BOTH** are on. New `genesis/collab/config.py` `CollabConfigStore` (`~/.genesis/collab.json`: `ui_enabled` default **off** + static `hub_app_uuid`; **no migration**). `CollaborationService.is_enabled()` = provider-built **AND** `ui_enabled` (live — flip without a restart); `_require()` gates all publish/pull. The Settings toggle can only be turned **on** when the **Genesis Hub app is reachable in the dev env** — `GET /collab/availability` (env master + `provider.is_available()` /meta ping) + `PUT /collab/config` → **409** on enable-when-unavailable (so nothing breaks). The **Hub URL is derived from the dev-tagged env** (`{dev.url}/suite/webapi`) — no separate URL setting; the **Hub app UUID** is a one-time Settings field (config wins over the env fallback). API: `GET /collab/availability` + `PUT /collab/config`; `GET /collab/config` extended (`enabled`/`env_enabled`/`ui_enabled`/`available`/`hub_app_uuid`/`hub_url`). Web: Settings→Collaboration `Switch` (disabled with a reason when env master off / Hub unreachable) + Hub-app-UUID field; **stale "wired in a later phase" copy removed**. **v0.66.1** fixed the derived Hub base to include the Appian Web-API mount `/suite/webapi` (v0.66.0 derived the bare root → availability always false). **v0.66.2** remapped feature stages to the Hub's **`stage_artifact`** contract kind/shape — the first live publish returned `400 invalid kind` because the binding sent `feature_stage` + `feature_sync_uuid` while the frozen contract wants `stage_artifact` + `parent_sync_uuid`/`parent_kind`/`upstream_versions`/`blob_key=<parent>:<stage>`; the permissive `LocalHubProvider` had hidden it (§7). Live-verified: `publish_feature_stage → PUT stage_artifact → created v0` against merge-assist-dev. Gates: backend pytest **781** (+2: availability-guard 409 + both-on gating) + ruff; web tsc/eslint 0/**vitest 262**/build + `web/static`; acceptance 35/37/38 pass. **Live-enabled** against the `merge-assist-dev` dev env: `/collab/config` → `enabled:true, available:true, hub_url:…/suite/webapi, hub_app_uuid:_a-0000f058-…`. As-built: `progress/collab-settings-toggle-adr-065.md`. **No active phase.**

- **2026-09-06 — Phase 38 (Collaborative SDLC, implements ADR-063): ✅ SHIPPED — genesis v0.65.0** (`394f29e`, tag `v0.65.0`; CI master #6745235 / tag #6745236). The **final** collaboration phase — the human-authored SDLC entities sync across a team over the Phase-37 `AppianHubProvider`; **opt-in / off by default**; genesis-only; **no migration** (m0019 stands). **38-01** features + published stage artifacts (`publish_feature_stage` publish-on-complete; **cross-machine parent resolution** `feature_id`↔`feature_sync_uuid`; artifact **bytes** as a blob; **pull = read-only mirror** to the puller's disk; `chat_session_id`/`run_id`/`html_path`/`feature_id` **never leave**; stages track their CAS base in `collab_sync_state`). **38-02** epics/stories (`publish_feature_backlog` at Finalize + edit + `publish_deletion` tombstones; AC/questions/labels travel as **arrays** [Appian `GH Story Item`] but stay JSON-string columns locally — **lossless round-trip gate**; optional epic parent). **38-03** boards (lane = the shared story `status`; **auto membership** over shared finalized stories; in-lane `board_position` + `workbench_boards` sidebar curation stay **local**; `BoardState` signal + CAS). **38-04** collaboration UX (`genesis/api/collab_sync.py` publish/pull/advisory/adopt endpoints [409/401/503/404 mapping]; `features/collab/SdlcControls.tsx` `<PublishToTeamButton>`/`<PublishedBy>`/`<AdvisoryMarker>` + hooks, mounted on the stage workspace + Workbench lane-move auto-publish + a Settings **Sync-now**; reuses the Phase-37 `<HubStatus>`/`<StalenessBadge>`; jest-axe). **38-05** adoption (one-time confirmed bulk publish of existing completed work; **idempotent**; never automatic; opt-in no-op). **38-06** independent audit → **SHIP** + C1 drag-lane publish + a finalize-republish fix (so `stories_finalized_at` propagates for cross-machine board membership) + board_state cursor seeding + `scripts/acceptance/phase-38-collaborative-sdlc-acceptance.py` (headless full-SDLC hand-off, ALL PASS). Gates: backend pytest **779** + ruff; web tsc/eslint 0/**vitest 262**/build + `web/static`. As-built: `progress/phase-38-collaborative-sdlc.md`. **The collaboration program (35→38) is DELIVERED for the KB + features + stories + boards; shared-memory sync is the one deferred future phase.** Deferred display niceties (per-stage attribution + cross-stage staleness badge mount) documented in `specs/phase-38-collaborative-sdlc/38-04-deferred-ui-notes.md`. **No active phase.**

- **2026-09-06 — Phase 37 (Hub Wire-up, implements ADR-063/064): ✅ SHIPPED — genesis v0.64.0** (`42e610e`, tag `v0.64.0`; CI master #6745106 / tag #6745107). The collaboration mechanism goes **live against the real Appian Hub** for the first two shared surfaces — the **application KB** + **identity/teams** — plus the reusable **sync-UX foundation**. **37-01** `genesis/collab/providers/appian.py::AppianHubProvider` implements the frozen Phase-36 contract (records CAS→409, blob dedup, changes/activity, teams as generic `records`; `appian-api-key` auth from the ADR-048 dev-env store; offline-first `is_available()`; **contract-conformance tested vs the shared Phase-36 fixtures**). **37-02** KB **pull-first** sharing: `KbStore.hydrate_from_blob` (SCD-2 replace-current, off-loop) + `kb/blob.py` (gzipped code-free blob, **deterministic `mtime=0`**) + `publish_kb`/`pull_kb_if_newer` (content-hash dedup, per-app lock, Hub-app exclusion) + `POST /applications/{uuid}/pull|/publish` + an off-by-default `kb-hub-pull` scheduler job. **37-03** identity/teams live (provider-agnostic `/collab/*` routes now hit Appian) + an **onboarding-before-first-publish gate** (`require_onboarded`/`NotOnboardedError`→409) + attribution to the Hub. **37-04** sync-UX foundation: `GET /collab/status`+`/changes`; `useHubStatus`/`useHubChanges`/`useSyncNow`; `<HubStatus>` offline-first + `<StalenessBadge>` notify-then-apply. **37-05** independent audit → fixed a **MUST-FIX** (gzip `mtime=0` so dedup works) + a **SHOULD-FIX** (pull/publish degrade to 503/401 on a down Hub) + `scripts/acceptance/phase-37-hub-acceptance.py`. **No migration** (m0019 stands); **genesis-only** (no core/SDK/workflows change). Gates: backend pytest **761** + ruff; web tsc/eslint 0/**vitest 254**/build + `web/static`. Live acceptance is user-driven (deploy the Hub → onboard two instances → refresh-from-Appian on A → pull on B). As-built: `progress/phase-37-hub-wireup.md`. **Next: Phase 38 (Collaborative SDLC — features/stories/boards publish-pull onto this foundation).**

- **2026-09-05 — Phase 36 (The Genesis Hub Appian application, ADR-064): ✅ COMPLETE + LIVE-VALIDATED.** The dedicated **`Genesis Hub`** Appian app (the Collaboration Hub, ADR-063; the team dev env, no separate Hub env) is fully built in the dev env by the write-capable Dev-MCP agent (genesis read-only — ADR-036/037) and **validated end-to-end** with the shared service-account API key. **Surface:** 11 UUID-keyed record types + 13 relationships + record-level security + 9 constants + a **base64 blob store** (Appian Documents via the `GH Convert Base 64 To Document` process, indexed by `GH Blob Version`, dedup + keep-last-N) + **11 Web APIs** (`meta`; `records` upsert[PUT base-version **CAS→409/201/200**]/get/list; `changes`; `activity` set/list/clear; `blobs` put/get/versions) + **12 helper expression rules** (`GH_casUpsert` capstone etc.). **Validated:** all 7 write kinds w/ CAS + 409 conflict, **unicode/whitespace lossless**, story-item explode/reassemble/replace, `/changes` feed, activity set→list→clear, blob **put 201 / dedup 200 / versions / GET byte-lossless** (`sha256(decoded)==content_hash`). **Frozen contract v1.0.0 + 12 fixtures + deployment runbook** checked into genesis (`specs/phase-36-genesis-hub-appian-app/contract/`) — the Phase-37 `AppianHubProvider` interface + shared harness. **Key SAIL lesson (do-not-regress):** Web-API `a!writeRecords`/`a!deleteRecords` must be the **terminal** smart service with a **fresh `a!httpResponse()`** in onSuccess/onError (a write in a referenced local silently 500s though it commits); changelog written in the same batch; reads eventually-consistent (trust the PUT version). **No genesis tag** (Appian-side; the contract doc is the only genesis-repo footprint). Optional future: an installable package `.zip` export for portability. As-built: `progress/phase-36-genesis-hub-appian-app.md`; §36-07 hand-off done. **Next: Phase 37 — the AppianHubProvider wire-up (genesis, ~v0.64.0).**

- **2026-09-05 — Phase 36 (Genesis Hub Appian app, ADR-064): FULL API SURFACE BUILT + FROZEN CONTRACT written (finishing).** Continued the Appian-side build in the dev env via `lcp-mcp-server` (genesis read-only — ADR-036/037). **Now built + verified:** the **full API surface** — **11 Web APIs** (GH_meta, GH_records_upsert[PUT, base-version CAS→409/201/200], GH_records_get[404], GH_records_list, GH_changes, GH_activity_set/clear/list, GH_blobs_put/get/versions) + **12 expression rules** (GH_isServiceCaller, GH_errorResponse, GH_appendChangeLog, GH_changesSince, GH_recordToJson[7-kind snake_case projection], GH_casUpsert[CAS capstone + story-item explode], GH_reassembleStoryItems, GH_queryRecords, GH_blobLatest, GH_pruneBlobVersions, GH_recordBlobVersion, GH_convertDocumentToBase64String) + the **base64 blob store** via the user-built `GH Convert Base 64 To Document` process (plugin `base64StringToDocument`; tested text+binary synchronous) + `GH_PM_STORE_BLOB` constant. **`GH_blobs_put` verified end-to-end** (201 new + 200 content-hash dedup). All objects associated with the app. **FROZEN CONTRACT written** — `specs/phase-36-genesis-hub-appian-app/contract/genesis-hub-api.md` (v**1.0.0**; per-endpoint request/response, status table incl. 409-CAS + 200/201-dedup, urlAlias↔path map, auth header, encoding conventions) + **12 adversarial lossless fixtures** `contract/fixtures/*.json` (all valid JSON) + a concrete **`contract/deployment-runbook.md`** (packaging inventory, security grants, service-account provisioning, CSRF/allowed-origins, the blob-put `documentId` activity-chain step, the 2 MB Web-API request-body gate, install/upgrade + per-env checklist). This is the Phase-37 `AppianHubProvider` interface + the shared 36-06 harness. **Remaining (user/admin + validation):** wire the `GH_blobs_put` `documentId` activity-chain (user); grant `GH Service Accounts` Web-API access + CSRF-exempt the write verbs + create `genesis.hub.service` + API key (admin); export the app `.zip` in Designer (no MCP export tool); **36-06** end-to-end round-trip validation per kind + a 2 MB blob (needs the API key). **NOT yet COMPLETE** — packaging export + validation + blob-put wiring remain. No genesis tag (Appian-side; the checked-in contract is the only genesis-repo footprint). See `progress/phase-36-genesis-hub-appian-app.md` + `specs/.../36-08-remaining-work.md`.

- **2026-09-05 — Phase 36 (The Genesis Hub Appian application, ADR-064): BUILD IN PROGRESS (data model complete).** Started the Appian-side build in the team dev environment (`merge-assist-dev.appianpreview.com`, Appian 26.6) via the `lcp-mcp-server` Dev MCP + the installed `appian` skill (genesis stays read-only — ADR-036/037). **Built + verified:** the isolated **`Genesis Hub`** app (prefix GH, appUuid `_a-0000f058-fed5-8000-9bfc-011c48011c48_27312`) + `GH Administrators`/`GH Users`/`GH Service Accounts` groups + `GH KB Blobs`/`GH Artifact Blobs` folders + 9 constants (`GH_CONTRACT_VERSION`="1.0.0", `GH_BLOB_KEEP_LAST_N`=10, 3 group + 2 folder refs); **36-02 COMPLETE** — the **11 record types** (`GH Team`/`Membership`/`Feature`/`Epic`/`Story`/`Story Item`/`Board State`/`Stage Artifact`/`Blob Version`/`Change Log`/`Activity`, all `jdbc/Appian`, PK `id`, `syncUuid` unique, no USER fields [attribution=Text], `description`/`text`=Extra Long Text 64000) + **13 bidirectional relationships** (joined on `syncUuid`) + **record-level security** (viewer=GH Users, data_steward=GH Service Accounts, admin/editor=GH Administrators); **36-04 started** — `GH_isServiceCaller` + `GH_errorResponse` expression rules created, SAIL primitives (`a!httpResponse`/`a!httpHeader`/`a!toJson`/`a!fromJson`) confirmed. **Remaining:** 6 helper rules (`GH_casUpsert`/`GH_appendChangeLog`/`GH_recordToJson`/`GH_changesSince`/`GH_blobDedupAndStore`/`GH_pruneBlobVersions`) + the 12 Web APIs + the **frozen contract doc + fixtures** (not yet written) + 36-05 packaging + service-account provisioning (manual admin step — no create-user MCP tool). **All captured UUIDs + the exact remaining plan are in `progress/phase-36-genesis-hub-appian-app.md` (the durable as-built — READ IT to continue).** No genesis-repo tag from this phase (Appian-side deliverable; the checked-in contract doc is the only genesis-repo footprint). Deviations: reused auto GH Users as read group; blob folders under Artifacts (KC-root 403); contract version 1.0.0 semver (avoid "1.0" coercion); `backlogKey` not reserved `key`; composite uniques enforced in Web-API logic.

- **2026-09-05 — genesis v0.63.0: Phase 35 Collaboration Foundations & Identity SHIPPED (ADR-063 Accepted; amends ADR-026).** The first of the four-phase Team Collaboration program — the **local-first → shared-Hub substrate**, **opt-in / off by default** (a solo install is byte-identical to before). **m0019** (`current_version` 18→19): global **`sync_uuid`** (+ backfill + UNIQUE) on the 6 synced tables (kb_features/kb_feature_stages/kb_epics/kb_stories/workbench_boards/kb_board_cards) + **`row_version`** where missing + owner/team/provenance cols + `upstream_versions_json` on the stage tables + `collab_identity`/`collab_teams`/`collab_memberships`/`collab_sync_state` local tables (additive + idempotent). **`genesis/collab/`**: the pluggable **`SyncProvider`** Protocol + DTOs (no Appian) + a file-backed **`LocalHubProvider`** (base-version CAS→`HubConflict`, content-hash blob dedup, change manifest, advisory TTL, teams) + **`CollaborationService`** (publish/pull + provenance + a `_LOCAL_ONLY` no-leak payload + `collab_sync_state` cursor + advisory + `is_enabled()` gate) + **`build_sync_provider()`** opt-in registry (returns None unless `collab_enabled` + `provider=local`). **Identity**: self-asserted (name + Appian **username** = the canonical user id) via `identity.py`; **`canonical_username()`** threaded into every `LifecycleService.transition(actor=)` site (m0013 audit now attributed) + the `memory_owner_username` consumers. **`api/collab.py`** (config/identity/teams) + a preflight item + a **Settings→Collaboration** page + a first-run **OnboardingDialog** (both gated on the opt-in flag). Exercised end-to-end against the local provider — **no Appian** (the Appian Hub is Phase 36/37). genesis-only (no core/SDK/workflows change). Gates: genesis pytest **730** + ruff; web tsc/eslint0/vitest **247**/build; clean-install migrates a fresh DB to **v19**; live-acceptance (`scripts/acceptance/phase-35-collab-acceptance.py`, two instances) all pass. Commit genesis `95b86e7`, tag **v0.63.0**, pushed master + tag. As-built: `progress/phase-35-collaboration-foundations.md`. **Next: Phase 36 — the Genesis Hub Appian application (ADR-064; separate Dev-MCP agent) — still specs-drafted, awaiting go-ahead.**

- **2026-09-05 — Team Collaboration program: SPECS DRAFTED (Phases 35–38; ADR-063 + ADR-064 Proposed). NOT built — awaiting sign-off.** After a design conversation, drafted the full spec set to make Genesis **team-collaborative** while staying **local-first + single-user + opt-in**: work locally, **publish** finished work products to a shared **Collaboration Hub** (= the team's Appian dev environment — a dedicated **"Genesis Hub" Appian app**), teammates **pull**. **Shared:** application KB (gzipped blob, pull-first, ~1–2 MB for a large app, content-hash dedup, hydrate into the local read cache), features + published stage artifacts (bytes only; chat/run stay local), stories/epics, board status (lane=status + membership shared; in-lane order + sidebar curation local). **Local-only:** chat, runs, catalog, documents, personal memory. **Shared-memory sync deferred.** Pluggable **`SyncProvider` seam** (`genesis/collab/`); publish-on-complete + pull (auto for read-only views + boards; notify-then-apply on a local draft; manual Sync-now). Stable **`sync_uuid`** on synced entities; optimistic **`row_version` CAS + soft advisory "in-progress"** (no hard lock); cross-stage staleness notify-only. **Self-asserted identity** (Appian username = canonical everywhere); **teams** = a Genesis-owned UUID-keyed Hub record; **open visibility** (owner/team_uuid stamped for later); a **Settings identity/team page** + onboarding. **Opt-in / off by default** (solo unchanged). **Four phases:** **35 Foundations & Identity** (genesis, m0019 `current_version` 18→19, the seam + local/mock provider + identity/onboarding + Settings page; ADR-063; suggest v0.63.0) → **36 the Genesis Hub Appian app** (Appian-side, **built by a separate write-capable Dev-MCP agent** — Genesis stays read-only per ADR-036/037 — record types + doc/blob store + Web APIs + service account + package + a **frozen contract** checked into genesis; ADR-064; no genesis tag) → **37 Hub wire-up** (genesis v0.64.0: `AppianHubProvider` vs the frozen contract + KB pull-first sharing + live identity/teams + the sync-UX foundation) → **38 Collaborative SDLC** (genesis v0.65.0: features/artifacts + stories/epics + boards publish/pull + the collaboration UX + one-time adoption bulk-publish). Specs: `specs/phase-35-collaboration-foundations.md` (+ `35-01..07`), `specs/phase-36-genesis-hub-appian-app.md` (+ `36-01..07`), `specs/phase-37-hub-wireup.md` (+ `37-01..06`), `specs/phase-38-collaborative-sdlc.md` (+ `38-01..07`); **ADR-063 + ADR-064** in `reference/decision-log.md`; bible/08 §9 SPECS-DRAFTED block. **Refinements (after grounding in the Appian skill + docs):** the local Genesis model stays **JSON** (no local normalization); a story's variable-length lists are **normalized into a `GH Story Item` child record type on the Hub side only** (Web API explodes/reassembles), long text uses Appian **`Extra Long Text` (64000)** (synced text tiers 255/4000/64000, ≤3 Extra-Long per record type), the KB blob is an **Appian Document + a `GH Blob Version` index** — and **lossless sync is a hard invariant** with a round-trip fidelity gate. **No implementation until the user says go.**

- **2026-09-05 — genesis v0.62.0 + genesis-workflows v0.16.0: Phase 34 Design Lane Automation SHIPPED (ADR-062 Accepted; amends ADR-061).** The FIRST Workbench per-lane automation, for **Design + Design Review**. Dragging a ticket into **Design** (from any lane) opens a confirm dialog → **Yes** moves it + launches (or **re-runs in place**) a grounded **`story-design-analysis`** ("Appian Ticket Design") run; **No** just moves it. The workflow writes an **object-level, code-level** `design.html` (one section per object, NEW/UPDATE tag + the ACTUAL code; **process models node-by-node**) — gather ticket research → per-object grounded design loop (@genesis-kb + @appian-dev, read-only) → deterministic assemble → grounded verify critic; reliability trio per agent. A **running** card is **locked** + shows a run link; a **failed** run paints it light-red. On success the card **auto-advances to Design Review** via an **audited** `LifecycleService` transition (m0013 — the first). A **Design Review** card opens a full-page workspace (reused `ChatThread` + annotatable **Lavish** preview → the agent revises `design.html`). Backend **m0018** (`kb_story_stages` + `kb_story_stage_revisions`; `current_version` 17→18) + `StoryStageStore` + `POST …/cards/{story_id}/design/start` (fail-fast 409 on no dev env / app not synced / workflow not installed) + `StoryDesignFinalizer` (bound-run guard + idempotency + on-read recovery) + a `story_design` chat mode; the board-card DTO derives running/failed from the durable `runs.status`. **Removed** the superseded Phase-15 `design-doc` (repo + running app). A post-build fix switched the board to a pointer-first collision so drops land on every lane + the hover highlight returns. Multi-repo (genesis + genesis-workflows; core/SDK/parser unchanged). Gates: genesis pytest **710** + ruff; genesis-workflows validate_library **12** + pytest **179**; web tsc/eslint0/vitest **239**/build; clean-install migrates a fresh DB to v18. Commits genesis `6d987a7`→`66b4585` (release), tag **v0.62.0**; genesis-workflows `9a9d3ab`/`245d769`/`006d5b4` (release), tag **v0.16.0**; both pushed, CI green. As-built: `progress/phase-34-design-lane-automation.md`. **No active phase.**

- **2026-09-04 — Phase 34 (Design Lane Automation) SPECS DRAFTED (ADR-062 Proposed; amends ADR-061). NOT BUILT — awaiting user sign-off.** The first per-lane Workbench automation, for **Design + Design Review**. Dragging a ticket into **Design** (from any lane) opens a confirm dialog → **Yes** moves it + launches (or **re-runs** in place) a grounded **`story-design-analysis`** ("Appian Ticket Design") workflow; **No** just moves it. The workflow takes one story + its feature's finalized Spec/UX/Technical-Design, gathers ticket-relevant research, grounds against the live app (`@genesis-kb` structure + `@appian-dev` actual code, read-only), and writes an **object-level design HTML** (one `<section>` per object, NEW/UPDATE tag, granular change + **actual code**; process models per-node with code, not summarized) — reliability trio + grounded critic + deterministic assemble (the Phase-30 lesson). Running card = **locked** + "Design generation running" + run link; success → **auto-advance to Design Review** via an **audited** `LifecycleService` transition (m0013 — the first); failure → **light-red** card + run link. A Design Review card opens a **full-page** reused stage workspace (chat + annotatable Lavish preview → agent revises the design). Data: **m0018** `kb_story_stages` (+ revisions) — the per-story analog of m0015; `current_version` 17→18; a `StoryStageStore` + `StoryDesignFinalizer` (bound-run guard + idempotency + on-read recovery). Also **removes** the superseded `design-doc` workflow (library + running app). Multi-repo (genesis + genesis-workflows). Specs: `specs/phase-34-design-lane-automation.md` + `phase-34-design-lane-automation/34-01..34-07`; ADR-062 in `reference/decision-log.md`; bible/08 §9 block. Locked decisions (the 10 planning answers) recorded in the umbrella §3. **Suggested release: genesis v0.62.0 + genesis-workflows v0.16.0. Do not build until the user approves.**

- **2026-09-04 — genesis v0.61.0: Phase 33 The Workbench SHIPPED (ADR-061 Accepted; amends ADR-049).** A new top-level **Workbench** nav surface holding one **Jira-style Kanban board per application** over the app's finalized stories (ADR-060, aggregated across its features), eight fixed lanes (**To Do → Design → Design Review → Implementation → Code Review → Verification → Deployment → Done**); a card's lane IS its `kb_stories.status`. Boards are **curated** — added/removed as **left-sidebar sub-tabs** under Workbench (a `+` beside the label adds applications, from any tracked app). Stories are **imported in batches** (→ To Do; never auto-loaded); cards **drag across lanes AND reorder within a lane** (`@dnd-kit` multiple-containers) persisted via a bulk `PATCH /workbench/boards/{app}/lanes/{lane}` reorder endpoint (sets each card's `board_position`=index + `status`=lane in one tx); a card **drawer** (reuses the Phase-32 detail) + **remove-from-board**. **This phase = the board UI/UX + import + drag/reorder ONLY** — the per-lane automation (drag into Design → run the design workflow, etc.) is a future phase this surface plugs into. Backend **m0017** (`workbench_boards` + `kb_board_cards`; `current_version` 16→17) + `kb/boards.py::BoardStore` + `api/workbench.py`; `LifecycleState` += `to-do`/`design-review` + `STORY_LANES`; the `STORY_STAGE` table reconciled (forward-compat, **not enforced on drag** — a recorded ADR-050 relaxation). Genesis-only + m0017 + **`@dnd-kit`** (core 6.3.1 / sortable 10.0.0 / utilities 3.2.2, pinned). Reference: the user's Jira board screenshots (`~/Documents/GSS/workbench-examples`). Iterative UX fixes applied on user feedback (boards → sidebar sub-tabs; slimmer board page; card-title truncation; lane contrast; then cross-lane drag; then intra-lane reorder). Gates: genesis pytest **701** (+17) + ruff; web tsc/eslint0/vitest **237**/build; clean-install migrates a fresh DB to v17. Commits `5c81284`→`2992be8` (release), tag **v0.61.0**, pushed master + tag. As-built: `progress/phase-33-workbench.md`. **No active phase.**

- **2026-09-04 — Phase 33 (The Workbench) SPECS DRAFTED (ADR-061 Proposed; NOT built).** After Phase 32 shipped (v0.60.0), the user described the next major feature: a place where the actual SDLC work happens. Drafted the umbrella `specs/phase-33-workbench.md` + `33-01..33-05` + **ADR-061 (Proposed)**. **Design:** a new top-level **Workbench** nav item holding one **Jira-style Kanban board per application** over the app's finalized stories (ADR-060, aggregated across all its features), with eight fixed lanes — **To Do → Design → Design Review → Implementation → Code Review → Verification → Deployment → Done** — where a card's **lane IS its `kb_stories.status`**. Boards are **curated** (empty Workbench → "Add applications" → pick any Genesis app → each gets a board + sub-tab; removable). Stories are **explicitly imported in batches** (never auto-loaded) → land in **To Do**. Cards **drag** between lanes (`@dnd-kit`) and a drop **persists the lane** (`row_version` CAS); **free movement** this phase. Remove-from-board keeps the story (stays in the feature's Stories tab, the CRUD surface); card click → a drawer reusing the Phase-32 detail. **This phase = board UI/UX + import + drag-persist ONLY** — the per-lane automation (drag into Design → run the design workflow, etc.) is future phases. **m0017:** `workbench_boards` (app→board registry, ON DELETE CASCADE) + `kb_board_cards` (membership + `board_position`; no lane column); `current_version` 16→17. `LifecycleState` += `to-do`/`design-review`; `STORY_STAGE_TRANSITIONS` reconciled (deployment after verification) — forward-compat, **not enforced on drag** (a recorded ADR-050 relaxation). Amends ADR-049 (nav); adds `@dnd-kit`. Reference: the user's Jira board screenshots (`~/Documents/GSS/workbench-examples`). **Locked with the user (2026-09-04):** 8 lanes; lane = status; import→To Do; free drag; curated one-board-per-app; remove-from-board keeps the story; `@dnd-kit`; card drawer; flat lanes; genesis-only + m0017. Single-repo (genesis); no core/SDK/workflows change. **Awaiting user sign-off on the specs before building (33-01 gate).**

- **2026-09-04 — genesis v0.60.0: Phase 32 Finalize Stories SHIPPED (ADR-060 Accepted).** Once a feature's Feature Breakdown stage is **completed**, a one-time, irreversible **Finalize Stories** action (header button replacing "Back to application" + a warning dialog) parses the canonical backlog embedded in `breakdown.html` and persists every epic + story/task into new **`kb_epics`/`kb_stories`** (m0016; `kb_features.stories_finalized_at` one-time marker; `current_version` 15→16) via `kb/stories.py::StoryStore`; API `POST /features/{id}/stages/breakdown/finalize` (409 unless completed + not finalized; 422 on empty) + story CRUD (`row_version` CAS). The reserved **Stories tab** is now a filterable/paginated grid (Epic·Type·Title, colored tags, grouped by epic) + a routed `StoryDetailPage` with story-level add/edit/delete (parent-epic dropdown); domain `Story`/`Epic` promoted to live. Also shipped: a themed `shared/ui` **`Select`** (Radix DropdownMenu) replacing native selects in the Stories + Runs filters, and **run-detail graph UX fixes** (live executed path + a refresh control, readable ×N, standard lucide icons, a path-color legend). Genesis-only + m0016; no core/SDK/workflows change. Gates: genesis pytest **684** + ruff; web tsc/eslint0/vitest **232**/build; clean-install migrates a fresh DB to v16. Commits `f4ca84a` (graph fixes) → `0de9b61` (release), tag **v0.60.0**, pushed master + tag. **Parked (user, revisit before a future change):** normalizing the AC/questions/labels JSON columns — kept JSON for v1 (lists always read/written whole, no cross-story queries). **Live acceptance** (finalize → grid → edit/add/delete) is user-driven.

- **2026-09-04 — Phase 32 (Finalize Stories) SPECS DRAFTED (ADR-060 Proposed; NOT built).** After Phase 31 shipped + the run-graph fixes, the user asked for the next step: once the Feature Breakdown is **completed**, a one-time **Finalize Stories** action lifts the file-only backlog into first-class Genesis data. Drafted the umbrella `specs/phase-32-finalize-stories.md` + `32-01..32-05` + **ADR-060 (Proposed)**. **Locked with the user:** name "Finalize Stories"; a header button near the feature name replacing "Back to application" + a warning confirm dialog; parse the canonical backlog embedded in the finalized `breakdown.html` (`_parse_embedded_backlog`) → persist epics + stories into new **`kb_epics`/`kb_stories`** (m0016; `kb_features.stories_finalized_at` = the one-time, irreversible marker; `current_version` 15→16); the reserved **Stories tab** becomes a formatted grid (Title · Type · Epic · Category · Appian part) with a routed detail page + **story-level CRUD** (add/edit/delete; parent reassignment from an epic dropdown; `row_version` CAS); epics fixed at finalize (no epic CRUD); a forward-compat story `status` default `design` (not surfaced); **no re-sync** after finalize. Realizes the ADR-056 reserved slot + the ADR-050/25-11 `Story` domain. Single-repo (genesis) + m0016; no core/SDK/workflows change. **Awaiting user sign-off on the specs before building (32-01 gate).**
