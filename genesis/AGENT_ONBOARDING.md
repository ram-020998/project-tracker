...
> §5 (ADRs), §7 (lessons), and §9 (roadmap). **Last refreshed: 2026-08-17 — latest SHIPPED: genesis v0.48.4 +
> genesis-workflows v0.9.4 +
> genesis-core v0.9.3 + kiro-agent-sdk v0.7.0 + genesis-appian-parser v0.2.0** (Phases 9 Agent-Artifact-I/O,
...
## 2. Current state (as of genesis v0.48.0)
...
**Tests, all green at last release (as of genesis v0.48.4):** genesis **474** pytest (Phase-23 **m0012 `scheduled_jobs`** + scheduler `due_slot`/`ScheduleStore`/tick-engine + `application-sync`/`document-library-sync` handlers + `/system/schedules` + full-package refresh [delta/refresh/auto + 409 guard]; **plus the v0.48.1–v0.48.4 patches: updater dev-guard + whole-stack apply, Dev-MCP `isError`/pagination + `LCP_API_PATH`, SPA-packaging resolver**; on top of the Distribution/Features/KB/Business-Map/Documents suites) · genesis-core **65** (incl. managed-native MCP **and** CLI launch resolution) · kiro-agent-sdk
**93** · genesis-appian-parser **25** (vs a real vendored package + a no-SAIL guard + a raw-XML accuracy oracle with a ≥95% gate) · genesis-workflows **75** (incl. 13 code-review + 16 design-doc + 12 sync-application + 11 generate-business-map + **7 sync-documents**) + `ci/validate_skills.py` + `ci/validate_library.py` (7 workflows) · web **161** Vitest
