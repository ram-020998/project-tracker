...
**Tests, all green at last release (Phase 23, v0.48.0):** genesis **464** pytest (adds Phase-23 **m0012 `scheduled_jobs`** + the scheduler `due_slot`/`ScheduleStore`/tick-engine tests + the `application-sync`/`document-library-sync` handler tests + the `/system/schedules` endpoint + the full-package refresh [delta/refresh/auto + 409 guard], on top of the Distribution/Features/KB/Business-Map/Documents suites) · genesis-core **65** (incl. managed-native MCP **and** CLI launch resolution) · kiro-agent-sdk
