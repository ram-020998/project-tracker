...
>
> **Last refreshed: 2026-09-05 — latest SHIPPED: genesis v0.63.0 — **Phase 35 Collaboration Foundations & Identity COMPLETE** (the local-first → shared-Hub substrate, **opt-in/off-by-default** — a solo install is unchanged: **m0019** `sync_uuid`/`row_version` backfill + owner/team/provenance + `collab_*` tables [`current_version` 18→19]; **`genesis/collab/`** — the pluggable `SyncProvider` seam + file-backed `LocalHubProvider` + `CollaborationService` [publish/pull + provenance + a no-leak payload] + `build_sync_provider()` opt-in registry; **self-asserted identity** + `canonical_username()` threaded into `LifecycleService.transition(actor=)`; a **Settings→Collaboration** page + first-run onboarding; ADR-063 **Accepted**). 🟡 **NEXT (specs drafted):** Phase 36 — the **Genesis Hub Appian application** (ADR-064 Proposed; built by a separate write-capable Dev-MCP agent), then 37 Hub wire-up + 38 Collaborative SDLC (see §9). Prior: genesis v0.62.0 + genesis-workflows v0.16.0 — **Phase 34 Design Lane Automation COMPLETE** (the first Workbench per-lane automation — drag a ticket into **Design** → a grounded `story-design-analysis` run writes an object-level, code-level design → the card auto-advances to **Design Review** for a chat + annotatable review; m0018; ADR-062; removed the superseded `design-doc`). Prior: **Phase 33 The Workbench COMPLETE** (a per-application Kanban board over finalized stories — curated boards as left-sidebar sub-tabs, batched import → To Do, drag across 8 lanes + intra-lane reorder; m0017; ADR-061). Prior: **Phase 32 Finalize Stories** (m0016; ADR-060) + genesis-core v0.9.6 +
