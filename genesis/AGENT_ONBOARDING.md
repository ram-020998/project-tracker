...
> §5 (ADRs), §7 (lessons), and §9 (roadmap). **Last refreshed: 2026-08-17 — latest SHIPPED: genesis v0.48.3 +
...
genesis/genesis/
  ...
  api/      app.py (create_app FastAPI; version 0.48.3; **`_resolve_web_static` serves the SPA from packaged `genesis/web_static` [installed wheel] with a repo `web/static` fallback [dev]**; instantiates ChatManager + ChatRunSupervisor + SkillService + **Scheduler (Phase 23)**; registers chat/copilot/skills + **native-mcp** + **applications** + **documents** + **features** + **system** + **schedules** routes + per-session skill-output endpoints).
