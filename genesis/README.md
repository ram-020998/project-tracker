...
| 33 | [phase-33](specs/phase-33-workbench.md) | The Workbench (a per-application Kanban execution board) | ✅ SHIPPED — genesis **v0.61.0** (33-01..33-05), CI green (#6738990 master / #6738991 tag). **ADR-061 (Accepted; amends ADR-049 nav)**. A new top-level **Workbench** nav surface holding one **Jira-style Kanban board per application** over the app's finalized stories (aggregated across its features), **eight fixed lanes** (To Do → Design → Design Review → Implementation → Code Review → Verification → Deployment → Done); a card's **lane IS its `kb_stories.status`**. Boards are **curated** — added/removed as **left-sidebar sub-tabs** under Workbench (a `+` beside the label adds applications). Stories are **imported in batches** (→ To Do; never auto-loaded); cards **drag across lanes AND reorder within a lane** (`@dnd-kit` multiple-containers) persisted via a bulk `PATCH …/lanes/{lane}` reorder endpoint (sets each card's `board_position` + `status`); a card **drawer** (reuses the Phase-32 detail) + **remove-from-board**. Backend **m0017** (`workbench_boards` + `kb_board_cards`; `current_version` 16→17) + `kb/boards.py::BoardStore` + `api/workbench.py`; `LifecycleState` += `to-do`/`design-review` + `STORY_LANES`. **UI-only board this phase — per-lane automation is a future phase.** **Genesis-only + m0017 + `@dnd-kit`.** |
