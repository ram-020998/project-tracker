...
| 29 | [phase-29](specs/phase-29-ux-design-stage.md) | UX Design Stage (mockup → grounded implementation analysis) | ✅ SHIPPED — genesis **v0.55.1** + genesis-workflows **v0.12.0** + genesis-core **v0.9.6** + kiro-agent-sdk **v0.7.1** (29-01..29-06), CI green. ADR-057. Upload a mockup **PDF** → the **`ux-design-analysis`** workflow (PyMuPDF render → per-screen **multimodal** analysis → grounding [genesis-kb structure + appian-dev code] → synthesize → **grounded verify** critic) → a **StageFinalizer** opens the **`ux_design`** completion chat + sets the stage in-review → annotatable review + Mark complete. Reuses the Spec-page components **generalized** (D0); Spec repointed onto **m0015** `kb_feature_stages`/StageStore (Decision A). `current_version`→15 |
