...
| 20 | [phase-20](specs/phase-20-features-and-spec-authoring.md) | Features & Spec Authoring (per-app Features + a conversationally-authored, annotatable HTML Spec) | 📋 SPEC DRAFT (20-01..20-06); awaiting build. m0010 `kb_features`/`kb_feature_specs`/`kb_feature_spec_revisions` + `FeatureStore`; Features tab + feature page; spec = a reused Chat (genesis-kb + business-artifact context injection); HTML-authoritative spec authored in the fs sandbox; embedded **vendored Lavish annotation SDK** (MIT) → annotation-to-chat bridge; status draft→in-progress→in-review→completed + milestones + MD export. Mostly **genesis**. ADR-042/043 (Proposed) |
