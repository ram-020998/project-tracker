...
1. **3 helper expression rules** (36-04 §B):
   - **`GH_casUpsert(kind, syncUuid, payload, baseVersion, actor)`** — the capstone. `a!match(ri!kind)` dispatch (audit fields differ per kind — Team has createdBy/createdAt/updatedAt; Membership has none; Feature/Epic/Story have createdBy/modifiedBy/createdAt/updatedAt; Board State has modifiedBy/updatedAt; Stage Artifact has publishedBy/publishedAt/createdAt/updatedAt). Per branch: query current (id+version+createdBy+createdAt) by syncUuid → `found`/`currentVersion` → if `found and currentVersion<>baseVersion` return `a!map(status:"conflict", current_version)` else `a!writeRecords` the kind's constructor with `id: if(found, currentId, null)` (null id = insert), `version: coalesce(baseVersion,-1)+1`, `modifiedBy/updatedAt=now()/actor`, `createdBy: a!defaultValue(currentCreatedBy, actor)`, `createdAt: a!defaultValue(currentCreatedAt, now())`, mapping payload snake_case keys → fields → `rule!GH_appendChangeLog(kind, syncUuid, nv, actor)` → `a!map(status: if(found,"updated","created"), version: nv)`. **For `kind='story'`:** after the story write, `a!deleteRecords` existing `GH Story Item` (filter storySyncUuid=syncUuid) then `a!writeRecords` new items from `payload.acceptance_criteria`/`questions`/`labels` — synthesize each child `syncUuid = <storyUuid>:<itemType>:<position>` (satisfies unique(sync_uuid) + unique(story_sync_uuid,item_type,position)), `itemType∈{ac,question,label}`, `position=fv!index`, `text=fv!item`. Validate via createExpressionRule testInputs (kind="feature", found=false create path; story branch checked at discovery).
   - **`GH_pruneBlobVersions(kind, key)`** — query GH Blob Version for key sort version desc; `a!deleteRecords` rows beyond `cons!GH_BLOB_KEEP_LAST_N` (+ delete their Documents — see the OPEN QUESTION).
   - **`GH_blobDedupAndStore(kind, key, bytesB64, contentHash, actor)`** — latest GH Blob Version for key; equal hash → `{status:"unchanged",version}`; else **create/upload an Appian Document** (see OPEN QUESTION) + insert GH Blob Version (version=latest+1, documentId, size, contentHash) + `rule!GH_appendChangeLog('blob',key,version,actor)` + `rule!GH_pruneBlobVersions`.
   - **⚠️ OPEN QUESTION (blob Documents, 36-03):** `a!writeRecords`/`a!deleteRecords` cannot create/upload/delete an Appian **Document** (bytes). Confirm the mechanism on 26.6 before building the two blob rules: a document-write **smart service** is process-only (would need a process model — the spec's "no process models" is a *default*, not a hard rule; 36-03 explicitly allows a process model for blob storage/retention if inline isn't possible). Options: (a) a small `GH_storeBlob` **process model** (Create Document / Save-to-file smart service) called via `a!startProcess` from `GH_blobs_put`; (b) a document-creating function if one exists in this env. **Resolve this first for the blob path**; the records + changes + activity paths need no Documents and can be finished independently.
2. **5 Web APIs remaining** (36-04 §C) — pattern proven; each auth-gates + parses `http!request`; add write verbs to allowed origins. **✅ DONE: GH_meta, GH_records_upsert, GH_changes, GH_activity_set, GH_activity_clear, GH_activity_list.** Remaining:
   - **`GH_records_get`** (GET, alias e.g. `records_get`; kind+syncUuid from pathSegments) — query the kind's RT by syncUuid (7-branch `a!match`, mirrors GH_casUpsert's current-query), `rule!GH_recordToJson(kind,row)`, 404 if none. **For `kind='story'` merge in `acceptance_criteria`/`questions`/`labels`** by querying `GH Story Item` (filter storySyncUuid, sort itemType+position) grouped by itemType — build a small helper `GH_reassembleStoryItems(storySyncUuid)`.
   - **`GH_records_list`** (GET, alias e.g. `records_list`; kind from pathSegments, `?since=&limit=` from queryParameters) — query the kind's RT (7-branch) with paging (optionally filter changed-since via `GH Change Log` join) → `{records:[GH_recordToJson…], next_cursor}`; story rows reassembled.
   - **`GH_blobs_put`/`GH_blobs_get`/`GH_blobs_versions`** — BLOCKED on the blob Document mechanism (OPEN QUESTION above). Resolve the Document create/upload path first (likely a small `GH_storeBlob` process model + `a!startProcess`, or a doc-create function), then build `GH_blobDedupAndStore`/`GH_pruneBlobVersions` + these three.
   - urlAlias uniqueness: each Web API needs a distinct alias (can't reuse `records`/`activity`); Genesis's provider maps contract paths → aliases. Finalize the alias↔path map in the contract doc.
