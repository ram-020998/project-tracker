...
2. **The FROZEN CONTRACT (36-01 §3):** write `specs/phase-36-genesis-hub-appian-app/contract/genesis-hub-api.md` + `contract/fixtures/*.json` into project-tracker (normative request/response per endpoint, status codes incl. 409-CAS + 200/201-dedup, pagination, auth header, `contract_version`="1.0.0", **and the urlAlias↔path map** — see below). Phase-37 interface + shared 36-06 harness. **Not yet written.**
3. **36-05 packaging + security:** export the Genesis Hub app as an installable package + install/upgrade runbook; document **service-account provisioning** (MANUAL admin step — create `genesis.hub.service`, add to `GH Service Accounts`, mint API key); confirm **allowed-origins/CSRF** for the write verbs (PUT `records`, POST `blobs`/`activity`, DELETE `activity_clear`) + the **Web API max request-body size** on 26.6 (the real 2MB-blob gate). 
4. **36-06 contract validation** (end-to-end round-trips per kind + blob, incl. the 2MB blob) → **36-07 hand-off** (contract doc into genesis; bible/tracker/ADR-064 updates).
