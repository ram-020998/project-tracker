...
## 🔨 IN PROGRESS — 36-04 (constants ✅ / expression rules / Web APIs)
**Expression rules created (2 of 8):**
- **GH_isServiceCaller** `_a-...27478` — `a!isUserMemberOfGroup(loggedInUser(), cons!GH_GROUP_SERVICE)` (no inputs).
- **GH_errorResponse(code:Integer, message:Text)** `_a-...27482` — `a!httpResponse(statusCode: ri!code, headers: {a!httpHeader(name:"Content-Type", value:"application/json")}, body: a!toJson(a!map(error: ri!message, code: ri!code)))`.

**SAIL primitives CONFIRMED valid in this env:** `a!httpResponse`, `a!httpHeader`, `a!toJson`, `a!fromJson`, `a!isUserMemberOfGroup`, `a!map`, `loggedInUser()`. NOTE: `validateExpression` can't evaluate `a!toJson` (no TypeService) or resolve `ri!`/context — validate list/query/errorResponse logic by wrapping in `a!localVariables` with test values, and lean on `createExpressionRule` + `testRule` for the ri!/toJson rules.

## ⬜ REMAINING (do next)
1. **6 helper expression rules** (36-04 §B): `GH_appendChangeLog(kind,syncUuid,version,actor)` (writeRecords a GH Change Log row) · `GH_recordToJson(kind, record)` (project a row → contract JSON field names) · `GH_casUpsert(kind,syncUuid,payloadMap,baseVersion,actor)` (the shared CAS write: query current by kind+syncUuid → if `current.version != baseVersion` return `{status:"conflict", current_version}` else a!writeRecords with `version=coalesce(baseVersion,-1)+1` + set modifiedBy/updatedAt (+createdBy/createdAt on create) → GH_appendChangeLog; **for kind='story' also explode acceptance_criteria/questions/labels arrays into GH Story Item rows [delete-for-story + insert current] in the same write**) · `GH_changesSince(cursor,limit)` (a!queryRecordType GH Change Log filter id>cursor sort id asc) · `GH_blobDedupAndStore(kind,key,bytesB64,contentHash,actor)` (latest GH Blob Version for key; if contentHash equal → unchanged; else create/upload an Appian Document into GH_FOLDER_KB_BLOBS/GH_FOLDER_ARTIFACT_BLOBS + insert GH Blob Version row version=latest+1 + GH_appendChangeLog('blob',...) + GH_pruneBlobVersions) · `GH_pruneBlobVersions(kind,key)` (delete GH Blob Version rows + Documents beyond GH_BLOB_KEEP_LAST_N).
2. **12 Web APIs** (36-04 §C) — each begins by asserting `GH_isServiceCaller()` else `GH_errorResponse(401,...)`; add write verbs to allowed origins (CSRF exemption): `GH_meta` (GET /meta) · `GH_records_upsert` (PUT /records/{kind}/{syncUuid}; CAS→409; kind∈team|membership|feature|epic|story|board_state|stage_artifact; story explodes/reassembles GH Story Item) · `GH_records_list` (GET /records/{kind}?since=&limit=) · `GH_records_get` (GET /records/{kind}/{syncUuid}; story reassembles arrays) · `GH_blobs_put` (POST /blobs/{kind}/{key}) · `GH_blobs_get` (GET /blobs/{kind}/{key}) · `GH_blobs_versions` (GET /blobs/{kind}/{key}/versions) · `GH_changes` (GET /changes?cursor=&limit=) · `GH_activity_set` (POST /activity) · `GH_activity_clear` (DELETE /activity/{kind}/{syncUuid}) · `GH_activity_list` (GET /activity/{kind}) · optional `GH_records_delete`.
