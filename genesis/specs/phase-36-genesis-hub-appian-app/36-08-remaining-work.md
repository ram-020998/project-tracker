...
### C. The FROZEN CONTRACT + fixtures (project-tracker; the Phase-37 interface)
- Write `specs/phase-36-genesis-hub-appian-app/contract/genesis-hub-api.md`: per-endpoint request/response JSON, status codes (incl. **409-CAS** + **200/201-dedup**), pagination, the **auth header**, `contract_version="1.0.0"`, and the **urlAlias↔path map** (below).
- Write `contract/fixtures/*.json`: adversarial **lossless** samples per kind (multi-KB unicode/emoji `description`, many/long AC, empty arrays, nulls) — shared with Phase 37's provider tests + the 36-06 harness.
