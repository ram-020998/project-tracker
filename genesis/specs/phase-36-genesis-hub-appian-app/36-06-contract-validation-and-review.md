...
2. **Round-trip fidelity (no data loss) — MANDATORY (§2.0):** an adversarial story fixture — a **long (multi-KB)
   description** (in an `Extra Long Text` field), **many long AC**, **unicode/emoji**, **empty `labels`**, a
   **null `epic`** — is `PUT` then `GET`; the response must equal the input **exactly**
   (content/order/count/empty-vs-null/unicode), proving the `GH Story Item` explode/reassemble + the text sizing
   are lossless. Same for a feature/epic with a long description. This fixture is shared with the Genesis-side
   round-trip test (38-02).
