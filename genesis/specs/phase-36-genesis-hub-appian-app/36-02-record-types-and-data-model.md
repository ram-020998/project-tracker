...
Build the **10 record types** (§2 of 36-01) on the Hub data source, with exact fields/types/keys, the
...
fields — attribution is Text; UUIDs sourced from the environment, never fabricated).
...
1. **Data source / tables.** Create the backing DB tables on the Hub business data source (skill:
   ...
   `gh_board_state`, `gh_activity`, `gh_blob_version`). (If using Appian-managed data-store entities, mirror the
   same structure.)
2. **Record types (create in this order):** `GH Team` → `GH Membership` → `GH Feature` → `GH Epic` → `GH Story`
   → `GH Board State` → `GH Stage Artifact` → `GH Blob Version` → `GH Change Log` → `GH Activity`. For each:
   map every field to its column with the Appian field type from §2 (Text / Integer / "Date and Time" / CLOB→
   Text); set the PK to `id`; **do not** add USER fields (attribution stays Text).
...
3. **Relationships (§2.11, after all record types exist):** declare **both** sides for each —
   ...
   `GH Story`↔`GH Stage Artifact`, `GH Team`↔`GH Membership` — joining on the `syncUuid`/`*SyncUuid` fields
   (skill: `relationship-patterns.md` — MANY_TO_ONE + ONE_TO_MANY, or the tool will leave the reverse missing).
...
The 10 record types + their tables + relationships + record-level security, matching 36-01 §2, with their UUIDs
captured for 36-04.
