...
| `GH_records_upsert` | PUT `/records/{kind}/{syncUuid}` | parse body (payload + `base_version` + `actor`); `GH_casUpsert(...)`; on `conflict` → `GH_errorResponse(409, "conflict", {current_version})`; else `200`(update)/`201`(create) + the record JSON. Valid `kind` ∈ {team, membership, feature, epic, story, board_state, stage_artifact} (400 otherwise). **For `kind='story'`:** the payload carries `acceptance_criteria`/`questions`/`labels` **arrays** → the rule **explodes** them into `GH Story Item` rows (delete-for-story + insert current) in the same write (the 4000-char normalization; §2.5b). |
| `GH_records_get` (story reassembly) | — | for `kind='story'`, `GH_records_get`/`GH_records_list` **reassemble** the `GH Story Item` rows back into the `acceptance_criteria`/`questions`/`labels` **arrays** in the response — so Genesis sees arrays, unaware of the Appian-side normalization. |
