...
- **Palette (light-first, FINAL — user chose Indigo·Slate, 2026-08-21):** cool neutral canvas `--bg #f4f6fa`, `--surface-1 #ffffff`, `--fg #111726`, `--fg-muted #48506a`, `--fg-subtle #5b6480` (AA); **PRIMARY `--primary #4F46E5` (indigo)** + **SECONDARY `--secondary #475569` (slate)** used consistently for all brand/decorative elements. Semantic `success #15803d` / `warning #a16207` / `danger #b91c1c` reserved for status/deltas (`--info` folds into the slate secondary). *(Earlier explored: Amber·Stone, Teal, Violet, Blue, Rose, Emerald — retained as selectable options in the mockup's swatch picker.)*
- **⭐ Single-source configurable theming (user requirement):** brand is a **one-place** token block — set `--primary` / `--secondary` and everything follows. Gradients (`--grad-brand`/`-soft`) + the live-pulse glow are **derived from `--primary` via `color-mix()`**, so changing the single value re-themes gradients + accents too (proven live in `/dev/mockups` — the swatch picker only sets `--primary`/`--secondary`). No hardcoded brand hex in components; 27-04 carries this into `styles/tokens.css` with a grep/lint guard + a "How to re-theme" note.
- **Type:** **Poppins** (geometric-modern) as the UI font (loaded via the scoped CSS; falls back to Inter).
- **Side-nav differentiation:** the nav is an **elevated white panel** (`.mock-sidebar` — subtle vertical gradient + a soft right shadow) that reads distinct from the page canvas and the white cards.
