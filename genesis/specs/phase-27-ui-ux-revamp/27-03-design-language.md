...
- **Palette (light-first, one primary + one secondary — user feedback 2026-08-21):** warm-gray page canvas `--bg #f2f1ef`, `--surface-1 #ffffff`, warm near-black `--fg #1c1917`; **PRIMARY `--primary #C2410C` (burnt orange)** + **SECONDARY `--secondary #57534E` (stone; doubles as `--fg-muted`)** used consistently for all brand/decorative elements; an orange `--grad-brand`. Semantic `success #15803d` / `warning #a16207` / `danger #b91c1c` reserved for status/deltas/health (`--info` folds into the stone secondary to keep the palette to two brand hues).
- **Type:** **Poppins** (geometric-modern) as the UI font (loaded via the scoped CSS; falls back to Inter).
- **Side-nav differentiation:** the nav is an **elevated white panel** (`.mock-sidebar` — subtle vertical gradient + a soft right shadow) that reads distinct from the warm-gray page and the white cards.
