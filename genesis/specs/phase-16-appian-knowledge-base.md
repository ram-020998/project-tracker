...
- **ADR-038 — Managed native Appian MCP servers (Accepted — SHIPPED 16-08).** The Dev MCP (`lcp-mcp-server`) + DevOps MCP
  ...
  dropping in a new bundle → Genesis installs it as a new version + swaps `current` (prior kept for rollback); no
  auto-fetch source (2026-08-05 decision); the bundle is never modified.
