...
| **Feature Impact Analysis** | `artifacts/07_APPLICATION_IMPACT_ANALYSIS.md` — application-wide impact of the hidden-child model: where a child-round evaluation leaks (task grids, emails, record list, consensus, documents) or distorts aggregations (counts, Process-HQ), graded by evidence, with a Q1–Q15 open-questions register. The *what breaks in the app* companion to 05/06. |
| **Implementation Backlog** | `artifacts/08_IMPLEMENTATION_BACKLOG.md` — the feature broken into **sequenced stories (US-n) + tasks (T-n)** in build order (epics A–J), with acceptance criteria/DoD, dependencies, and a build-order summary. The *what to build, in what order* companion; derived from 01/05/06/07. |
...
### 10.50 Carry factor→document mappings to a new round (criteria pre-write approach)
When a new round is created, carried vendor documents must also carry their **factor↔document mappings** (`AS_GSS_FactorDocumentMapping_SYNCEDRECORD` `acd503e1`). The mapping has two FKs — `documentId` (`a048579b`→EvaluationDocument) and `factorId` (`a5c1fdd5`→Criteria) — both of which are **DB-generated during the round write**, so they can't be set inside `AS_GSS_UT_duplicateEvaluationForNewRound` (which builds the graph pre-write). `Criteria` has **no** ONE_TO_MANY to FactorDocumentMapping, so the mapping can't be nested under the new factor either.

**Chosen approach (PO-approved "criteria pre-write"):** the Setup-New-Round PM `000bf04a-37b2-8000-fbec-7f0000014e7a` already **writes the criteria in node 6 "Write Evaluation"** and returns the persisted records (with new `criteriaId`s) into `pv!newEvaluation`. So the criteria are written *before* the mapping step; we resolve the new ids and write the mappings right after.

- **NEW rule (built + verified):** `AS_GSS_UT_constructFactorDocumentMappingsForNewRound` = `_a-0000f04c-8a45-8000-9bac-011c48011c48_83697` (inputs `newEvaluationId`, `sourceEvaluationId`, `initiator`). Correlates **factor by `factorNumber`** (source `criteriaId`→`factorNumber`→new `criteriaId`) and **document by `appianDocId`** (source `documentId`→`appianDocId`→new `documentId`), copying only source mappings that are `isActive=true`, and returns new `FactorDocumentMapping` rows (`isActive=true`, `createdBy=initiator`, `createdDatetime=now()`). Naturally drops mappings whose factor/doc wasn't carried (matches "reset if factor removed"). Verified via `testRule` (new round 21, source 16 → 3 mappings, doc 52 → factors 43/44/45, `error:null`).
