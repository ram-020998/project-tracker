...
- **`AS_GSS_UT_duplicateEvaluationForNewRound`** (`_a-0000f04a-0c6d-8000-9ba8-011c48011c48_42160`, **v8**): now copies each carried vendor's **documents** into the new round. Added the `documents` relationship (`f017ee11-aaec-48c1-bef6-ba4e201c4017`, ONE_TO_MANY on `vendorId`) inside the vendor `updateRecordsByModelRecord`, mirroring the `businessTypes` pattern. Each doc copied via `updateRecordsByModelRecord` with **nulled** `evaluationDocumentId`(PK), `vendorId`(cascade), `evaluationId`, `consensusId`, `criteriaId`, `taskId`; **preserved** `appianDocId` (same underlying file), `documentName/Description`, `fileType`, `docType`, `documentTemplate`, `documentSubType`, `version`, `isPriceExtractionSelected`, `sourceApplicationId`, `isDeleted`; created/modified reset to initiator/now.
- **Dependency (must verify):** reads `fv!item[…{f017ee11}documents]` off `ri!templateEvaluation.vendor` — only copies if that vendor query loads the `documents` relationship (same as `businessTypes`). If the Setup New Round vendor source (`AS_GSS_FM_startNewRound` → `AS_GSS_GRD_vendorListForSelection`) doesn't fetch it, it silently no-ops → patch that query to include `documents`.
