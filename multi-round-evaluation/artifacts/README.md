...
| `08_IMPLEMENTATION_BACKLOG.md` | **Implementation Backlog** — the whole feature broken into **sequenced stories (US-n, testable) and tasks (T-n, technical)** in actual build order across 10 epics (A–J), synthesized from docs 01/05/06/07 + VM/GCW + deferred UI. Includes epic map, per-item acceptance criteria / DoD, dependencies, and a build-order summary. | ✅ Ready for review |
