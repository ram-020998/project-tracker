...
| `07_APPLICATION_IMPACT_ANALYSIS.md` | **Feature Impact Analysis** — grounds the feature into the whole GSS app: every place a hidden **child-round** evaluation can leak (task grids, ~32 email rules, the Evaluations record list, consensus, documents) or distort an aggregation (task/rating counts, Process-HQ mining). Evidence-graded findings (CONFIRMED / LIKELY / VERIFY) + a numbered PO/team open-questions register (Q1–Q13). | 🟡 In progress (first pass) |
