...
- **Vendor documents carried to new round** (2026-08-26): `AS_GSS_UT_duplicateEvaluationForNewRound` (v8) now copies each carried vendor's `documents` (rel `f017ee11`) alongside the vendor, nulling PK/old FKs and preserving `appianDocId` + content. Depends on the Setup New Round vendor query loading the `documents` relationship (see `03_…` §10.46).
