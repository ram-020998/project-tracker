...
- **WI-1 — Best Value "Start Evaluation" as Round 1** (2026-08-26, built; pending end-to-end UI verification): a **new** Best-Value-only Start Evaluation flow that registers the parent evaluation as **Round 1** and generates ratings/tasks/consensus **scoped to selected factors**. Objects: interface `AS_GSS_FM_startEvaluationBestValue` (`_a-…42569`, PO-verified) + process model `AS GSS Start Evaluation Best Value` (`0000f04b-68ab-8000-fbf5-7f0000014e7a`, clone of `0002ecdd…` minus LPTA + a **Scope Selected Factors** node). Anchor rule `AS_GSS_UT_returnEvaluationRoundsForGivenEvaluation` updated to family resolution (`coalesce(parentEvalId, evaluationId)`) so Round 1 shows in every tab. The **record action** + the guard on the existing `startEvaluation` action are **manual in Designer** (MCP blocked on this RT). Full detail: `06_START_EVALUATION_WI1_IMPLEMENTATION_PLAN.md` §7 + `03_AGENT_ONBOARDING.md` §10.45. Deviations: the two Start Process steps are async SUB_PROC (MCP can't create `start-process-4`).
- **Vendor documents carried to new round** (2026-08-26): `AS_GSS_UT_duplicateEvaluationForNewRound` (v9) copies each carried vendor's documents into the new round. It **queries the source evaluation's vendor documents once** (`vendorId not null`) and maps them per vendor by `vendorId` (nulling PK/old FKs, preserving `appianDocId` + content; cascade re-links via the new `vendorId`). Self-contained — does not depend on the vendor query loading the `documents` relationship. Detail: `03_…` §10.46.
- **WI-2 — Start Round** (2026-08-26, built): starts an already-created child round (down-selected) **without creating a new round**, reusing the existing Start Evaluation process. Objects: confirm form `AS_GSS_FM_startRound` (`…42738`, dynamic round # + included-vendor count) + wrapper PM `AS GSS Start Round` (`0000f04b-9451…`) that calls `0002ecdd…` as a subprocess (evaluation pre-set INPROGRESS, `userAction=SUBMIT`). Action `startRound` (manual). Surfaced in the rounds card on the **latest Set Up** round. Detail: `03_…` §10.48.
- **WI-3 — Complete Round + Setup New Round gating** (2026-08-26, built): backend-only round completion via wrapper PM `AS GSS Complete Round` (`0000f04b-add3…`, no start form; reuses `0004e60d…` Mark-Complete, sets COMPLETE in backend). Action `completeRound` (manual), shown on the **In Progress** round in the rounds card. `setupNewRound` visibility gated so a new round can be set up only when **all existing rounds are COMPLETE**. Helper `AS_GSS_UT_returnLatestChildEvaluationInSetupForGivenEvaluation` (`…43812`). Detail: `03_…` §10.49.
- **Cross-application integration surface mapped** (2026-08-27): GSS integrates with **VM (Vendor Management), GCW (Contract Writing), GSM/DRM, Source Selection, SAM.gov, SharePoint, Azure OpenAI** — 7 connected systems, 15 integrations, 5 Web APIs. Multi-round makes one solicitation = anchor + N round evaluations, which likely breaks single-evaluation assumptions in **GCW status sync** and **VM vendor/proposal** flows (HIGH-impact). Impact analysis phase kicked off in **`07_CROSS_APP_IMPACT_RESEARCH.md`** (inventory + hypotheses + method); analysis-only, no integration changes yet.

### Record-based tab specifics
- **Leaf-eval fallback:** `returnEvaluationRoundsForGivenEvaluation(id)` returns children only, so a leaf/round-clone eval returns `[]`. Instead of the null-default-empty-round, these parents compute `local!roundTabs = if(isNullOrEmpty(roundsRaw), {a!map(evalId: ri!evaluationId, seq: null)}, forEach → a!map(evalId, seq))` and drive the forEach off plain maps. This shows the current eval's own data instead of passing a null evaluationId (which crashes Ratings/Eval-History). **Update (2026-08-26):** with the anchor change now live in `returnEvaluationRoundsForGivenEvaluation`, this fallback is compatible and still handles non-round evals — the planned retrofit of the 4 ID-based parents is **not needed** and was intentionally skipped (no functional gain, avoids destabilizing verified tabs).
...
4. **Round-1 / "Start Evaluation" path** — **ADDRESSED (2026-08-26, pending UI verification).** New Best-Value-only Start Evaluation (interface `_a-…42569` + PM `0000f04b-68ab-8000-fbf5`) registers the root eval's Round 1 via the `round` relationship cascade (`sequence=1`, `parentEvalId` null). Record action + guard on the existing action are manual in Designer. `startNewRound` still handles round 2+.
...
6. **Cross-cutting UI:** round sub-tabs on Ratings/Consensus/Teams ✅, Vendors "Last participated round" ✅, anchor resolution ✅. **Remaining:** Factors "Rounds" column, and Summary showing only current-round factors (mockup p14 — Summary shows current round's factors; Factors tab shows all).
